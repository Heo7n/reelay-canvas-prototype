...
这仍不是生产系统：没有真实生成接口、正式账号生命周期、素材持久化、积分账本或部署链路。当前后端只接受五个固定的 `.test` 演示账号；会话、组织、项目元数据、项目成员关系与路由画布文档已保存到本地 PostgreSQL。登录、主页和项目库只保留 React 正式路由，`index.html` 继续作为迁移期画布 iframe。
...
安装 Node.js 20 或更高版本及 Docker Desktop，然后安装锁定依赖：
...
访问 `http://127.0.0.1:5173/app/login`。五个演示账号都属于 `星海视觉工作室`；组织角色依次为主账户、管理员和成员，个人项目彼此隔离，协作项目另按 `admin/edit/view` 预置项目成员：
...
- 全部项目：个人 / 协作项目是同一组织下的项目访问类型，支持本地搜索、PostgreSQL 持久化的新建与重命名、协作标识、快捷改名笔和统一三点菜单；修改封面、转换归属和删除尚未接入。
- 多账号组织演示：五个固定账号可用于同一组织的权限场景，两个隔离浏览器会话可同时读取和重命名同一协作项目，各自的个人项目不可互见。
...
- 工程工具链：迁移期旧画布继续保留 JavaScript、配置、结构检查与真实序列化 / 只读行为测试；React 壳使用 TypeScript、Vite 与 Vitest，并通过 `npm run check` 一起验证。格式化、lint 和浏览器端到端测试在出现对应代码量与稳定主链路后再引入。
