...
export async function registerSessionRoutes(
  ...
  app.get("/api/session", async (request) => ({ actor: await getRequestActor(request, store) }));
  ...
  app.post("/api/demo/session", async (request, reply) => {
    ...
    const actorId = await store.authenticate(parsed.data.account, parsed.data.password);
    if (!actorId) {
      return reply.code(401).send({ error: { code: "invalid_credentials", message: "演示账号或密码错误。" } });
    }

    const sessionId = await store.createSession(actorId);
    reply.setCookie(DEMO_SESSION_COOKIE, sessionId, { ...cookieOptions, secure: secureCookie });
    return reply.code(201).send({ actor: await store.getSessionActor(sessionId), mode: "demo" });
  });
