(function registerCanvasAssetLibraryMenuController(root) {
  ...
  function create({ grid, placeAnchoredPopover, onDismiss }) {
    ...
    function sync() {
      ...
      if (active?.menu === menu && active.trigger === trigger && menu.matches(":popover-open")) {
        active.schedule();
        return;
      }
      dispose();
      if (!menu || !trigger) return;
      const session = { menu, trigger, schedule, events: new view.AbortController(), observer: null, frame: 0 };
