(function registerCanvasEntityUseView(root) {
  ...
  function resolveCoverMedia(entity, media) {
    ...
    if (requested && normalizeMediaKind(requested) === "image") return requested;
    return media.find((candidate) => normalizeMediaKind(candidate) === "image") || null;
  }
  ...
  function renderEntityDetail(options = {}) {
    ...
    const canAdd = options.canAdd !== false && !busy && Boolean(entityId) && media.length > 0;
    ...
    return `
      <section class="${classNames("entity-use-detail", pinned && "is-pinned", busy && "is-busy")}" role="dialog" aria-modal="false" aria-labelledby="entity-use-detail-title" data-entity-use-detail="${safeId}" data-entity-use-state="${state}" data-pinned="${pinned}" data-placement="${escapeHtml(placement?.side || "right")}"${detailPlacementStyle(placement)}>
        <div class="entity-use-detail-cover" data-cover-kind="${cover ? "image" : featuredMedia ? "preview" : "empty"}" data-media-kind="${normalizeMediaKind(featuredMedia) || "empty"}">
        ...
        <footer>
          <button class="entity-use-detail-add" type="button" data-entity-use-action="add-canvas" data-entity-use-add-canvas="${safeId}"${canAdd ? "" : ' disabled aria-disabled="true"'}>
  ...
  function normalizeSelectedIds(selectedIds) {
    const ids = [];
    const seen = new Set();
    for (const value of selectedIds == null ? [] : Array.from(selectedIds)) {
  ...
  function renderPickerCard(entity, selected) {
    ...
    return `
      <article class="${classNames("entity-use-picker-card", selected && "is-selected")}" role="listitem" aria-selected="${selected}" data-entity-use-picker-card="${safeId}" data-selected="${selected}" data-media-count="${count}">
        <button type="button" aria-label="${escapeHtml(actionLabel)}" aria-pressed="${selected}" data-entity-use-action="toggle-entity" data-entity-use-toggle="${safeId}">
  ...
  function renderEntityPicker(options = {}) {
    ...
    const busy = Boolean(options.busy);
    const canAdd = options.canAdd !== false && !busy && selected.size > 0;
    const cards = visibleEntities.map((entity) => renderPickerCard(entity, selected.has(entity.id))).join("");
      ...
      <div class="entity-use-picker-backdrop" data-entity-use-picker-backdrop="true">
        <section class="${classNames("entity-use-picker", busy && "is-busy")}" role="dialog" aria-modal="true" aria-labelledby="entity-use-picker-title" data-entity-use-picker="true" data-space="${activeSpace}"${busy ? ' aria-busy="true"' : ""}>
          ...
          <div class="entity-use-picker-tools">
            <div class="entity-use-picker-spaces" role="tablist" aria-label="主体空间">
              ${ENTITY_SPACES.map((space) => `
                <button class="${space.id === activeSpace ? "active" : ""}" type="button" role="tab" aria-label="${space.label}，${spaceCounts[space.id]} 个主体" aria-selected="${space.id === activeSpace}" aria-controls="entity-use-picker-results" data-entity-use-action="change-space" data-entity-use-space="${space.id}">
            ...
            <label class="entity-use-picker-search">
              ...
              <input type="search" value="${escapeHtml(query)}" placeholder="搜索主体" autocomplete="off" data-entity-use-search="true">
              ${query ? `<button type="button" aria-label="清除搜索" data-entity-use-action="clear-search" data-entity-use-search-clear="true">${icon("x")}</button>` : ""}
            </label>
          </div>
          <div class="entity-use-picker-results" id="entity-use-picker-results" role="list" aria-live="polite" aria-busy="${busy}" data-entity-use-picker-results="true">
            ${cards || `
          ...
          <footer>
            ...
            <div>
              ...
              <button class="primary" type="button" data-entity-use-action="add-entities" data-entity-use-picker-add="true"${canAdd ? "" : ' disabled aria-disabled="true"'}>
