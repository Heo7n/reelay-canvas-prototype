(function registerCanvasEntityUseModel(root) {
  ...
  function getMediaIdentityKeys(value) {
    ...
    const keys = [];
    const seen = new Set();
    for (const field of ["id", ...sourceIdFields]) {
  ...
  function getSourceMediaId(media) {
    ...
    return normalizeId(media?.id);
  }

  function normalizeMediaRef(value) {
    return normalizeId(typeof value === "string" ? value : value?.mediaId || value?.id);
  }
  ...
  function createEntityMediaPlan(options = {}) {
    ...
    for (const value of media) {
      ...
      const mediaId = normalizeId(record?.id);
      if (!mediaId || mediaById.has(mediaId)) continue;
      mediaById.set(mediaId, record);
    }
