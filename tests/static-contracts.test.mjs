...
test("connection ports keep their external field while media frames accept body drops", async () => {
  ...
  assert.match(html, /styles\.css\?v=20260827-selection-hierarchy-50/);
...
test("multi-selection uses a quiet shared container and one aggregate output port", async () => {
  ...
  assert.match(appSource, /function syncSelectionOverlayProjection[\s\S]*?--multi-selection-frame-radius[\s\S]*?nodeMediaCornerRadiusWorld \* state\.scale/);
  ...
  assert.match(appCss, /\.multi-selection-port\s*\{[\s\S]*?left:\s*calc\([\s\S]*?var\(--multi-selection-port-offset,[\s\S]*?var\(--multi-selection-frame-border-width\)[\s\S]*?width:\s*max\(44px, var\(--multi-selection-port-visual-size/);
