...
test("accepts persisted HTTP and domain Media identifiers without changing Entity reference order", () => {
  const plan = model.createEntityMediaPlan({
    entities: [{
      id: "persisted-subject",
      mediaRefs: [
        { assetId: "workspace-video", order: 0 },
        { mediaAssetId: "workspace-image", order: 1 },
      ],
    }],
    media: [
      { assetId: "workspace-video", mediaKind: "video", displayName: "动作" },
      { mediaAssetId: "workspace-image", mediaKind: "image", displayName: "定妆" },
    ],
  });

  assert.deepEqual(plain(plan.entries).map((entry) => [entry.mediaId, entry.media.id]), [
    ["workspace-video", "workspace-video"],
    ["workspace-image", "workspace-image"],
  ]);
  assert.deepEqual(plain(plan.media).map((item) => item.id), ["workspace-video", "workspace-image"]);
});
