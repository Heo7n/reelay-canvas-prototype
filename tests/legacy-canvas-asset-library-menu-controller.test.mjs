...
function fixture() {
  ...
  menu.showPopover = () => { opened = true; };
  menu.hidePopover = () => { opened = false; };
  menu.matches = () => opened;
  ...
  return { view, grid, menu, controller, frames, flush, move: (value) => { anchor = value; }, rect, opened: () => opened, dismissals: () => dismissals, close: () => { controller.dispose(); dom.window.close(); } };
}
