...
test("detail preview remains non-interactive until pinned and fails closed without usable media", () => {
  ...
  assert.match(empty, /data-entity-use-add-canvas="empty" disabled aria-disabled="true"/);
...
test("picker renders personal and organization tabs, search, checkbox multi-selection, and exact labels", () => {
  ...
  assert.match(markup, /role="tab" aria-label="个人，3 个主体" aria-selected="true"[^>]*data-entity-use-space="personal">\s*<span>个人<\/span>/);
  assert.match(markup, /role="tab" aria-label="组织，2 个主体" aria-selected="false"[^>]*data-entity-use-space="organization">\s*<span>组织<\/span>/);
  ...
  assert.match(markup, /data-entity-use-action="add-entities" data-entity-use-picker-add="true">\s*<span>添加<\/span>/);
...
test("picker resolves ordered Media references, escapes data, and renders useful empty states", () => {
  ...
  assert.match(disabled, /data-entity-use-picker-add="true" disabled aria-disabled="true"/);
