...
- Entity-use 的 model / view / controller 已分开，后续主体选择器、详情、焦点和监听器生命周期进入对应模块。节点生成与提示词优化的注册、timer、取消和完成归属由 `canvas-node-task-runner.js` 独占；`app.js` 保留输入验证、模拟扣费、字段写入、撤销和保存适配。新增节点异步任务复用该边界，不恢复全局 task Map 或另起一套 timer。
