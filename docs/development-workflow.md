...
- `main` 是已验证集成基线，主工作目录可以检出活动 `codex/<具体任务>` 分支；新切片从最新已验证基线开始，同一切片的连续调整复用活动分支，需要并行时再建立独立 worktree。以 `npm run worktrees` 输出的实际路径与分支为准，不根据旧任务标题或目录名称猜测版本。
- 功能验收后通过 PR 合入远端 `main`，再同步本地基线；推送远端、创建 PR 和部署分别核对范围，不能把本地快进等同于公网更新。CI 使用 `package.json` 中声明的 Node 版本。
- 远端 `main` 已启用保护：通过开发分支推送与 PR 同步，满足必需检查后再合并，不直接推送主线或绕过规则。推送前重新核验保护与部署配置；远端归档只补备份尚未被主线或其他远端引用包含的历史提交，避免全量复制冗余分支名。
...
- 已有服务运行时，先确认前端来自当前活动目录、API 使用已记录的数据位置，再直接复用。重启命令、端口诊断和当前机器的数据位置统一见 [本地开发与数据位置](local-development.md)。日常恢复预览不执行初始化或 seed。
...
- 同一轮界面反馈复用活动 worktree 和预览服务；先完成受影响界面的视觉与交互验收，再形成阶段提交。除非用户明确要求，不把每次间距、文案、颜色微调分别变成新 PR 和公网发布。
- 迭代中先按具体改动选择测试文件或单项检查。当前 `check:canvas` 涵盖大部分 legacy 测试，并不等同于单个控件的定向验证；只有影响整个画布时才把它作为最小迭代入口。
- 提交前仍执行第 3 节的完整检查。记录检查所对应的代码、环境和结论；同一代码状态已经通过的检查，不因准备 PR 描述、轮询 CI 或补充发布记录再执行。出现新改动、失败或新的风险证据时，按影响范围补验，阶段提交继续遵守完整检查要求。
- 一次发布候选先完成本次 UI 验收，再构建、部署、验收并提升同一部署。内部持久站与外部体验站明确选择各自目标；候选已验证后不无故重建。发布后的新改进应先判断是否阻断当前使用，避免把无关收尾不断追加到本轮。
- 交接只描述当前代码、数据与下一入口，历史耗时和验收细节优先链接对应 PR。报告耗时时区分实现、验证、CI、部署与等待，不把整轮时长统称为“同步”。
...
- Entity-use 的 model / view / controller 已分开，后续主体选择器、详情、焦点和监听器生命周期进入对应模块。节点生成的注册、timer、取消和完成归属由 `canvas-node-task-runner.js` 独占；`app.js` 保留输入验证、模拟扣费、字段写入、撤销和保存适配。提示词优化只生成建议，由独立 `prompt-optimization-service.js` 与 controller / view 管理；完成不修改内容，手动应用经 adapter 提交一次可撤销写入。新增会修改节点的异步任务复用 runner，非写入建议不混入生成生命周期。
- 节点字段与分组事务复用 `canvas-content-commands.js` 的字段白名单、组规划和双向成员验证；已有离散参数、生成媒体命名、组成员结算和局部布局入口已接入。后续节点结构与素材引用按功能需要继续迁移，不新增整节点快照撤销。治理验收以状态归属和行为保持为准，不要求先完成整个入口文件的拆分。
