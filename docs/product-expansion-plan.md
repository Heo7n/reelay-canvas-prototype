...
3. 一旦引入跨页面任务、稳定 Media 引用、协作写入等新边界，主动复核当前架构是否足够，只补必要的局部治理。普通节点创建与其余 legacy 内容命令按实际功能需要迁移，不机械追求拆完大文件。
4. 阶段结束把经过验证的行为写入实现规范，从本规划移除对应待办；交接留下剩余规则与下一入口。新反馈推翻假设时同步改本表及对应章节，避免旧路线成为永久约束。
