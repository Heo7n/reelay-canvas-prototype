...
2026-09-09 首期生成模式的一条式记录、参考 / Prompt 阅读、7 秒取消、模拟扣费退款、结果自动放入发起画布及开发模拟入口已经接入；已实施行为与职责迁至 [当前产品规范 10.7](current-product-spec.md#107-生成模式的一条式记录与模拟任务)，本节不再保留第二份状态、样式或积分规则。普通 Agent 模式仍保留原有示例消息，消息编排未扩展。
...
- **性能与增量治理。** `app.js` 保持依赖组装与输入 / 积分 / 画布适配，新状态机、任务模板和播放器生命周期继续归独立模块。拆分本身不等于首屏提速；后续任务 / 历史规模扩大时检查入口体积、初始化耗时和媒体请求，再决定分页、按需初始化或加载。按改动范围运行相关检查，里程碑执行统一检查，不把局部任务调整扩大成无关重建。
