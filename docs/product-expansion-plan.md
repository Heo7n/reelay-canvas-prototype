...
2026-09-05 已确认的后续方向：主体库与节点配合、Agent 对话中的模拟生成、项目生成历史、画布整理、消息通知与必要配置，以及分享权限 / 申请加入闭环；协作最终必须支持两位成员以不同账号、不同浏览器真实同时编辑同一画布。它们是产品目标，不表示已经实现，也不自动确定全部规则或执行顺序。本批节点 / 分组基础治理已完成选定入口，剩余治理随产品切片推进；建议路线、阶段验收和进入阶段时的复核机制见第 8.1 节。

用户已启动独立的首页 / 登录设计切片，旧“重设计后置”的建议不再限制该切片。产品面向 B 端，采用可浏览的访客主页与带媒体展示的登录弹窗；首页能力名称、入口组合与正式展示图片仍可继续评审。手机号 / 邮箱注册保留为后续方向，邀请码已经明确用于注册资格，不能据此默认建立组织 Membership。邀请码的发放 / 次数 / 有效期、验证码、密码与账号关联等具体规则仍待定；当前注册入口只保留禁用态。旧登录草稿不替代这些需求，已实现的访客与登录交互见 `current-product-spec.md` 2.1。
...
- 左上角：返回工作台、项目路径、项目名称。
- 左侧中部：内部画布、资产库、分享和个人入口；积分与快捷键说明归入个人菜单。
- 右上角：Reelay Agent。
- 左下角：小地图、适应视图、禁用的整画布整理占位和缩放滑条；此处不再放置资产库入口。
...
- 组织中心已提供组织资料、服务端只读成员目录、积分管理演示和确定性数据驱动的用量看板，包含时间范围、消耗构成、来源分析与导出；它不等于成员生命周期或真实账单已经完成。邀请、角色持久写入、凭证重置、外部分享、团队资产和真实积分账本仍不在当前基础切片中。
...
- 当前参考草稿、消息展示由独立 legacy Agent 模块负责。发送入口未来交出不可变的提示词、模型参数、按顺序排列的媒体引用，以及 workspace / project / conversation 和目标 canvas 作用域；不要让任务读取之后仍可编辑的输入框或草稿。
- 模拟任务状态、唯一任务 ID、取消 / 完成判定与结果记录进入独立 application 层用例和无 DOM 的任务存储；模拟时钟、预置结果与后续真实服务放在可替换执行 adapter。任务卡与项目历史读取同一记录，不在对话、历史、画布各写一套任务 timer 或结果副本。
- 消息与任务卡渲染进入 Agent view；结果放入画布通过既有内容 command 执行权限、撤销和保存。`app.js` 只组装依赖与适配入口，不继续承载新状态机、完整消息模板或媒体生命周期。沿用现有节点 runner 的归属校验原则，但不把 node runner 强行作为 Conversation 服务。
- 生命周期验收覆盖双击发送去重、切会话 / 画布、删除目标、取消后迟到回调、失败与重试；是否扣演示积分、如何退款、结果是否自动建节点在执行切片明确后统一实现。本轮参考素材接入不提前添加任务 timer 或计费路径。
- 文件拆分用于约束职责，并不等同于首屏提速：画布主逻辑仍保留 legacy 入口，提示词内核已独立按需加载，当前边界见产品规范第 12 节。控制媒体按需读取、挂载与释放；后续任务 / 历史能力增加时检查入口体积、初始化耗时与实际请求，再选择延后初始化或按需加载。局部迭代运行对应检查，里程碑统一 `npm run check`；未改应用壳时可只更新 legacy 静态构建，避免无关的重建与整套人工回归。
...
2026-09-09，当前参考区的 `@` 候选、结构化输入与引用生命周期已完成代码接入；运行行为统一见 [当前产品规范 7.3.1](current-product-spec.md#731-提示词中的素材引用)、10.3 与第 12 节。最终整体验证结果以 `agent-handoff.md` 的本轮记录为准，本节只保留后续事项。
...
- **Agent 任务。** 按 §4.8 实现模拟及后续真实 GenerationTask、任务卡和结果记录。当前消息的冻结正文 / 参考快照可作为输入基础，执行切片还须冻结模型、参数、费用及目标作用域，定义幂等发送、失败、取消与结果入画布规则；本轮没有新增任务 timer 或扣费。
- **服务端提示词优化。** 后续接入真实优化服务时，以稳定引用 ID 保护原子关系；提交结果前校验返回引用完整性及所属输入，失败保留原文，不根据显示编号重新绑定。
- **扩大引用范围。** 全库 / 跨项目搜索、文本素材以及跨输入作用域直接保留引用均不在本轮范围。若后续开放，先明确访问权限、媒体加入关系、ID 映射及用户反馈，不能将普通粘贴文字或失效引用自动解释为另一个输入。
