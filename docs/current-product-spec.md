...
- 共同包围框旁显示固定屏幕尺寸的上下文工具条，不额外显示已选节点数量。最左端是“布局”纯图标入口，图案为上方一个横矩形、下方两个小矩形；细分隔线之后依次为“新建主体、打组、保存到素材、下载”。只要选择中包含已有持久组成员，就隐藏打组，其余入口保持各自可用性。下载使用一个包含箭头的单一按钮，点击后展开“批量下载 / 打包下载”菜单；后者在当前静态原型中仍以逐个文件下载模拟。
