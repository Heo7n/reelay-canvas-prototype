...
- “账号设置”在 React 宿主中以桌面弹出面板打开，包含“个人主页 / 积分记录 / 用量看板”三栏；不包含订阅和设备管理。个人主页以左右分栏摘要展示头像、用户名、账号、所属组织与组织角色，不显示演示态标签或重复登录标识；可选联系邮箱与手机号码通过明确的“保存资料”操作提交，格式无效或请求失败时保留当前输入并给出提示。联系资料不改变登录标识，也不会自动订阅报表；后续报表发送必须另设显式开关。积分记录和用量看板在真实账本与任务统计接入前只展示标明边界的空状态，不生成虚构流水。旧画布中的账号设置入口通过 bridge 打开同一面板。
