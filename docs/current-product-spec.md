...
- 用量看板的“预计可用”以近 30 天趋势作为唯一主估算；近 30 天日均与历史日均只作为解释性参考，不生成第二个预计天数，也不随期间分析筛选变化。估算保留零消耗日，不把少量活跃日误当成连续日均。消耗构成使用“图片生成 / 视频生成 / 媒体处理 / Agent 处理”的稳定业务分类，其中“媒体处理”承载高清化、去字幕、提升帧率等可扩展工具，不被限定为编辑增强。类型图例与消耗来源使用不同下钻语义：类型图例展开该业务类型下的“模型或处理工具 → 聚合规格”；模型来源只展开图片与视频生成模型的计费规格、主要使用成员和关联项目；成员来源展开消耗构成、使用模型和关联项目；项目来源展开消耗构成、使用模型和参与成员。所有抽屉一次呈现必要聚合层级，不再要求逐级点击进入二级分析页。模型明细只按影响扣费的维度聚合：视频保留分辨率并在产出列累计总秒数，普通图片保留分辨率并在产出列累计张数，GPT Image 2 保留质量与输出尺寸；画面比例、批量张数和单次视频时长不占据聚合规格列。任务数只作为抽屉顶部的整体规模摘要，不进入模型与规格主表；完整单次配置继续留在任务流水。抽屉不直接复制任务流水，也不替代组织账本。
