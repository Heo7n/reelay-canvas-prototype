...
- 组织中心使用 `/app/w/:workspaceId/organization`、`/organization/credits` 与 `/organization/usage` 三个可深链页面；为减少重复上下文，这组三页只保留品牌头，不重复显示工作台账户入口。组织中心在普通笔记本上保持紧凑桌面排版，并在 `1680px`、`2200px` 以上的宽视口逐级扩展外层面板和内容区，避免大屏继续受笔记本宽度上限约束。组织管理把精简组织信息和成员管理放在同一页；组织卡显示成员数与组织 ID，主账户可在卡片内预览修改组织头像和名称。成员列表来自服务端 Workspace Membership 投影，显示真实演示成员、登录标识与组织角色，不读取项目权限；主账户和管理员可从角色列打开独立选择浮层，主账户角色不可变，管理员也不能修改自身角色。主账户和管理员可打开非主账户成员的账号三点入口；紧凑浮层用于演示重置登录密码和停用 / 恢复账号，但当前登录账号只提供密码重置，不允许停用自身，管理员也不能操作主账户。停用状态只在当前页面生命周期内降低该成员行的视觉权重，不额外显示状态文字，刷新后恢复，普通成员的不可用账号操作显示为 `—`。组织资料、角色和成员账号操作均不会修改共享组织、演示账号、会话或 Membership；系统不展示任何成员的现有密码。积分页以单张“组织积分余额”总览展示当前尚未消耗的 `100000` 积分，并在卡内按“所有成员账户余额 `33000`、可分配余额 `67000`”的顺序展示比例与静态构成，不在构成区重复显示成员数量，也不把余额数字误呈现为导航入口。组织总卡内的“积分流水”入口默认打开“入账明细”，成员余额标题右侧的“分配明细”入口默认打开内部额度流水；同一抽屉可切换“入账明细 / 分配明细 / 消耗明细”，其中入账只记录外部积分进入组织，分配只记录成员额度发放与回收，消耗只记录生成任务扣减，后两类支持按成员筛选且筛选会同步更新摘要。成员表直接显示每人的当前余额和本月消耗，将发放 / 回收收拢为“调整”浮层，并以独立“明细”入口打开该成员筛选后的消耗明细。发放和回收当前只给出前端反馈、不改写余额；演示流水包含可核对的近期生成消耗，但仍明确标注尚未接入真实 `CreditLedger`。组织用量页仅对主账户与管理员开放，使用明确标注且可整体替换的确定性前端演示数据，提供概览与流水明细两种视图、今日 / 本周 / 本月 / 累计 / 自定义范围、消耗趋势与上期比较、任务构成、成员 / 项目 / 模型排行、最近 365 天热力图、组合筛选及 CSV / Excel 兼容导出。组织余额与近 30 日预测不随页面筛选变化，内部分配不计入组织消耗；在 `GenerationTask`、计费快照与不可变 `CreditLedger` 接入前，页面不得把演示数据描述为真实组织账本。
