...
- 没有真实 AIGC API。
- 没有生产账号生命周期、完整云端素材库或积分账本。本地已有首个媒体资产持久化切片，但 Entity、Folder、组织共享、平台审核与积分仍是原型边界。公网原型已通过 Vercel 与 Supabase 部署用于评审，但尚未接入私有 Supabase Storage，不构成生产可用承诺。
...
- 用户、会话、唯一组织 Workspace、Project、项目成员关系、CanvasDocument、WorkspaceMediaAsset 元数据、个人 placement 与 ProjectAssetReference 保存在本地 PostgreSQL；资产二进制交给本地 filesystem ObjectStore。这条最小链路可在服务重启后回读，内存 adapter 只用于快速契约测试和显式开发回退。
...
- 左下角是收起态约 `248 × 44px` 的视口工具胶囊，依次提供小地图、适应视图、暂不可用的整理画布和 20%–200% 缩放滑条。滚轮位于整个工具胶囊及其空白内边距时都不会带动画布。
...
Agent 对话栏展开或调整宽度时，不拖动左上、左侧和左下工具区。生成节点不设置常驻添加按钮；用户通过双击画布打开“添加节点”菜单，明确选择图片生成、视频生成或上传，空状态只提供视觉提示。资产库展开后，左侧胶囊隐藏，项目栏与左下工具仍留在面板上下同一列；面板使用独立的“返回画布 / 资产库 / 空间切换”头部，不复制项目导航。资产面板与上方项目栏、下方画布工具条均由共享尺寸变量保持约 `8px` 呼吸缝，三块既不相粘也不过度松散。数量统计继续位于面板底部；当前按桌面视口验收，历史窄屏规则仅作为防御性降级保留。
...
资产库现在直接进入独立的资产管理面板，不再先展示当前画布节点树，也不再通过书本图标进入第二层“全局库”。面板头部使用 `资产库 + 个人空间 / 组织空间 / 平台空间`；个人与组织空间以 `素材 / 主体库` 分栏，平台空间则进入单一的 `灵感搜索`。

当前交互：

- 面板首选宽度为 `550px`，右边缘可用主指针拖拽或键盘方向键在 `380–780px` 间调整。键盘步进始终从当前实际宽度开始，separator 的当前值和有效上下限与画面同步；和 Agent 同开时可以临时压缩，但会保留用户最近选择的首选宽度，Agent 关闭后恢复该首选宽度。个人 / 组织空间的素材与主体库固定顶部目录命令行，只让下方卡片内容区独立滚动；平台空间没有目录行，搜索与命令组固定在顶部。
...
- 只有模型目录中 `brand: "seedance"` 的 Seedance 系列视频模型显示“添加主体”入口；图片模型和 Kling 不显示，也不为参考素材列保留空槽。入口使用专用的主体卡图标，打开标题为“主体库”的近工作区尺度多选面板，只列出个人 / 组织空间主体，支持空间切换、宽搜索框和跨空间多选；卡片区独立滚动，底部计数与“取消 / 添加”固定。确认后按主体及其引用顺序把去重 Media 展开为现有参考素材；和目标节点中已有素材重复的条目会跳过，展开后的素材可独立删除和排序，不保留持续 Entity 绑定。一次确认只形成一条节点撤销记录；切换到不支持主体引用的模型只关闭入口与已开面板，不删除已经展开的参考素材。
...
左侧中部胶囊中的资产库是当前画布使用和整理可复用媒体的入口。其 Media 上传已接入首个服务端纵向切片：`WorkspaceMediaAsset + personal placement + ProjectAssetReference + ObjectStore`；Entity、Folder、组织 / 平台 placement、共享与审核仍由无 DOM 页面内存 store 承载。

空间与分栏：

- `个人空间` 与 `组织空间` 在当前前端原型中可编辑。`平台空间` 是独立的只读内容源：进入后固定到仅含 Media 的“灵感搜索”，不显示主体库、目录、上传、视图切换或条目修改菜单；搜索框与图片 / 视频 / 音频筛选用于缩小全平台结果。平台条目支持多选，但批量操作只提供“添加到画布 / 保存到素材”：前者把所选 Media 加入当前画布，后者在页面内存 store 中导入一条独立的个人 Media 副本并保留平台来源标识，按平台源幂等；个人副本后续改名不会修改平台目录记录。该差异只用于验证交互，不代表公共素材服务与服务端导入策略已经完成。
- `素材` 存放图片、视频和音频；可按全部、图片、视频、音频筛选，并在网格与列表视图间切换。
- `主体库` 存放由若干素材组成的主体，不重复保存媒体内容，并与素材分栏一样支持网格与列表视图切换。每个主体只保存去重、有序的 Media 引用和可选封面 ID；封面一旦设置，必须指向引用集合中的图片，视频和音频不能成为封面。主体卡片只保留封面、名称和操作入口，不在网格角标或列表副文案重复显示“几个素材”；卡片优先展示所选图片封面，没有显式封面时只可回退到首张引用图片，纯视频 / 音频主体显示占位，不再拼贴其他引用。编辑器和详情仍从引用解析全部媒体，名称不使用媒体文件扩展名。
- 同一 Workspace Media 可以通过 placement 出现在个人或组织空间，空间共享不会克隆 Media 记录。平台空间使用独立只读目录记录；保存平台条目时导入个人副本并记录其平台来源，而不是把平台目录记录变成可写 Workspace Media。文件夹通过 placement 决定个人 / 组织条目所在目录；旧的按来源和类型派生重叠“虚拟文件夹”已经移除。
...
- 主体库的“创建主体”进入专用两栏编辑工作区：新建态标题为“新建主体”，编辑已有主体时标题直接显示主体名称，关闭入口位于标题栏左侧；左栏宽度继承资产库当前宽度（默认 `550px`，并跟随用户调整），编辑名称、描述和已添加素材，右栏查看图片、视频或音频，只有当前预览为图片时才在媒体左上角提供“设为封面 / 当前封面”，与添加素材卡片左上角的封面标识使用同一视觉锚点，底部统一使用“取消 / 完成”。底部操作区不使用贯穿面板的明显分隔线，以按钮自身边界完成视觉收尾。左栏中的名称、描述、“添加素材”标题及工具保持固定，只让添加素材下方的卡片区独立滚动；卡片保持固定比例并按完整行高排列，滚动区下缘直接衔接操作按钮顶边。右栏是连续的中性灰媒体检视舞台，不再使用白色内容区套预览卡；媒体按真实比例在舞台中略微上移并最大化，横图优先利用宽度、方图和竖图优先利用高度，媒体左下角轻量显示当前序号与原始尺寸，音频使用紧凑舞台且所有媒体均不裁切。素材可通过“从素材库添加”的多选器引用当前空间既有 Media，也可通过“上传”新增 Media 后自动加入草稿；多选器计数只表示本次新增选择，已有引用保持锁定。上传内容先登记到素材分栏，Entity 始终只保存 Media 引用；取消 Entity 草稿不会回滚已经完成的 Media 上传。从资产库进入编辑器后关闭时，右侧预览收拢，左栏在原位交叉过渡回资产库，不再次触发资产库的侧滑入场；窄屏与减少动态效果环境直接完成状态切换。
- Entity 创建和更新要求至少一个可见 Media，引用按首次出现去重并稳定排序；封面可为空，非空时必须来自引用集合且为图片。编辑共享到多个空间的 Entity 时，会先验证更新后的全部引用在其现有 placement 空间中都可见，再原子提交；校验失败不会留下部分更新。Entity 本身刷新后仍会重置，当前没有版本历史或服务端 Entity repository。
...
- 卡片右上三点菜单默认在触发按钮右侧顶对齐展开，临近面板右缘时自动翻到卡片左侧，避免裁切。主体单项菜单固定为“编辑 / 重命名 / 移动 / 复制到组织空间 / 删除”；主体批量菜单只有“批量移动 / 复制到组织空间 / 批量删除”，不提供提交审核。素材分栏中的图片与视频仍可提交 Seedance 合规审核，但音频素材单项以及任何包含音频素材的批量选择不提供该操作；组织空间不显示“复制到组织空间”。这里的“复制”只新增 organization placement，不复制底层 Media 或 Entity；审核仍只在内存中登记幂等请求并显示反馈，没有接入审核服务或结果页面。
- 多选只作用于当前筛选后可见的条目；所有素材卡、主体卡和主体编辑器素材选择卡统一在左上角使用一个选择标记：待选态是半透明浅色实心方块，选中态替换为浅色实心方块与清晰深色勾，不同时并列展示两个状态，也不使用容易误解为“新增”的加号。个人 / 组织空间命令组按 `全选 → 取消 → 筛选 → 视图` 排列；平台空间没有视图按钮，保持 `全选 → 取消 → 筛选`。“取消”会明确退出多选并清空当前选择。切换空间、分栏、目录或搜索 / 筛选上下文也会清理不再可见的选择，避免跨上下文误操作。
...
- 点击素材卡图区会打开模态预览：图片显示大图，视频和音频提供原生播放控件。主体卡 hover 或键盘聚焦时在当前卡片右缘约 `10px` 的近邻位置显示轻量详情快看，而不是固定贴在整个资产面板外缘；移入详情不会意外收起，点击主体卡会固定详情，主体编辑继续从三点菜单进入。离开卡片时会取消尚未触发的延迟打开，普通 hover 也不能覆盖已固定详情，避免横向移动途中误切到相邻主体。详情以来源卡顶边为首选锚点并做视口碰撞与 Agent 避让，空间不足时才翻转或平移；上部用大尺寸主体封面与中性灰信箱留白保留真实比例，下部依次放置同排的名称 / 素材数、描述和整宽“添加到画布”，不保留厚重标题栏或缩略图带。详情层使用固定定位，不随资产内容滚动；平台空间不提供主体分栏。
- 固定主体详情的主按钮文案为“添加到画布”。执行后按主体引用顺序把尚未存在于画布的 Media 展开成独立素材节点，围绕当前可用视口居中排列，并临时全选新节点；不会创建组框，也不会让节点持续绑定 Entity。重复素材按稳定来源标识跳过，整批创建只形成一次撤销边界。
- 素材预览可把 Media 添加到当前视口中心，或在由生成节点发起时引用到该节点；素材卡也可以直接拖到画布或生成节点。
- 卡片名称区域不再响应双击重命名；指针重命名通过更多菜单进入，键盘仍可使用 `Enter / F2`。列表视图下整行提供统一的悬浮反馈，点击行可打开文件夹、预览素材或固定主体详情，多选状态下则切换该条目选择。平台卡片没有修改菜单。
- 当前项目可列出 ProjectAssetReference 并通过受权限保护的内容地址读取媒体。画布节点消费时仍会建立 legacy 媒体投影，`workspaceAssetId` / `librarySourceId` 只是迁移期来源关系，还不是 Node 级正式引用。资产二进制、Entity、Folder、审核和 UI 选择状态都不写入 CanvasDocument。

持久化边界：

- 初始素材、主体、文件夹和组织 / 平台 placement 仍来自确定性 seed；个人空间包含 Image 2 生成的“雾森信使”和“曜石勘探体”两套主体样例，每个主体包含 3 张一致素材，覆盖宽幅主视觉、`3:4` 人物近景、`1:1` 装备 / 结构资料与 `4:3` 机械侧视，用于验证卡片和主体预览的多比例适配。除上述个人 Media 上传与项目挂载外，创建 Entity、目录整理、组织共享和审核只保存在页面运行内存中，刷新会重置。平台内容只是只读 seed，不是已上线的公共目录服务。
...
- Entity / Folder repository、organization placement、平台目录版本与导入策略、组织资产权限、Node 级引用、删除 / 恢复和 GenerationResult 显式晋升仍未实现。公网还需私有 Supabase Storage adapter，不能把本地文件系统当作云端对象存储。
...
- 资产库 UI：`librarySection`、`librarySpace`、`libraryFolderId`、`librarySearch`、`libraryFilter`、`libraryDisplay`、多选 / 菜单 / 重命名 / 移动 / 素材预览状态，以及个人 / 组织按空间与分栏、平台按单一搜索上下文记忆的搜索和筛选；主体编辑器另持有冻结 draft、当前预览、封面、已添加素材筛选和素材选择器临时多选状态。Entity、Folder、organization placement、平台目录及其导入副本 仍由页面内存 asset-library store 持有，已持久 Media 由宿主 repository 投影进来。这些 UI 状态和资产实体都不写入 CanvasDocument。
- 主题：`themeMode`。
...
- 会话、账号联系资料、Workspace、Membership、Project、CanvasDocument 以及首个 WorkspaceMediaAsset / personal placement / ProjectAssetReference 切片已通过 PostgreSQL 持久化；本地资产二进制使用 filesystem ObjectStore。可重复 migration、幂等写入和重启集成测试覆盖这些边界。画布文档仍处于迁移桥阶段，Entity / Folder、生成任务与积分仍只存在原型状态。
...
- 资产库已落地 WorkspaceMediaAsset、personal placement、ProjectAssetReference 与 ObjectStore 的最小切片，但画布消费仍会创建 legacy 媒体投影。Entity / Folder / organization placement 还在无 DOM 内存 store 中，公网私有对象存储、Node 级引用、回收站和 GenerationResult 晋升也尚未落地。
- 完整演进顺序只在 `docs/product-expansion-plan.md` 维护，本说明不再保留第二套路线路。

## 15. 当前产品一句话

Reelay 当前是一个本地可运行的 AIGC 创作平台原型：十个演示账号通过受保护的 React 工作台进入同一组织，使用持久化的个人 / 协作项目和项目级 `admin/edit/view` 权限，再进入旧无限画布验证生成节点、媒体组织和 Agent 辅助；首个个人媒体资产与项目引用可在本地持久恢复，但正式账号生命周期、完整画布模型、Entity / Folder、生成任务与积分仍未达到生产持久化标准。
