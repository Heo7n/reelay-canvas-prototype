...
- 初始空状态：中心显示紧凑标题“开启创作之旅”，下方为现有光标图标与“双击画布创建节点，或拖拽上传素材”说明，再下方排列“图片/视频生成 / Agent一站式创作 / 影视灵感与拉片复刻”三个带图标的展示块。点击提示图标为 18px；展示块高约 40px，采用低对比底色与细边框，第三块稍宽以容纳长文案，窄屏前两块并排、第三块独占一行；光标、线性图标、底色与边框随深浅主题适配。整体仅为视觉引导、不提供点击入口、不拦截画布手势，影视灵感与拉片复刻功能尚未实现；只读空画布显示“画布暂无内容”，隐藏图标、操作说明及展示块。
...
- 右侧按“添加 / 搜索 / 筛选 / 多选”排列，常态四项等分 204px 操作区，统一 28px 高、16px 图标与 6px 间隔；常态无底色和外框。目录中的加号展开 176px 宽的紧凑菜单，依次为“新建文件夹 / 上传资产”；新建在当前目录下进入目录树行内命名，第五级或无权限时禁用，上传沿用当前空间与目录。菜单支持再次点击、外部点击或 Escape 关闭，Escape 归还加号焦点，方向键选择菜单项。主体库加号直接新建主体，成员中原位提供编辑。进入多选时，右侧四个图标在同一行替换为“操作 / 全选 / 取消”，三项均常显图标与文字，每项 64px 宽、左右 8px 内边距；操作在未选择素材时禁用。右侧常态四项与多选三项铺满同一 204px 宽度，左右边缘对齐，保持左侧目录位置稳定；已展开的搜索保留在左方，不覆盖三项操作，关闭搜索恢复来源浏览状态与可见入口焦点。
...
- `个人空间` 的持久目录和自定义标签只对本人可见；`组织空间` 的持久 Media 目录和自定义标签由当前 Workspace 成员共享，成员可直接保存素材，不隐式发起审核。组织成员身份由服务端独立校验，不用项目编辑权限替代；`平台空间` 是扁平的只读灵感搜索结果，只展示 Media，不显示目录、上传、新建、主体入口或单条修改菜单。平台结果仍可搜索、筛选、预览和进入多选；多选只提供“添加到画布”和“保存到素材”。前者会从结构化 Media 投影创建真实画布节点；后者在后端受控 import、来源授权与个人 placement 写入命令落地前保持禁用，不能用页面内存 placement 冒充已保存。平台导入与主体共享不随组织 Media 保存一起开放。
...
- 主体专区的“新建主体”和普通素材多选操作中的同名入口进入独立编辑工作区，主体卡直接点击、菜单编辑及详情编辑按钮也打开同一编辑器。新建标题固定“新建主体”，编辑标题跟随主体名称。编辑器采用左侧加宽的单栏面板，首选宽 720px，普通资产库的宽度偏好不变。顶部名称 / 标签与右侧描述约 3:7，描述跨两行；下方为分类 / 添加工具行和固定四列、最多两行高度的素材区，超出部分独立滚动。素材名称在图块下单独显示并保留文件后缀；再下方为当前素材的大预览，底部取消 / 保存固定。编辑器提供成员 `全部 / 图片 / 视频 / 音频` 筛选、从个人素材库添加、上传、单项预览和移除，仅图片可设封面。下方预览标题显示媒体类型与文件名，双击或 F2 可编辑主文件名并保留扩展名，只修改个人 placement 名称，不改源文件、画布或其他空间。右上“设为封面 / 当前封面”使用同尺寸文字按钮或状态，左侧对应成员显示轻量封面标记。成员筛选不是全库筛选；主体至少引用一个 Media 才能保存。
...
- 编辑器素材区不足两行时按实际行数收紧；点击预览、设置封面或编辑标签保留素材区滚动，切换分类回到顶部。短桌面窗口降低控件与缩略图高度，为下方预览和固定操作保留空间；不隐藏预览，也不新增手机布局。退场使用整块面板淡出，取消旧左右面板分离动画。
- 主体编辑器下方的大图 / 视频预览按剩余可用区域等比完整适配并居中，竖图、横图和窗口缩小时都不裁切；预览轨道由容器尺寸约束，不能被媒体的固有尺寸撑大。资产与编辑器的小卡片仍使用统一比例的封面缩略图，大图预览和放入画布使用完整原图。
- 主体编辑器顶部标题栏将主体图标与名称放在左侧，关闭按钮放在右端，长名称截断而不挤压关闭按钮；打开时焦点落在标题栏右侧关闭按钮，名称保持普通显示，用户主动点击后再编辑；必填校验失败仍定位名称输入。编辑器标题使用标准 Lucide `square-user-round` 主体图标，预览文件名按图片 / 视频 / 音频使用 `image / square-play / audio-lines`，两处统一 `18px`、`1.8px` 线宽，使用本地官方 SVG 路径。底部操作区高 `48px`，取消 / 保存按钮高 `32px`，不额外绘制上分割线，素材滚动区延伸至操作区上缘。
...
- 主体编辑器通过 X、取消、Escape 或成功保存返回时，保留实际面板完成约 `200ms` 退场：左栏原位淡出，与底下同位置恢复的主体库衔接，资产库不重复从左侧滑入；右预览在 `160ms` 内向右 `12px` 淡出，外壳背景同步消退，不缩放媒体。`≤720px` 上下排列时，预览改为向下 `8px`；减少动态效果直接切换。未保存的草稿先确认再退场，选择继续编辑保留当前画面；确认 / 退场期间阻止重复关闭和编辑，重新打开或销毁会取消旧动画并使旧回调失效。退场结束才释放画布交互，焦点返回库中的原主体入口，找不到时回到资产库入口。
