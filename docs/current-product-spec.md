...
- 没有真实 AIGC API。
- 没有生产账号生命周期、完整云端素材库或积分账本。本地已有个人 Media 与 Entity 持久化；实验分支新增个人 / 组织 Media 的保存目录、标签和放置，尚未迁移共享或公网环境。素材组目录与共享、平台审核及积分仍保留原型边界。公网原型已通过 Vercel 与 Supabase 部署用于评审；私有 Supabase ObjectStore 配置与三主体 12 图迁移已完成，正式主域已通过资产 / 主体 HTTP 读取、鉴权、大小限制与三主体浏览器展示验收，不构成生产可用承诺。
...
- 进入画布只注册可访问的素材、素材组与保存目录，不为整个资产库预读原图或探测媒体尺寸。资产库关闭时不生成隐藏的媒体预览；打开后按当前列表展示，实际使用素材或编辑时仍保留必要的尺寸读取。
- 同时进入的相同默认 API 读取共用进行中的网络请求，避免开发态重复挂载多读一遍画布和目录；结果完成后不缓存，再次进入仍读取服务端。登录、退出、保存等写入会隔开前后的读取，不复用写入前的进行中请求。
- 进入画布先请求文档，文档成功返回（包括尚无文档的空画布）后才并行请求个人素材、素材组、个人 / 组织 Media 保存目录和项目资产目录；文档失败、已离开的路由或失效重试不启动目录读取。文档与 iframe 就绪后即可恢复编辑，不等待目录；目录随后补齐，加载中或失败有明确反馈，依赖目录的操作在对应目录就绪后启用。晚到的目录不会重置节点、任务、撤销或保存状态；旧 iframe 保留原握手兼容路径。
...
本节及第 7.6 节描述 `codex/experiment-20260910` 的资产库实验，基线为 `f21f3f6`；公网最近发布记录仍见当前交接。本实验将原来的“主体”在产品界面中统一为“素材组”，与单份图片、视频、音频混排，不再提供素材 / 主体分栏。内部 Media / Entity 记录及其有序引用继续复用。

当前交互：

- 面板不单独显示“资产库”标题，保留无障碍名称。顶部只有两行紧凑操作区：第一行常驻带线性图标的 `个人 / 组织 / 平台` 等宽空间导航，收起按钮位于面板右边缘，垂直中心与左侧资产库入口一致，不占顶部导航；窄至全屏面板时收起移至面板内以避免越界；以底部线标识当前空间，不使用分段底色；导航与下方目录左边缘对齐；目录框与“个人”导航列等宽，右边缘对齐选中线，搜索紧接目录框右侧，调宽时保持此关系。目录、搜索与工具组采用轻描边和统一 28px 高度。第二行为目录、添加目录、搜索与右侧上传和多选 / 筛选按钮。空间切换支持方向键、Home / End 和可见键盘焦点，不再使用原悬停空间菜单。
- 个人 / 组织的搜索图标紧接添加目录右侧；悬停展开但不抢焦点，点击展开并聚焦输入，以 180ms 从左向右揭开的过渡覆盖上传与命令组，命令组同时退出键盘和读屏访问。空搜索在移出 140ms 且输入未聚焦时收起，已有查询保持展开。关闭或 Escape 清空本次搜索、收起输入并返回搜索按钮焦点；输入法组合中的 Escape 不误关。平台搜索框常显，与右侧多选 / 筛选并排，不覆盖或禁用工具；清除与 Escape 只清空查询，保持输入可用。减少动态效果时取消过渡。搜索当前目录的单素材与素材组，类型筛选对组按其引用媒体类型匹配。平台仍跨平台目录搜索 Media。
- 目录命令行控件高 28px，比空间栏更薄；多选期间右侧两个工具原位切换为“全选 / 取消”，上传原位切换为同宽的批量“操作”。不新增一行，目录与搜索的位置保持不变。搜索、空间或目录改变后清理不可见选择，切换空间退出原多选和组内上下文。
- 面板首选宽度仍为 `550px`，右边缘主指针拖拽或键盘方向键在 `380–780px` 间调整；和 Agent 同开可临时压缩，并在 Agent 关闭后恢复用户首选宽度。顶部操作固定，只让下方卡片内容区独立滚动。平台隐藏目录但保留搜索与命令行。
- 网格卡片约 `164px`，单素材与素材组共用网格卡片基底，不提供列表视图；组使用单封面、轻叠层及 `N 项` 标记。调宽改变列数而不拉伸卡片。右缘继续使用 `12px` 调宽命中区、原生分栏光标和悬停细边线。
- 进入素材组只展示其可访问的有序引用；常驻“资产库”返回入口和组名。返回恢复进入前的目录、搜索词、类型筛选和滚动位置。组名读取当前记录，失效记录显示不可用，不自动扩展到全库。组内不直接上传，编辑入口打开既有编辑器后添加素材，避免上传成功却不属于当前组。
- 面板收起时使用 `inert` 和 `aria-hidden` 退出键盘与读屏访问，焦点返回资产库入口。极窄视口仍全屏降级并隔离背景焦点，此时隐藏调宽器；回到桌面恢复首选宽度，不代表已经验收移动端画布。
- 默认目录计为第 1 层，总计最多 5 层；只展示直属子目录和条目，父级与同级通过目录树切换。“保存到素材”创建的个人 / 组织 Media 目录与放置已持久化，并同步回资产库。旧内存目录及素材组目录保持独立，不自动迁移为新的统一目录。
- 个人 / 组织底部数量统计当前目录的单素材与素材组条目，筛选后显示“当前结果 / 当前目录总数”；组内统计引用素材，平台显示结果数。
...
左侧中部胶囊中的资产库是当前画布使用和整理可复用媒体的入口。Media 上传已接入 `WorkspaceMediaAsset + personal placement + ProjectAssetReference + ObjectStore`；个人根目录 Entity 也已接入独立 repository、乐观版本和 iframe bridge。实验分支的“保存到素材”已接入个人 / 组织 Media 的目录、标签和 placement 元数据持久化；素材组目录、平台目录及旧共享 / 审核原型仍保留原边界，不等同于这些能力全部上线。

空间与内容：

- `个人空间` 的持久目录和自定义标签只对本人可见；`组织空间` 的持久 Media 目录和自定义标签由当前 Workspace 成员共享，成员可直接保存素材，不隐式发起审核。组织成员身份由服务端独立校验，不用项目编辑权限替代；`平台空间` 是扁平的只读灵感搜索结果，只展示 Media，不显示目录、上传、新建、素材组入口或单条修改菜单。平台结果仍可搜索、筛选、预览和进入多选；多选只提供“添加到画布”和“保存到素材”。前者会从结构化 Media 投影创建真实画布节点；后者在后端受控 import、来源授权与个人 placement 写入命令落地前保持禁用，不能用页面内存 placement 冒充已保存。平台导入与素材组共享不随组织 Media 保存一起开放。
- `素材` 存放图片、视频和音频；可按全部、图片、视频、音频筛选，统一采用网格视图。
- 单份素材和素材组在同一目录混排。素材组不重复保存媒体，只保存去重、有序的 Media 引用；使用一个显式封面或首个可视 Media，轻叠层和 `N 项` 标记区别于单份素材。
- 素材组与文件夹独立：创建组不自动建立日期或同名目录，改名、增减引用不移动原文件。混排按创建时间最近优先，改名不改排序，缺少时间保留稳定顺序。点击组卡或菜单“查看内容”按引用顺序跨目录展示可访问素材；组内返回恢复原目录、搜索、筛选和滚动，不创建虚拟 placement 或真实文件夹。失效组显示不可用，不能静默回到全库。
- 同一 Media 可以通过 placement 出现在个人、组织或平台空间，空间共享不会克隆 Media 记录。文件夹也通过 placement 决定条目在对应空间中的目录位置；旧的按来源和类型派生重叠“虚拟文件夹”已经移除。
- 顶部空间行和目录命令行采用第 6.1 节的两行布局，个人 / 组织搜索展开覆盖右侧命令，平台搜索常显且与工具并排。多选期间上传原位切换为同宽的“操作”，两个工具原位切换为同宽的“全选 / 取消”，不新增行或挤动左侧目录、搜索。未选择时批量操作禁用，退出多选恢复上传与筛选入口。

创建与整理：

- 混排列表的主操作是多文件上传，接受图片、视频和音频。在受保护的 React 画布宿主中，个人空间上传会创建带 checksum 的 upload intent，写入 ObjectStore，完成 WorkspaceMediaAsset 登记并幂等挂到当前项目。上传完成仍先建立 personal root placement；选择其他目录或组织空间需经下述保存流程明确处理。宿主内非个人空间直接上传仍明确拒绝，直接打开静态画布时才使用页面内存媒体。
...
- 多选操作中的“新建素材组”进入独立编辑工作区；组卡菜单及组内编辑按钮也进入同一编辑器；新建时标题固定为“新建素材组”，编辑既有记录时标题跟随素材组名称。左侧编辑栏跟随资产库的当前首选宽度；名称、描述、“添加素材”标题及其下由素材分类和“从素材库添加 / 上传”组成的工具行固定，只让工具行以下的素材卡片区独立滚动。编辑器包含该素材组已添加素材的 `全部 / 图片 / 视频 / 音频` 四类筛选、从个人素材库添加、上传、单项预览与移除；只有图片可以设为封面。预览头部左侧以单行的媒体类型图标和文件名称标识当前素材；双击文件名称或按 `F2` 可原位重命名，输入只修改扩展名前的主文件名，扩展名固定保留，修改只写入当前用户的 personal placement 名称，不改写底层媒体、已有画布节点或其他空间的名称，也不在 Entity 内复制名称。右上角使用约 `80 × 30px` 的纯文字圆角按钮显示可执行的“设为封面”，已为封面时原位替换成同尺寸、非交互的“当前封面”状态；不使用旧书签图标。左侧对应素材卡片常显轻量“封面”标记。四类筛选只改变当前素材组素材的显示，不是全局资产筛选；素材组至少引用一个 Media 才能保存。
- 受保护宿主中的新建与编辑分别调用个人 Entity 的幂等创建和带 `expectedVersion` 的更新。新上传内容先走现有 Media 持久化链路，再由 Entity 保存资产 ID；上传成功但素材组保存失败时 Media 仍留在个人素材库，不伪装成跨资源原子事务。版本冲突失败关闭并保留编辑草稿供用户处理。
- 画布多选“新建素材组”打开同一编辑器，预填当前选中节点的可用素材：素材节点只取当前展示项，生成节点只取已完成结果，不把输入参考或历史结果自动加入。按节点从上到下、同行从左到右排序，再按稳定 Media 标识去重；支持图片、视频和音频，首张图片默认作为封面，没有图片时不强设封面。混合选择说明初始带入、合并与跳过数量；至少有一个可用素材即可创建，超过 100 个去重素材则明确要求减少选择，不静默截断。目录加载 / 不可用、只读、无有效素材等状态在工具栏解释原因。
- 个人目录已确认的素材直接复用。尚未入库的同源 Blob、图片 / 视频 / 音频 Data URL 以及受控同源 `/assets/` 媒体，可随草稿预览；仅在确认创建时读取原内容，核验类型、大小与 SHA-256，经既有 personal 上传链写入默认目录，再用返回的稳定 ID 创建素材组并映射封面。创建页随当前草稿更新新增入库数量；未确认取消不触发入库，未入库素材的改名属于草稿。正常宿主上传画布原本就建立 personal placement，此路径不会重复上传它；新导入按真实内容去重，已有素材保持原目录，不建立文件夹或改写画布。未知跨域 URL、无个人目录归属的既有持久资产不自动复制，跳过原因明确；SVG 不在此自动导入范围。该能力只承接现有画布媒体，不代表正式 GenerationResult repository / 晋升命令已实现。
- 批次按顺序完成素材入库；入库成功但后续素材或素材组保存失败时，已入库素材保留，草稿可重试。导入复用稳定幂等键及已确认的素材，未改动的素材组创建请求重试复用同一幂等键；不把独立媒体与素材组写入描述为跨资源原子事务。沿用现有上传大小和凭证有效期限制，项目 / 画布实例 / 权限变化后不继续后续写入。
- 多选创建默认保存到个人空间；名称仍需填写，可调整描述、素材及封面后确认创建。打开草稿不写素材组、画布或撤销记录；未编辑时取消直接回到原选择和工具栏焦点，修改后沿用放弃确认，成功后进入个人资产库并定位新素材组。创建不移动、打组或替换选中节点，也不建立持续绑定；项目 / 画布实例 / 写权限在提交前与异步完成后校验，旧上传或改名回调不污染新的编辑草稿。较矮的桌面窗口允许左侧表单滚动，保留素材可操作高度及固定的创建 / 取消区域。
- 默认目录计为第 1 层，总计最多 5 层。新的持久目录只承载 Media；素材组与旧内存目录仍保留内部 kind 和 placement 边界，不新增持久素材组目录、跨类型搬移或文件夹共享。已保存目录的改名、移动等尚未接入的操作不会以页面内存修改冒充成功；旧内存目录的操作继续校验同级唯一、同空间、无循环及深度上限。
- 文件夹移动仍使用目录树选择目标，保留当前目录高亮与内部滚动。包含素材组的目录共享不可绕过素材组的能力限制；旧内存 placement 行为不代表组织共享已持久化。
- 卡片三点菜单在浏览器顶层靠触发按钮展开，按视口避让；滚动跟随，按钮离开列表或关闭库时收起。单 Media 保留适用的重命名、移动、组织复制、删除和审核原型；个人组卡显示查看内容、编辑素材组、重命名与删除，不显示审核、移动或共享。持久 Entity 编辑与重命名沿用版本检查；持久 Media 的保存位置和标签通过“保存到素材”弹窗修改，未接入持久化的旧整理动作不冒充成功。
- 多选只作用于当前可见条目，以 `${kind}:${id}` 保存选择，避免 Media 与 Entity 同 ID 相互污染。卡片悬停或键盘聚焦露出复选框，点击进入多选；多选时常显，触摸设备保留入口。全选、取消和上下文清理作用于网格。批量“添加到画布”及“新建素材组”复用当前空间的有序 Media 引用解析并去重，再校验目标和权限；包含组的选择不提供审核、移动或共享；删除按实际选中的组与单素材执行，不把组操作偷换为删除全部组内媒体。
...
- 可共享的单 Media 只新增 organization placement，不复制媒体；文件夹共享沿用既有类型及子树权限限制。本实验不通过文件夹路径开放素材组共享，新的组织 Media placement 只通过已授权的保存命令写入。
- 单素材、个人素材组、混合多选与文件夹支持持久删除。确认框说明范围，提交期间防止重复操作，失败留在确认框内并可重试；成功后才更新资产库。删除只移除当前空间条目，保留底层媒体和已经放到画布上的内容；删除素材组保留组内素材。删除文件夹递归处理子目录与素材，默认目录不可删。若素材仍被本次未删除的素材组引用，整批拒绝，不能部分删除；组版本过期同样拒绝。个人仅操作本人条目，组织删除限组织所有者和管理员，成员仍可上传贡献，平台不提供删除。服务端校验权限与引用关系，删除与重试不复活已移除的组条目；跨项目、画布或空间后的旧确认不继续写入新上下文。当前没有回收站和恢复入口。
...
- 个人项目画布文件和个人资产库共用个人容量；协作项目文件和组织资产库使用组织容量。服务端在创建上传时根据真实项目权限或显式空间确定存储归属，源文件归属创建后不随 placement 的共享、移动或增加而改变。个人跨 Workspace 合并统计；同一对象多个画布、目录或素材组引用只计一次，独立上传的副本分别计量。默认个人、组织各 10 GiB（界面显示 GB），可按归属账户配置限额；额度下调不删除已有媒体，只阻止超额新增。
...
- 素材组编辑器右侧的大图 / 视频预览按可用区域等比完整适配并居中，竖图、横图和窗口缩小时都不裁切；预览轨道由容器尺寸约束，不能被媒体的固有尺寸撑大。资产与编辑器的小卡片仍使用统一比例的封面缩略图，大图预览和放入画布使用完整原图。
- 素材组编辑器左标题栏将素材组图标与名称放在左侧，关闭按钮放在右端，长名称截断而不挤压关闭按钮；打开时焦点落在标题栏右侧关闭按钮，名称保持普通显示，用户主动点击后再编辑；必填校验失败仍定位名称输入。左标题使用标准 Lucide `square-user-round` 素材组图标，右文件名按图片 / 视频 / 音频使用 `image / square-play / audio-lines`，两处统一 `18px`、`1.8px` 线宽，使用本地官方 SVG 路径。底部操作区高 `48px`，取消 / 保存按钮高 `32px`，不额外绘制上分割线，素材滚动区延伸至操作区上缘。
- 点击单素材预览区打开模态预览：图片显示大图，视频和音频提供原生播放控件。点击素材组图区只进入组内浏览，不打开编辑器或自动使用全部内容；用户选择单份或多份素材后再使用。批量添加时展开为独立素材节点，不创建整体素材组节点；这不代表正式 Node 级引用及持久化验收已经完成。
- 素材组编辑器通过 X、取消、Escape 或成功保存返回时，保留实际面板完成约 `200ms` 退场：左栏原位淡出，与底下同位置恢复的素材组库衔接，资产库不重复从左侧滑入；右预览在 `160ms` 内向右 `12px` 淡出，外壳背景同步消退，不缩放媒体。`≤720px` 上下排列时，预览改为向下 `8px`；减少动态效果直接切换。未保存的草稿先确认再退场，选择继续编辑保留当前画面；确认 / 退场期间阻止重复关闭和编辑，重新打开或销毁会取消旧动画并使旧回调失效。退场结束才释放画布交互，焦点返回库中的原素材组入口，找不到时回到资产库入口。
- 素材组编辑器返回后，空间切换仍使用常驻标签按钮；没有悬停展开菜单，也不会因静止光标意外切换空间。
- 素材预览可把 Media 添加到当前视口中心，或在由生成节点发起时引用到该节点；素材卡也可以直接拖到画布或生成节点。
- 素材组使用详情的延迟打开和回焦均绑定当前项目、画布与库空间；关闭、切换或销毁后，旧回调不会重新打开详情。
- 素材组悬停预览优先出现在来源卡片或列表行右侧，两者的水平中线按面板实际渲染高度对齐；靠近视口顶部、底部时将预览移回距边缘至少 `12px` 的可见范围，继续避让 Agent 与来源条目。网格和列表共用定位规则，窗口尺寸变化时重算；屏幕高度不足以容纳内容时，面板内部纵向滚动，描述与“添加到画布”仍可访问。滚动资产列表关闭未固定的预览，重新悬停时使用条目滚动后的实际位置。
...
- 单份 Media 继续支持拖入画布或参考区；组卡不拖出。多选同时含组与 Media 时拒绝拖拽并提示先展开组选择素材，或使用批量添加，不擅自将组打散。组内可选择单份或多份 Media 再使用。


持久化边界：

- 实验分支的个人 / 组织 Media、保存目录、自定义标签及 scoped placement 元数据来自服务端目录，可在 PostgreSQL 重启后恢复；个人根目录 Entity 保持既有持久化。内存体验版使用同样的保存规则，刷新重置。素材组目录、旧整理 / 共享 / 审核原型和只读平台 seed 不代表已上线的公共目录服务。
...
- 目录示例只保留一条五级路径：`默认目录 / 角色参考与视觉设计 / 第二幕角色与装备 / 精修与材质参考 / 最终选用的武器方案`。叶级放置“玄翎武器设定”，复用“玄翎_04_武器装备设定.png”，标签为“物品、Others”；其余样本保持原目录，素材组引用和源名称不变。免注册体验每次重置恢复该示例；账号种子默认不创建，只有显式使用 `seedDemoAssetLibrary(..., { withDirectoryExample: true })` 才建立 / 复用目录并放置未被用户修改过的样本。重跑不重复建树，也不覆盖已有的改名、标签或目录调整。
...
- Entity 已有个人根目录 create / get / list / update repository；本实验新增 `0014_media_library_catalog.sql`，为个人 / 组织 Media 保存提供持久目录、自定义标签和 placement 元数据。新迁移只在隔离本机验证环境应用，尚未应用到共享开发或公网。Entity 删除、持久素材组目录与共享、平台目录版本与导入策略、Node 级引用、恢复和 GenerationResult 显式晋升仍未实现。
...
- 共同包围框旁显示固定屏幕尺寸的上下文工具条，不额外显示已选节点数量。最左端是“布局”纯图标入口，图案为上方一个横矩形、下方两个小矩形；细分隔线之后依次为“新建素材组、打组、添加到对话、保存到素材、下载”。只要选择中包含已有持久组成员，就隐藏打组，其余入口保持各自可用性。下载使用一个包含箭头的单一按钮，点击后展开“批量下载 / 打包下载”菜单；后者在当前静态原型中仍以逐个文件下载模拟。
...
- 删除后不显示撤销浮层；节点删除、节点 / 组移动、组框缩放、打组 / 解组、局部排列与整画布整理、Alt 复制、生成参数修改、媒体重命名和素材组引用展开可通过 Ctrl+Z 撤销。项目重命名当前不进入画布级撤销历史；普通双击新建尚未统一登记创建撤销。
...
资产库的“素材组”是可复用媒体引用集合，和本节画布上的空间“打组”不同；从选区新建素材组不移动节点、不建立画布组或持续绑定。资产库内混排多选、批量解析与权限规则见第 7.6 节。
