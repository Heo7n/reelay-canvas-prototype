...
- 没有真实 AIGC API。
- 没有生产账号生命周期、完整云端素材库或积分账本。本地已有个人 Media 与 Entity 持久化；实验分支新增个人 / 组织 Media 的保存目录、标签和放置，以及个人 / 组织主体的标签与删除；主体在各空间根级虚拟专区集中浏览，普通目录不再混排主体。新增持久化切片尚未迁移共享或公网环境。主体组织共享、平台审核及积分仍保留原型边界。公网原型已通过 Vercel 与 Supabase 部署用于评审；私有 Supabase ObjectStore 配置与三主体 12 图迁移已完成，正式主域已通过资产 / 主体 HTTP 读取、鉴权、大小限制与三主体浏览器展示验收，不构成生产可用承诺。
...
- 用户、会话、唯一组织 Workspace、Project、项目成员关系、CanvasDocument、WorkspaceMediaAsset 元数据、个人 placement、ProjectAssetReference，以及个人 / 组织 Entity 的字段、有序 Media 引用、版本和实验分支的目录 placement 保存在 PostgreSQL；独立本地模式使用 filesystem ObjectStore，共享开发与 Vercel 入口使用私有 Supabase ObjectStore。最小链路已通过本地服务重启回读；公网已迁移至 `0013` 并完成私有桶与三主体 12 图写入，正式主域已验证三主体精确目录、12 图 HTTP 内容及个人主体页三张封面，内存 adapter 只用于快速契约测试和显式开发回退。
...
- 进入画布只注册可访问的素材、主体与保存目录，不为整个资产库预读原图或探测媒体尺寸。资产库关闭时不生成隐藏的媒体预览；打开后按当前列表展示，实际使用素材或编辑时仍保留必要的尺寸读取。
- 同时进入的相同默认 API 读取共用进行中的网络请求，避免开发态重复挂载多读一遍画布和目录；结果完成后不缓存，再次进入仍读取服务端。登录、退出、保存等写入会隔开前后的读取，不复用写入前的进行中请求。
- 进入画布先请求文档，文档成功返回（包括尚无文档的空画布）后才并行请求个人素材、主体、个人 / 组织 Media 保存目录和项目资产目录；文档失败、已离开的路由或失效重试不启动目录读取。文档与 iframe 就绪后即可恢复编辑，不等待目录；目录随后补齐，加载中或失败有明确反馈，依赖目录的操作在对应目录就绪后启用。晚到的目录不会重置节点、任务、撤销或保存状态；旧 iframe 保留原握手兼容路径。
...
- 主体编辑器右侧先显示该素材的小图，原图加载并完成解码后原位切换；原图失败保留小图。切换素材立即换成当前素材的预览，旧异步完成不能覆盖它。同素材的控件更新保留主预览及视频元素；主体专区卡片及悬停预览优先使用小图，悬停不预读原图。
...
- 此入口不实现多人实时协作、云端上传、账号注册或测试作品留存；实验分支的个人 / 组织主体专区、创建与删除使用同样的内存规则，刷新重置；平台主体专区和个人主体共享到组织仍未开放。
...
- 初始空状态：中心显示紧凑标题“开启创作之旅”，下方为现有光标图标与“双击画布创建节点，或拖拽上传素材”说明，再下方排列“图片/视频生成 / Agent一站式创作 / 影视灵感与拉片复刻”三个带图标的展示块。点击提示图标为 18px；展示块高约 40px，采用低对比底色与细边框，第三块稍宽以容纳长文案，窄屏前两块并排、第三块独占一行；光标、线性图标、底色与边框随深浅主题适配。整体仅为视觉引导、不提供点击入口、不拦截画布手势，该展示块尚未接入点击，平台空间已提供下述影视参考与预整理拉片分析；只读空画布显示“画布暂无内容”，隐藏图标、操作说明及展示块。
...
- 浅色画布沿用原来的纯净浅灰蓝 `#f1f3f6`、面板为 `#f8fafc`，不生成材质层，保留定位点阵。深色画布为炭灰 `#171819`、面板为 `#202123`，仅在画布底层铺低对比的静态交织细网纹。纤维明暗色取底色 RGB 各通道上下偏移 5、相同 0.7 透明度，采用固定 4px CSS 单元；纹理在屏幕坐标中保持密度，不随画布平移或缩放，不响应指针或进入可访问树。图片、视频、文字、输入框和管理面板不叠加纹理。Agent 输入区恢复与对应主题面板一致的底色，通过原有细边线区分区域，不单独提亮或压暗。
- 缩放感知网格：使用更细、低对比的定位点阵，近景可见，远景淡出，分别适配浅色和深色模式；点阵仍随世界坐标平移与缩放，与固定材质层独立。
...
当前产品定位为 Windows / macOS 的 PC Web 创作工作流，后续功能设计、视觉验收与回归测试以桌面视口为准，不主动开发或验收手机端、超小视口布局；已有窄屏降级样式可保留。
...
- 模拟生成任务记录项目、画布和节点归属以及启动时的参数快照；生成期间锁定该节点的生成输入，包括资产库选择、本地文件选择和拖放等外部素材入口，切换画布后仍按启动快照把结果写回发起任务的画布。生成只接受活动画布内的实际节点，同一节点不能重复启动；陈旧回调不能写入同 ID 的替代节点。删除节点（含撤销创建）、删除画布、替换文档或项目上下文，以及访问失效时会取消相应任务；删除撤销恢复空闲节点，不恢复原任务。取消沿用当前内存 mock 的扣费结果，不增加退款规则。
- 资产库展开时保留左上项目级导航、隐藏左侧中部胶囊；资产库头部提供空间切换与当前区域操作，不重复显示资产库标题，收起按钮固定在右缘，与左侧资产库入口保持同一水平线，不在管理素材时重复放置项目和内部画布菜单。关闭资产库后恢复左侧中部胶囊。
...
本节及第 7.6 节描述 `codex/experiment-20260910` 的资产库实验；公网最近发布记录仍见当前交接。个人空间以普通目录浏览单份 Media，以独立“主体”专区浏览描述角色、产品、物品或场景的有序参考集合。界面名称恢复为“主体”，内部继续复用 Media / Entity 记录及其引用关系，不复制文件、不新增另一套主体实体。

当前交互：

- 面板不单独显示“资产库”标题，保留无障碍名称。顶部为两行：34px 高的 `个人 / 组织 / 平台` 等宽空间导航，以底部线标识当前空间、无外围框；28px 高的目录 / 主体库导航与右侧四个图标操作。内容共用左右 14px 内边距，工具行与素材相隔 12px。目录导航随可用空间伸缩、最大 240px，不绑定上方空间导航列宽；目录菜单独立保持约两张素材卡宽。收起按钮仍固定面板右缘，与画布左侧资产库入口同高。空间切换支持方向键、Home / End 和可见键盘焦点。
- 个人空间将“当前目录 / 主体库”合并为无外围边框的紧凑分段导航，宽度约 7:3，主体库文字居中、最小 58px；缩短中间分隔线并保留上下留白。浅中性底色归组，轻量状态底色表示当前区域。主体库常驻右侧，不受目录深度、搜索、多选和只读状态影响；组织与平台暂不提供主体专区。专区不是目录记录，不计入五级目录限制。
- 右侧按“添加 / 搜索 / 筛选 / 多选”排列，常态四项等分 188px 操作区，统一 28px 高、16px 图标与 4px 间隔；常态无底色和外框。目录中的加号展开 176px 宽的紧凑菜单，依次为“新建文件夹 / 上传资产”；新建在当前目录下进入目录树行内命名，第五级或无权限时禁用，上传沿用当前空间与目录。菜单支持再次点击、外部点击或 Escape 关闭，Escape 归还加号焦点，方向键选择菜单项。主体库加号直接新建主体，成员中原位提供编辑。进入多选时，右侧四个图标在同一行替换为“操作 / 全选 / 取消”，三项均常显图标与文字，每项 60px 宽、左右 6px 内边距；操作在未选择素材时禁用。右侧常态四项与多选三项铺满同一 188px 宽度，左右边缘对齐，保持左侧目录位置稳定；已展开的搜索保留在左方，不覆盖三项操作，关闭搜索恢复来源浏览状态与可见入口焦点。
- 目录名称与装饰下拉箭头是同一个点击块：普通目录中点击开关目录菜单，主体列表或成员中点击只恢复来源素材目录，再点才展开菜单；没有子目录左侧的独立返回上一级按钮。下拉箭头在主体列表与成员中也保持显示和原有位置；所有层级通过目录菜单切换，长目录名称单行省略、悬停完整路径。目录树对应行的 `+` 同样支持在该目录下新建，复用同一命名与层级校验。进入主体保留来源目录、筛选和滚动，主体库入口可从成员返回列表；主体统一平铺，不按旧 folderId 隐藏。
- 个人 / 组织搜索默认收成右侧图标，仅点击展开。平台搜索条始终展开，与右侧筛选 / 多选并排；输入非空时显示清空按钮，清空或 Escape 只清除查询，不收起搜索条。输入区域从搜索入口向左约 160ms 展开，覆盖目录 / 主体库及添加；筛选与多选保持可用，覆盖控件退出键盘访问，输入框立即聚焦。背景浅中性、无常驻描边；减少动态效果时不播放展开动画。多选状态为批量操作保留空间。关闭按钮或非输入法 Escape 退出搜索，恢复开始搜索前的目录 / 主体成员、查询、筛选及滚动，焦点回到搜索入口；有内容时失焦不自动关闭。
- 搜索范围固定为当前个人 / 组织 / 平台空间，跨全部目录，包含目录名称及完整路径、素材、个人主体的名称 / 描述 / 标签与可访问成员名称。从主体成员启动也搜索整个空间，不局限成员。非空查询按“目录 / 素材 / 主体”分组，只展示有结果的分组；目录是紧凑导航行，素材与主体继续沿用各自图卡。类型筛选作用于素材，标签按素材或主体自身关联取交集；存在类型 / 标签筛选时不列目录。空输入保持原浏览内容。没有文字搜索时普通类型 / 标签筛选仍覆盖当前目录及子目录，主体区域筛选行为不变。
- 点击目录结果继续浏览，点击主体卡打开编辑器，关闭后回到原搜索结果；参考选择状态下点击主体进入成员浏览，查询和结果滚动独立保留，成员中的返回入口显示“搜索结果”。目录结果内继续进入子目录不丢失查询；明确切换区域退出该搜索会话。切空间时在新空间继续当前查询，同时分别保留原浏览位置。搜索生命周期跟随资产库，不因同一库内切换画布留下不可见查询；作用域变化清理搜索会话，删除标签也清理保存快照中的关联。
- 搜索结果只允许同类多选，选择一种后另一种的选择入口禁用；尚未选择且同时有素材与主体时，“全选”先提供全选素材 / 全选主体。批量操作按选中类型提供，主体不出现创建主体、移动或媒体审核。关闭搜索、打开结果和切空间清理选择；搜索与筛选更新清理原结果选择。右下统计在搜索时包含匹配目录，否则只计算素材或主体。
- 面板首选宽度 `550px`，右缘拖拽或键盘方向键在 `380–780px` 间调整；与 Agent 同开可临时压缩，Agent 关闭后恢复用户首选宽度。顶部固定，下方内容区独立滚动。平台隐藏目录，保留常显搜索条和操作行。关闭面板时使用 `inert` 和 `aria-hidden` 退出键盘与读屏访问，焦点返回资产库入口；现有极窄视口降级不代表移动端产品承诺。
- 网格卡片约 `164px`，单素材和主体分别在各自区域显示。独立方形圆角预览下方为单行居中名称，不用外壳将图文包成一块，不提供列表视图；主体封面后露出两层薄片，取消图内数量角标；名称右侧用四个实心圆角菱形标识主体，名称与标识作为一组居中。主图尺寸、名称基线和占位一致，叠片不挤动布局。长文件名保留真实扩展名，主体名按普通名称省略；悬停和选中只强调图块。调宽改变列数，不拉伸卡片。
- 点击主体卡打开原主体编辑器；参考选择状态下点击主体进入其可访问的有序成员，保留主体名和返回“主体”的入口；返回恢复主体列表的搜索、标签及滚动。专区再返回恢复原素材目录，形成两层独立浏览上下文。失效主体明确显示不可用，不自动扩展到全库。主体内不直接上传，需进入编辑器添加素材。
- 默认目录计为第 1 层，普通目录总计最多 5 层。文件夹优先，单份素材按加入当前目录的时间最近优先排列；主体专区按既有 placement 的加入时间排序，旧数据回退创建时间。改名、改标签和编辑主体内容不改变顺序。右下角统一显示“共 x 项”，只计当前结果中的素材或主体，不计文件夹和专区入口；底部分割线左右各留 14px，与上方横线对齐。
...
- 视频节点参数栏包含模型、综合参数和高级设置，仅 Seedance 系列显示提示词优化；生成方式并入综合参数菜单的第一项。图片节点没有生成方式，保留模型、综合参数与高级设置的紧凑工具栏结构。
- 融合显示的积分 + 生成按钮。
...
- 模型、工作流、比例、时长、质量、分辨率、音频、输出格式和高级设置开关的离散修改进入字段级撤销；一次修改连同必要的参数归一化只形成一条记录。撤销只恢复本次改动的内容字段，比例变化所需的位置调整随同恢复，不替换节点对象，也不恢复无关的提示词、素材元数据、组归属或任务状态。生成期间锁定输入；提示词优化只产生建议，不锁定输入、参考素材、参数或对应撤销。

### 7.3 提示词输入区

- 用户确认填入优化建议后，撤销只恢复填入前的提示词文档和对应编辑历史，不恢复整节点快照；节点移动、组归属和其他字段变化保持独立。建议处理完成本身不创建撤销或保存。聚焦正文时由编辑器处理文本 / 引用撤销，填入事务与画布记录协调；填入后再编辑时，旧填入撤销不会覆盖新草稿，也不阻塞更早的移动等记录。
...
- 展开工作区不会自动聚焦提示词。未聚焦正文显示普通指针，单击或通过键盘 Tab 聚焦后显示文本光标，保留光标定位、选区、换行、空格、方向键和文本自身的撤销。提示词优化期间正文仍可编辑，忙碌反馈仅显示在优化入口。
...
参数不再按媒体类型写死，而由每个模型的能力表动态提供。节点工作区底部保持一条内容驱动宽度的主工具栏：模型名和综合参数摘要按实际内容紧凑排列，模型后保留细分隔；入口默认透明，仅在悬停、键盘聚焦或激活时显示轻底色，剩余空间由右侧操作区前的弹性间隔吸收。模型和综合参数分别打开独立弹层；受支持的 Seedance 视频节点将提示词优化作为一级动作，完成后由用户查看并手动填入；高级设置是同一卡片的内联展开入口，不再形成“更多”二级菜单。
...
- 综合参数弹层集中显示模型实际支持的模式、比例、分辨率或画质、时长、音频和输出格式等能力项；收拢状态只汇总界面中有意义的模式和参数。可调时长统一使用细轨横向滑杆，右侧紧凑数值框实时显示秒数并支持直接输入；普通滑杆以模型声明的最低秒数起步；支持 Auto 的延长模式使用独立自动档，随后直接进入最低有效秒数，不产生无效秒数。新节点及切换视频模型后使用该模型声明的默认时长，拖拽、点击轨道、键盘操作或数值输入都不能越界，模型能力保持 `1s` 步长；滑条不再呈现额外刻度区。Seedance 2.5 三种模式始终保留同高的时长标题、滑杆和数值框，切换时不折叠这一区域。视频编辑的数值框只读显示「智能」，悬停说明跟随原视频，滑杆禁用；视频延长保留原有 10px 圆形滑点，仅增加轻描边和 2px 低对比外圈，悬停和键盘焦点增强轮廓，不增加常显提示文字、额外行或按钮；滑杆最左端为「智能」，向右一档进入 `4s`，随后逐秒至 `30s`；数值框同步显示智能或秒数，并支持输入。Auto 档保留可操作滑块、隐藏秒单位；视频编辑仍禁用滑杆并隐藏滑块。三种模式仅 tab 选中底色过渡，内部控件不做整体位移或高度动画，快速切换保持对齐。摘要仅显示具体秒数或“智能”，不附加“时长”前缀；编辑固定请求时长 `-1`，延长 Auto 的配置保存为 `auto`，请求参数映射为 `-1`。当前模拟延长 Auto 按模型默认 `10s` 估算费用与节点结果时长，不把估算写回用户选择，也不代表供应商实际输出时长；真实接入前须校准该通道的自动时长与计费规则。弹层从综合参数入口上方展开并覆盖提示词工作区，不再向下增加节点外部显示范围；左缘与综合参数入口对齐。弹层底部在最后一组控件与轮廓线之间保留独立留白，外轮廓与激活的参数入口之间保持稳定的 `8px` 屏幕间距，不让底色或阴影互相覆盖。弹层定位使用不受入场动画影响的布局尺寸；打开或切换参数只更新浮层，不重播整个提示词工作区或参数弹层的入场位移动画。Seedance 2.5 的全模态参考菜单按“模式 / 比例 / 分辨率 / 时长 / 音频 / 输出格式”组织，视频编辑和视频延长保留相同参数顺序，时长行分别呈现只读与可选状态，不由 CSS 写死能力值。
- Seedance 2.5 在全模态参考、视频编辑和视频延长之间切换时复用同一个综合参数弹层；比例与时长在固定行内更新，内部不做整体位移、淡入或高度动画，不重播弹层的入场动画。视频编辑与视频延长互切时同步更新时长行；数值或 Auto 选择穿过只读编辑模式后保留，切回不支持 Auto 的全模态参考时恢复模型默认秒数。快速连续切换直接呈现最新任务类型，选中底色从当前状态继续，不排队或闪回；系统减少动态效果时直接显示最终布局。
...
- 节点底栏右侧是高级设置和“积分 + 生成”，Seedance 视频模型在高级设置前显示提示词优化；优化结果保留、重新优化来源与填入规则统一见 [10.3](#103-输入模式与本地动作)，本节只定义入口布局。高级设置按钮只负责展开或收起卡片内区域。积分和发送保持一个复合按钮，与 Agent 对话框共用布局和图标样式：高 `32px`，左侧为 `14px` 积分图标和 `12px` 等宽数字，右侧为 `32px`、`9px` 圆角的发送块，二者间隔 `8px`；不绘制外围复合底座，只用发送块的主题对比色强调主操作。图标、数字与箭头沿同一水平中线排列，宽度随积分位数自然增长。两处使用同一细线向上箭头和主题变量，节点与 Agent 均显示本次预计消耗，个人菜单独立显示可用余额；禁用时保留积分可读性并弱化发送箭头。积分图标与工作台、账户和组织积分入口使用同一青蓝双锥资产。
...
生成节点输入面板左侧有相邻但语义独立的“添加主体”和“添加参考素材”入口；主体是素材的组织与选择范围，确认使用时展开为各项 Media，素材入口仍负责单个 Media。当前不在生成节点建立持续的 Entity 绑定，主体编辑只更新主体库投影，不回写已展开的节点输入。主体 placement 的持久删除与引用保护见第 7.6 节；源记录回收、恢复及完整生命周期的后续规划见 `product-expansion-plan.md` 第 4.2 节。

节点与右侧对话框的“添加参考素材”统一提供两个入口：

- 本地上传：打开本地文件选择器。
- 选择素材：从资产库添加。

两处菜单共用 176px 宽、36px 选项行高与 16px Lucide 图标，分别使用 `upload / folder-open`，统一留白、圆角及浅深主题状态。
...
- 可编辑且空闲时，参考小格显示抓手光标，拖动展示拖影和插入位置；素材列溢出时可在列内滚轮横向浏览、拖到边缘自动横滚，键盘聚焦离屏小格时只滚动素材列。松手才提交一次排序，取消 / Escape / 失焦 / 作用域变化不改内容；卡片聚焦后可用 `Alt + 左右方向键` 调序。普通点击定位和移除入口保留，空格 / 中键画布平移不被排序占用。只读或生成中可预览但不可排序；提示词优化期间仍可调整参考。
...
- “选择素材”打开与左侧入口共用的资产库，默认进入个人空间普通素材根目录并清除本次旧搜索 / 筛选 / 多选。画布顶部持续显示“从素材库选择参考”、目标类型（节点 / 对话）、实际参考总数及“退出”；个人 / 组织单份素材点击立即加入打开时的目标参考区，再点立即移除，支持连续选择多份，操作后保留面板。卡片显示勾选并轻微压暗封面，勾选直接来自目标已有参考，跨目录、搜索、空间切换及输入区移除后同步，不另维护临时选择集合；重复引用按源素材身份匹配，不重复添加。
- 参考选择期间保留目录、搜索、筛选及主体成员浏览；点击主体卡查看其成员以逐项选取，不进入编辑器。暂隐藏上传、目录管理、标签管理、卡片菜单及管理用多选，禁用平台灵感入口，避免混用管理与参考选取。由画布连线提供的参考显示已选但不能在库中移除，提示在画布管理连线。节点直接引用的增删使用既有撤销体系，取消引用只移除对应项，保留其他参考及其顺序。
- “退出”、Escape 或收起资产库只结束选择状态，保留已生效的引用；Escape 优先关闭当前子菜单。切换对话 / 画布、目标删除、开始生成或权限失效时结束旧目标选择，旧点击不写入新目标。退出后恢复普通浏览；普通资产库入口继续点击预览，拖拽和多选添加保留，主体与平台片段继续各自的编辑 / 分析入口。选择只更新参考，不改提示词、生成内容或积分。
...
- 本地文件和资产库素材拖入时，只突出鼠标所在且可编辑、未生成中的节点媒体区或输入区；画布空白可继续承接新建媒体，但不让全部节点一起高亮。与右侧对话输入区共用细边界和外侧柔和阴影，浅深主题分别适配，提示不改变控件尺寸；跨到对话输入区、移出、取消或所属画布 / 权限变化立即清理原落点反馈。
- 主体选择器绑定打开时的项目、画布与节点；切换所属画布 / 项目、目标节点失效、开始生成、失去写权限时关闭，确认前再次验证目标。空间切换、搜索、勾选与数据刷新在原面板内局部更新，保留遮罩、面板、控件及未变化的卡片与封面，不重复播放入场动画；跨空间保留搜索和已选主体，切到另一空间时仅将结果区回到顶部，重复点击当前空间不刷新或重置滚动。中文输入法组合及提交搜索始终保留原输入框、文本与光标，勾选时保留列表滚动和按钮焦点；取消或 Escape 关闭后只回焦同一作用域内仍存在的入口。
- 点击空白区域时，小菜单会收起。
...
左侧中部胶囊中的资产库是当前画布使用和整理可复用媒体的入口。Media 上传已接入 `WorkspaceMediaAsset + personal placement + ProjectAssetReference + ObjectStore`；个人 / 组织 Entity 已接入独立 repository、乐观版本和 iframe bridge，产品界面称为“主体”。实验分支的个人 / 组织 Media 目录、标签和 placement 元数据已持久化；个人与组织主体分别集中展示在各空间的根级虚拟专区，保留原有 Entity 数据和引用关系。组织主体共享、平台目录及旧共享 / 审核原型仍保留原边界，不等同于这些能力全部上线。

空间与内容：

- 组织主体由 Workspace 持有，成员可创建、读取和使用，owner/admin 可编辑、重命名、整理标签和删除。成员引用必须已有同一组织的 Media placement；已共享素材可复用，不要求改变原文件的容量归属，不隐式分享个人素材。组织主体内上传的新文件计入组织容量，创建者离开后主体与组织引用继续保留。删除主体保留素材，仍被存续组织主体引用的组织素材禁止删除；已展开到画布的素材不因主体编辑而回写。个人、组织的浏览位置和筛选独立保存，本轮不引入合集。

- `个人空间` 的持久目录和自定义标签只对本人可见；`组织空间` 的持久 Media 目录和自定义标签由当前 Workspace 成员共享，成员可直接保存素材，不隐式发起审核。组织成员身份由服务端独立校验，不用项目编辑权限替代；`平台空间` 定位为创作参考发现，目前为本地只读短片目录：12 个 5–30 秒连续视频节选，覆盖人物动作、自然空间、动画叙事、城市科幻与幻想场景，新增开放短片的来源与署名可从详情来源链接查看，来源标签保留原发布素材名，不代表已接入真实电影搜索索引。两列 16:9 卡显示标题、来源、时长、真实镜头数与轻量点分隔特征；常显搜索覆盖内容、来源、逐镜头描述和常用技法别名，多个空白分隔词取交集。搜索保持常显，内容与场景词通过标题、描述、镜头文案和别名检索，不在筛选中穷举。点击筛选按钮展开单页浮层，默认六行摘要：景别、运镜、光影、构图、剪辑、片段时长；每行默认“不限”，已选项用顿号连接并单行省略。一次展开一行，显示可搜索的选项列表；选项搜索覆盖名称与别名，保持输入 DOM 和中文输入法，不改变主搜索或已选条件。只展示全库有素材的值，常用项按全库覆盖数排序，时长按区间升序；最多约六行选项可见，余项滚动。选项右侧显示保留其它维度及主搜索时的匹配数，零结果仍可选择，不静默放宽条件；行内“不限”清除该维度，顶部重置清除所有筛选但保留主搜索。底部结果数与“完成”固定可见；即时生效，完成 / 外部点击 / Escape 收起，Escape 与完成返回入口焦点。浮层最大宽度 360px、受资产栏约束，不占列表布局；关闭后筛选按钮数量回显，切空间保留条件，切画布重置。分类内多选取并集、跨维度及主搜索取交集；已知明确景别从镜头 framing 提取，未标注不猜测；片段时长采用真实秒数，区间为 ≤10 秒、>10 且 ≤20 秒、>20 秒，不卡整数四舍五入。手动筛选标签只证明片段包含该特征，多个维度不保证落在同一镜头；从提示词进入的匹配另按单镜头证据排序及定位。卡片技法文字可点击，幂等追加该条件并保留其它条件及主搜索，不打开详情；多选时变为普通文字，避免误改条件。筛选变化清除批量选择，未实现自然语言语义搜索。不提供图片 / 音频类型、主体标签或未标记。悬浮 / 聚焦卡片 180ms 后只挂载一个静音循环视频，离开、滚动、多选、打开详情、收起侧栏或页面隐藏停止并释放资源；减少动态效果时只展示静态封面。平台不显示目录、上传、新建、主体入口或单条修改菜单；多选保留添加到画布，保存到素材在受控 import 落地前仍禁用。
- 片段卡打开原生模态详情，上方约 6:4 为完整比例播放器和来源 / 描述，下方提供秒级起止点、当前播放位置标记与播放选段。范围须在原片内且至少 0.1 秒，播放到终点暂停，不生成裁切文件。首次点击“拉片分析”在按钮内转圈并显示“分析中”，约 2.2 秒后在同一详情下方平滑展开结果；不伪造进度百分比，关闭、范围改变、媒体失败、切画布或撤销权限立即取消待完成任务，旧回调不能恢复结果。弹窗宽度 1160px、顶部距视口 24px，展开不改变顶部位置；短 PC 窗口仅滚动必要距离以露出结果首行。镜头网格与右侧“镜头分析 / 生成提示词”为 55:45，三列镜头和右侧阅读区共用 360–440px 高度，超出各自滚动；标签切换保留 DOM、提示词与滚动位置，外层尺寸不随内容变化，只做轻微淡入，减少动态效果时取消动画。标题关闭区和底部复制按钮固定。当前 12 段内容使用核对原视频的 58 个预整理镜头及真实关键帧，按真实切镜划分，长镜头不机械拆分，并以“模拟”轻量标记区分真实自动分析；点击镜头联动播放器和右侧分析，片段的局部交集标注“局部”，不将原镜头全部动作断言为所选范围内发生。整段与单镜头均提供直接描述画面、动作和运镜的生成提示词，各自编辑独立保留；切换镜头时分析、提示词与复制内容的范围一致，不使用对话式 AI 请求，也不展示未知声音等占位说明；更改范围保留旧结果但使其失效，恢复原范围可继续使用；重新分析覆盖自定义提示词前就地确认。
- 参考使用保持手动：“复制提示词”仅复制当前整段 / 单镜头的已编辑文本，要求当前范围分析有效、处理完成且非空；成功按钮就地显示“已复制”，编辑、改范围或切镜头后恢复，复制失败可重试，旧异步完成不污染新会话。“完整片段加入画布”添加整个目录片段，沿用可撤销媒体节点创建及 librarySourceId；选段只限制播放与分析，尚不生成裁切文件。移除平台详情到 Agent 草稿的自动连接，不改变已有会话、发送、生成或扣费。详情以同一检索词汇展示参考特征及镜头数；关闭释放视频并回焦原卡，同一画布会话内记住最近 12 个片段的范围、结果、提示词与播放位置；刷新不保留，切画布 / 权限撤销清理，只读可查看分析和复制但不能添加节点。实际 AI 分析、语义检索、后台自动推荐、项目参考集合、主体替换、裁切导出和持久平台目录尚未接入。
- 节点提示词及 Agent 生成模式输入区提供“找灵感”图标入口，非空文字才可用。点击打开现有资产库的平台空间，结束参考选择、清空旧平台查询和筛选，以当前纯文字建立一次匹配快照；不发送消息、不改提示词 / 参考素材、不生成或扣费。顶部紧凑依据栏显示不可点击的来源说明及文字快照，独立 × 用于退出匹配；编辑原文不自动刷新或重排，用户点击“更新匹配”才应用当前文字，空白草稿不可更新。退出匹配保留手动搜索与筛选并聚焦搜索；关闭资产库、离开平台、切换画布 / 对话、源节点删除或离开生成模式结束旧匹配，不把旧目标绑定到新草稿。
- 文字快照下方显示从当前规则可识别的匹配条件。点击标签文字在普通 / 重点参考之间切换，重点以圆点、边线及按下状态标识；标签旁 × 只移除该依据，不排除含此内容的素材。重点采用普通权重的三倍，影响片段排序和最佳镜头选择，命中理由优先展示重点，不成为硬性筛选。调整立即生效、清空批量选择并回到列表顶端，不改原提示词；“恢复默认”恢复当前快照条件及普通权重。全部移除时明确提示并按既有手动搜索 / 筛选展示，未识别条件则显示未识别状态，不假造标签或自动展示无关匹配。修改原提示词仍须“更新匹配”，保留新旧文本共有条件的侧重，新增条件默认普通，消失条件清理；重新点击找灵感开启默认侧重的新快照。长条件组限高滚动，删除后焦点移到下一条件或恢复入口。
- 提示词匹配当前为本地关键词与预整理镜头字段的原型：逐镜头比较运镜、光线、景别、场景、动作及构图，取每个片段最相关的一镜稳定排序，不跨镜头拼接证据，不展示未经校准的匹配百分比，也不分析用户附带的图片 / 视频。原平台文字搜索与多维筛选继续取交集，不为填满结果静默放宽；筛选选项计数与列表使用相同匹配范围，零结果显示调整描述或条件的空态。卡片保留完整片段与来源，封面换为命中镜头关键帧，并显示最多两项依据和开始时间；悬浮预览只循环该镜头。打开详情定位到该镜头且保持暂停，提供播放命中镜头入口；片段范围、拉片分析、已编辑提示词、手动复制与加入画布语义保持。匹配仅用于查找参考，不连接自动改写、主体替换或 Agent 编排。
- 普通素材目录存放图片、视频和音频，统一采用网格视图；主体专区只展示当前空间 Entity。普通素材及主体详情的筛选浮层提供横向 `全部 / 图片 / 视频 / 音频` 类型单选与当前空间的多选标签；主体专区仅保留标签，不显示媒体类型，所有区域均移除形式筛选。类型与标签取交集，多个标签按任一命中；“未标记”与具体标签互斥，不写入数据。重置只改草稿，应用后才更新列表并清除多选；取消、外部点击或 Esc 丢弃草稿，入口用条件数量标记生效状态。各空间的普通素材和主体专区各自保留浏览条件；目录始终作为导航，筛选内不提供新建标签。
- 单份素材与主体分区浏览，卡片均采用独立方形圆角预览和下方居中名称，不用外壳将图文包成一块。主体由去重、有序的 Media 引用组成，多个资料共同描述一个可复用对象；封面使用显式封面或首个可视 Media，背后保留两层薄片，名称右侧以四菱形标识区分主体，图内不显示成员数，不常显重复的“主体”说明。右下角“共 x 项”只计当前结果，不计普通文件夹和主体专区入口；底部分割线左右各留 14px。
- 主体专区是个人和组织空间各自的根级虚拟浏览区，与默认目录同级；常驻入口与目录入口并列，在任意目录层级都可进入。入口不能重命名、移动、删除或多选，不新建文件夹记录，也不占五级目录。专区平铺当前空间全部主体，忽略旧 placement 目录进行汇总；普通目录只显示文件夹和单 Media。创建主体不建立日期或同名目录，改名、增减引用不移动原文件；同一素材可被多个主体引用，仍留在原素材目录。主体内按有序引用跨目录展示成员，返回恢复专区状态；专区返回再恢复进入前的素材目录、查询和滚动。失效主体显示不可用，不静默切到全库。
- 同一 Media 可以通过 placement 出现在个人、组织或平台空间，空间共享不会克隆 Media 记录。普通 Media 的目录位置由对应空间的 placement 决定；主体专区不改写成员位置。旧的按来源和类型派生重叠“虚拟文件夹”已经移除。
- 顶部空间行和目录命令行采用第 6.1 节的两行布局；普通目录提供上传和添加目录，主体专区替换为“新建主体”且不显示添加目录。个人 / 组织搜索展开覆盖右侧命令，平台搜索常显并与工具并排。多选期间主操作原位切换为同宽的“操作”，两个工具原位切换为“全选 / 取消”，不新增行或挤动左侧控件；未选择时批量操作禁用，退出后恢复所在区域的主操作和筛选。

创建与整理：

- 普通素材目录的主操作是多文件上传，接受图片、视频和音频；主体专区通过新建 / 编辑主体添加所需参考。在受保护的 React 画布宿主中，主体编辑的本地上传按当前空间申请 library upload intent，写入 ObjectStore，并在对应空间默认目录建立 placement；组织上传计入组织容量，不同时建立个人条目。普通素材上传仍按其保存流程选择位置与标签。直接打开静态画布时才使用页面内存媒体。
...
- 主体专区的“新建主体”和普通素材多选操作中的同名入口进入独立编辑工作区，主体卡直接点击及菜单“编辑主体”也打开同一编辑器。新建标题固定“新建主体”，编辑标题跟随主体名称。编辑器采用左侧加宽面板，首选宽 760px，普通资产库的宽度偏好不变。主体编辑器及其素材选择面板直接共用资产库的面板底色、次级表面、媒体预览底色、边界和悬停变量；浅深主题保持同层级颜色一致，不另写独立色板。上半区左右约 38:62：左侧纵向排列名称、标签、描述，描述填满剩余高度；右侧类型筛选与名称输入框顶部对齐，下方固定两列、两行可见高度的素材区独立滚动，底部等宽的“选择素材 / 本地上传”按钮与描述框底边对齐。素材名称在图块下单独居中显示并保留文件后缀；下半区为跨两栏的当前素材大预览，底部取消 / 保存固定。编辑器提供成员 `全部 / 图片 / 视频 / 音频` 筛选、从当前个人或组织素材库添加、上传、单项预览和移除，仅图片可设封面。下方预览标题显示媒体类型与文件名，双击或 F2 可编辑主文件名并保留扩展名，只修改当前空间 placement 名称，不改源文件、画布或其他空间。预览块内右上角的“设为封面 / 当前封面”使用同尺寸文字按钮或状态，距上、右各 10px，左侧对应成员显示轻量封面标记。成员筛选不是全库筛选；主体至少引用一个 Media 才能保存。
- 受保护宿主中的新建与编辑分别调用当前空间 Entity 的幂等创建和带 `expectedVersion` 的更新。新上传内容先走现有 Media 持久化链路，再由 Entity 保存资产 ID；上传成功但主体保存失败时 Media 仍留在对应空间素材库，不伪装成跨资源原子事务。版本冲突失败关闭并保留编辑草稿供用户处理。
- 主体编辑与上传 / 保存到素材的标签栏目均为一个完整点击区：已选名称用顿号分隔显示为单行纯文字，不显示标签底色块或叉号；过长省略并通过悬停读取完整名称，清空后显示“选择标签”。取消选择统一在展开的列表中取消勾选，箭头随展开状态翻转。
- 主体编辑器的标签可留空，使用当前个人或组织标签字典中的角色 / 场景 / 物品及现有自定义标签，展开直接显示可勾选列表，不提供搜索；底部“新建标签”展开行内输入，创建成功自动勾选，失败就地重试；最多 50 个。标签只修改草稿，取消沿用放弃确认；主体内容与自己的 placement 标签在同一保存事务内提交，成员素材的标签、目录和源文件不变。编辑请求同时校验主体版本与打开时的标签集合，标签被其他窗口修改或删除时明确拒绝，不静默覆盖；保存响应回显当前 placement 标签。标签不继承成员标签，不自动创建目录；自定义标签可在此新增，删除仍由资产库“管理标签”入口处理；已单独确认创建的标签在取消主体草稿后保留。
...
- 从个人或组织普通目录及主体专区创建，目标为当前空间主体专区；画布多选创建仍明确进入个人主体专区；创建请求明确使用 `folderId: null`，编辑器不显示保存位置选择。主体无需依附成员目录，已有主体编辑不改变旧目录元数据。草稿打开不写主体、画布或撤销；名称必填，可编辑描述、可选标签、成员和封面。未编辑取消直接返回，已修改沿用放弃确认，成功后进入对应空间主体专区并定位新主体；回到素材浏览时恢复创建前的目录上下文。创建不移动、打组或替换选中节点，不建立持续绑定。提交和异步完成均校验项目 / 画布实例与权限，旧回调不污染新草稿；矮桌面窗口保留固定创建 / 取消区域。
- 默认目录计为第 1 层，总计最多 5 层，仅作为 Media 整理结构。主体不再提供单项或批量移动，也不在普通目录中显示；旧 Entity placement 的目录字段继续可读并保留，不通过显示时清洗或批量迁移改写历史记录。主体专区按既有 `addedAt` 排序，缺失时回退创建时间，改名、标签和内容编辑不重排。新建主体统一没有普通目录位置。
- 普通文件夹的既有整理和共享入口继续遵守能力限制，不将历史主体位置解释为共享主体的授权；未接入的动作不能通过页面内存修改冒充持久成功。
- 卡片三点菜单在浏览器顶层靠触发按钮展开并按视口避让；滚动跟随，触发器离开列表或关闭库时收起。单 Media 保留适用的重命名、移动、组织复制、删除和审核原型；个人主体卡提供编辑主体、设置标签、重命名和删除，不再重复提供“查看内容”；组织主体的编辑、标签整理、重命名及删除限 owner/admin，普通成员可查看与使用，不提供移动、审核或共享。Entity 编辑与重命名沿用版本检查；Media 保存位置仍通过“保存到素材”弹窗修改，标签可独立整理，未接入持久化的旧动作不冒充成功。
- 多选只作用于当前可见条目，以 `${kind}:${id}` 保存身份。悬停或键盘聚焦露出复选框，点击进入多选后常显；全选、取消和上下文清理作用于当前区域。普通素材多选保留“添加到画布”和“新建主体”等适用操作；主体专区仅提供使用所含素材、设置标签和删除，不提供新建主体、移动、审核或分享。使用所含素材沿既有有序引用解析并去重，有指定目标时沿用目标动作文案；删除只处理选中的主体，不隐式删除成员。两个区域不产生主体与 Media 的混合选择。
- 资产列表按空间和条目身份复用卡片；画布选择、空白点击与无关刷新不重建列表，勾选、菜单和名称更新只修改相应内容区。未变化的视频 / 图片预览保持挂载与加载状态，避免视频重新加载闪白；列表滚动、仍存在控件的焦点与进行中的行内改名保持。
- 多选期间单素材、主体及目录卡片不显示三点菜单，退出后恢复；批量操作统一从顶部进入。顶部多选 / 筛选与全选 / 取消共用 130px 宽操作区，切换不推动左侧目录和搜索；窄面板优先压缩目录名称，保留按钮完整文字。
- 选择中仅有单 Media 时，从已选素材卡片开始拖动，按列表顺序携带当前可见的整组选中素材，拖动提示显示素材数量；拖未选卡片只携带该项。落到画布空白处后围绕落点排列为独立节点，落到可编辑的空闲生成器后整批追加参考素材，一次拖放只产生一条创建或新增引用撤销，撤销保留原有节点、原有素材及无关修改。拖放校验项目、画布、空间与素材可见性，失效批次整体拒绝；只读画布不能接收。平台多选“添加到画布”复用同一批量路径。
- 可共享的单 Media 只新增 organization placement，不复制媒体；文件夹共享沿用既有类型及子树权限限制。本实验不通过文件夹路径开放主体共享，新的组织 Media placement 只通过已授权的保存命令写入。
- 单素材、个人 / 组织主体及普通文件夹支持持久删除。确认框说明实际范围，提交期间防重入，失败保留草稿并允许重试，成功后才更新列表。删除只移除当前空间条目，保留底层文件和已有画布内容；删除主体保留成员。普通目录递归删除子目录与其中的 Media placement，不删除历史上放在该目录的主体；同一事务仅将这些存续 Entity placement 的 `folderId` 解除为 null，保留标签、`addedAt`、内容版本和成员。若目录中任一素材仍被存续主体引用，整批拒绝且不解除目录关系；只有显式选择删除的主体才从引用保护中排除，主体版本过期同样拒绝。默认目录和主体专区不可删。个人仅操作本人条目，组织删除限所有者 / 管理员，平台不可删；服务端校验与幂等重试不复活已删除主体，旧上下文确认不能串写。当前没有回收站或恢复入口。
- 目录下拉树宽度约为两张素材卡片加中间间距（当前 342px），右缘与第二张素材卡片接近对齐，并受面板可用宽度约束。保留五级缩进、固定 32px 行高与选中勾号；名称始终单行，过长时省略，悬停可读取完整名称和路径。目录多时内部滚动，预留滚动条位置避免展开跳动。当前目录按钮悬停和辅助技术可读取完整路径。
- 个人与组织空间点击上传先打开“上传资产”空弹窗，复用保存弹窗的位置、标签和提交逻辑。左侧始终为三列可滚动宫格，第一格常驻“＋”本地文件入口，不显示下方文字；只有点击这格才打开本地文件选择器，支持分次追加与逐项移除，单份文件也保持宫格。素材右上角移除图标仅在悬停对应卡片或键盘聚焦按钮时显示，保存期间隐藏并禁用。右侧默认选中上传入口所在目录，可调整位置和标签，文件沿用原名称；取消与保存位于右栏底部，与左侧浏览区底边对齐。“上传资产”标题旁提供轻量的“格式与限制”入口，完整图片、视频、音频清单与单文件限制默认收起，不在右侧表单内常显；清单直接沿用上传校验的扩展名映射。悬停或键盘聚焦可查看，点击固定阅读；鼠标可移入浮层继续查看，点击外部、移走键盘焦点或再次点击入口收起，Esc 优先收起说明再处理上传弹窗。浮层不参与表单布局，不改变浏览区或保存按钮位置；浅深主题使用同一层级，类型图标略强于辅助文字，仅本地上传模式显示入口。上传 / 保存弹窗、资产库与主体编辑器素材卡、右侧文件名及媒体参考标签的长文件名只截断主名，保留最后的真实文件扩展名；目录名、主体名仍按普通名称省略，悬停与编辑保留完整原文。追加与移除不重置目录、标签或滚动，已有预览保持挂载；图片成功加载后移除占位图标，透明区域只透出预览底色，失败或无预览时保留类型占位。文件选择器取消保留弹窗和已选文件。每次追加先整批校验格式、数量与大小，累计最多 100 份，错误只在弹窗底部反馈；没有待保存素材时禁用保存。点击保存后才开始上传，期间锁定加删；取消整个弹窗仅释放本地预览。上传按新增语义处理，目标空间已存在的内容不改名称、位置或标签；缺失或已被删除的初始目录须重新选择，不静默保存到根目录。实际上传成功而后续放置失败时保留已入库媒体并可重试，与画布保存复用同一生命周期。

上传格式与容量（实验分支）：

- 资产库本地上传支持图片 `JPG / JPEG、PNG、WebP、SVG`，视频 `MP4、MOV、WebM`，音频 `MP3、WAV、M4A、FLAC、AAC、OGG`。格式提示、文件选择器、前端校验和服务端白名单由同一策略提供；旧媒体继续可读。音视频容器可入库不代表其所有编码均能在浏览器播放，也不代表所有生成模型均可直接接收。
- 库入口单文件最多 50 MiB（界面显示 MB），当前环境限制更低时取较低值；SVG 原文件最多 4 MiB，仅通过受保护的图片地址显示，不将 SVG 标记内联或交给服务器缩略图渲染器。已经持久保存的画布媒体新增库条目只建立引用，不再次套用库上传大小限制。画布入口保留环境实际上限：本机 API 64 MiB、云端直传最高 50 MiB；大视频分片上传及剪辑导出尚未实现。体验模式继续为单文件 4 MiB、临时合计 128 MiB，不展示持久账号容量。
- 个人项目画布文件和个人资产库共用个人容量；协作项目文件和组织资产库使用组织容量。服务端在创建上传时根据真实项目权限或显式空间确定存储归属，源文件归属创建后不随 placement 的共享、移动或增加而改变。个人跨 Workspace 合并统计；同一对象多个画布、目录或主体引用只计一次，独立上传的副本分别计量。默认个人、组织各 10 GiB（界面显示 GB），可按归属账户配置限额；额度下调不删除已有媒体，只阻止超额新增。
- 服务端串行核验同一归属下已用和预留容量，原文件计入已用，缩略图缓存不计；上传成功后幂等转为已用。取消先阻断完成，再确认原对象物理删除，最后释放预留；删除失败保留额度且可重试。远端签名上传及历史授权不明的上传不能仅凭超时释放，仍保留预留。移除素材卡或画布节点不等于原文件回收，当前没有源文件回收入口。
- 账号设置新增“存储空间”，显示个人及当前组织的真实已用 / 总量、可用和上传预留，可刷新；读取失败不伪造数值，接近用满或已经用满显示相应状态。普通上传通过标题旁的“格式与限制”按需展示清单；服务端拒绝超额上传时，错误区显示对应空间的剩余量。模型输入限制与存储限制独立，后续剪辑和回收边界见[扩展规划](product-expansion-plan.md#画布大文件剪辑与存储回收后续)。

画布保存交互：

- 单选与多选媒体工具栏的“保存到素材”打开同一弹窗，采用桌面左右双栏，底部为取消 / 保存。单选左侧大预览与文件名整体对齐右侧名称、保存位置与标签三项表单的上下边缘，预览高度随表单区域适配。多选左侧改为三列素材宫格，各格包含封面与原文件名，长名称单行省略并可悬停查看完整名称；宫格独立纵向滚动，右侧仅显示保存位置、标签，底部操作固定。图片按需加载，视频使用已有封面、音频使用类型图标，不为整批挂载播放器。图片 / 视频缩略预览等比铺满整个显示块，超出部分居中裁切，图块与下方文件名共用左右边缘。打开弹窗时不自动进入名称编辑，输入聚焦只改变原有单层边线，不添加外围焦点框；读取状态在位置控件内部显示，不能增删表单行，标签保持单行可横向浏览，错误放在既有底部操作区；键盘导航、校验失败定位和关闭后的焦点返回保持。单份可编辑入库名称；多选仅新增目标空间中尚未入库的素材，沿用各自名称，并将所选目录与标签应用到这些新条目；已有条目的名称、位置、标签保持原样，不自动建立主体。总数量在弹窗标题旁显示一次，保存按钮显示实际待新增数量，全部已入库时禁用并显示“已保存”。同一媒体按稳定 ID 去重，批量 placement 写入全成功或全失败。
- 保存位置下拉顶部使用与资产库一致的 `个人 / 组织` 下划线标签页，不显示选中底块，不提供平台目标。完整位置按实际可用宽度显示；不足时仅显示“个人（或组织） / … / 末级目录”三段，不再保留其他中间层；末级名称仍过长时在本段内截断。悬停或聚焦可读取完整路径，目录树仍显示完整层级。点击箭头只展开目录，点击目录名称选择位置。
- 保存位置和资产库面板的目录树使用一致交互：当前目录仅常显右侧勾号，不保留选中底色；悬停行高亮，右侧以 `+ / 重命名 / 删除` 三个图标覆盖勾号与名称尾部，鼠标离开恢复，不推移文本。默认仅预留勾号宽度，目录名单行截断并保留完整路径提示；无鼠标悬停行时，键盘聚焦行也能显示操作，同一时刻最多一行。默认目录仅提供 `+`，第五级禁用新增；个人目录可管理，组织普通成员可新建但重命名 / 删除仅所有者或管理员。`+` 在所在目录内行内新建，笔图标在原行编辑名称，Enter / Escape 和确认 / 取消支持中文输入法；确认前不写入目录记录，失败保留草稿，名称为 1–100 字且同级不可重名。创建成功自动选中新目录；重命名保持目录 ID、子目录、素材和当前保存位置，路径同步新名称，并发名称变化拒绝覆盖。删除复用持久删除与确认，递归处理子目录和 Media placement，历史主体只解除目录关系且保留内容，并原子检查所有存续主体的成员引用；在保存窗口显式删除所选目录或其祖先时，位置回到被删目录的父级，外部目录失效仍需重新选择。
- 标签为可选多选项，内置仅 `角色 / 场景 / 物品`；已选标签显示可移除的小标签，`+` 在菜单中展开行内输入、确认与取消。同名标签复用；个人自定义标签属于本人，组织自定义标签在当前组织共享。切换空间保留内置标签，清除原空间自定义选择，不隐式将个人标签发布到组织。移除音效内置项与 Others 预置项；已保存的旧 `builtin:sound` 由迁移转为所属空间的普通“音效”标签，保留既有标注，不按名称删除用户自定义标签。
- 单素材与主体的卡片菜单、多选操作均提供“设置标签”。单项直接回显当前空间 placement 已关联的标签，不显示添加 / 移除切换；勾选、取消及全部清空都在应用时一次性保存，未修改时禁用应用。保存校验打开时的标签集合，拒绝旧草稿覆盖并发修改，同结果重试保持幂等；不修改名称、目录或主体内容。多项继续使用添加标签 / 移除标签，只增删指定 ID并保留各项其余标签。提交前不修改数据，同一批次原子提交；失败保留草稿、成功后退出多选，避免筛选变化留下不可见选择。组织标签整理仅所有者 / 管理员可用，普通成员仍可在上传时选择或创建标签。新建标签在行内显式确认后独立保存，即使取消外层或切换库空间也同步原空间字典；跨账号 / 画布的旧结果不能污染新上下文。
- 主体与成员共用标签字典但分别标注，不自动上下继承。主体专区仅按主体自身标签筛选，进入主体清空外层条件以查看完整成员，再在成员范围使用类型 / 标签筛选；返回恢复各层条件与滚动。主体标签属于 personal Entity placement，由 `0017_entity_library_tags.sql` 与目录 API 持久保存，内容编辑与版本变化不覆盖标签。新建主体初始无标签，可随后独立设置，编辑器不隐式进行两阶段标签保存。已知角色案例主体显式标为角色，12 份固定案例素材按明确内容标记为 8 份角色、4 份物品；仅初始化写入，重跑不覆盖用户编辑，普通上传和未知旧主体不自动分类。
- 同一媒体在同一空间只保留一个 placement。单选已入库素材时位置字段回显现有目录，下方不放置常显说明或移动勾选块；修改目录后点击保存，才通过既有确认弹窗明确“移动并保存”，返回保留草稿。多选保存不承担整理或批量编辑。新增操作在服务端事务内判断目标条目是否已存在，重复或并发新增不覆盖已有位置、名称、标签或更新时间；显式移动时素材已被其他操作移走则提示刷新。保存到另一空间保留原空间条目，并复用底层媒体；名称、位置、标签都属于目标 placement，不改画布节点、源文件或其他空间的元数据。
- 自定义标签可从筛选的“管理标签”入口或“设置标签”中的同名入口删除；管理在独立弹窗内进行，筛选内不提供新建或直接删除。内置角色 / 场景 / 物品保留，个人仅管理本人标签，组织仅 owner/admin 可删。删除前在管理面板上方的独立小弹窗显示关联素材 / 组数量并确认，管理列表始终保留；取消或 Esc 只关闭确认并返回原标签行，成功后只更新列表并将焦点移到相邻标签，滚动位置保留。服务端按同空间原子删除字典项及全部 placement 关联，保留素材、组、文件与目录。关联数量变化时拒绝并刷新确认，缺失标签重复删除幂等，并发保存不能恢复已删除 ID。删除后清理生效 / 草稿 / 返回上下文的失效标签及多选；嵌套的标签编辑草稿保留其它选择。
- 画布上传已经建立个人根目录 placement；这类素材再次保存会复用原记录。尚未持久化的受支持同源媒体，在确认后先经既有上传链入库，再完成目标放置；上传与批量放置是两个阶段，放置失败不会删除已经上传的素材，重试复用已确认的资产。未知跨域或无法读取的模拟结果会明确报错，不显示虚假成功。
- 位置、标签和名称的未提交输入属于弹窗草稿；已经单独确认创建的文件夹或自定义标签，在取消整个保存弹窗后仍保留。保存失败保留草稿并允许重试，成功反馈以实际持久化结果为准。打开、关闭、异步完成和提交均绑定当前项目 / 画布实例及权限，旧回调不串写新画布，不把资产库数据放入 CanvasDocument 或画布撤销记录。

预览与画布使用：

- 编辑器素材区固定保留两行高度，空状态、少量素材和切换分类不改变描述区、添加按钮与预览区位置；点击预览、设置封面或编辑标签保留素材区滚动，切换分类回到顶部。短桌面窗口降低控件与缩略图高度，为下方预览和固定操作保留空间；不隐藏预览，也不新增手机布局。退场使用整块面板淡出，取消旧左右面板分离动画。
- 主体编辑器下方的大图 / 视频预览按剩余可用区域等比完整适配并居中，竖图、横图和窗口缩小时都不裁切；预览轨道由容器尺寸约束，不能被媒体的固有尺寸撑大。资产与编辑器的小卡片仍使用统一比例的封面缩略图，大图预览和放入画布使用完整原图。
- 主体编辑器顶部标题栏将主体图标与名称放在左侧，关闭按钮放在右端，长名称截断而不挤压关闭按钮；打开时焦点落在标题栏右侧关闭按钮，名称保持普通显示，用户主动点击后再编辑；必填校验失败仍定位名称输入。标题下不绘制分割线，表单顶部留白收为 4px；编辑器标题与主体卡共用 Lucide `layout-grid` 旋转 45° 的实心四角菱形标识，预览文件名按图片 / 视频 / 音频使用 `image / square-play / audio-lines`，标题标识为 `16px`，预览类型图标为 `18px`，与文件名垂直居中；使用本地官方 SVG 路径。底部操作区高 `48px`，取消 / 保存按钮高 `32px`，不额外绘制上分割线。
- 点击单素材预览区打开模态预览：图片显示大图，视频和音频提供原生播放控件。点击主体图区只进入组内浏览，不打开编辑器或自动使用全部内容；用户选择单份或多份素材后再使用。批量添加时展开为独立素材节点，不创建整体主体节点；这不代表正式 Node 级引用及持久化验收已经完成。
- 主体编辑器通过 X、取消、Escape 或成功保存返回时，整块面板在约 `160ms` 内向左 `8px` 淡出，与底下同位置恢复的主体库衔接；资产库不重复滑入，不缩放媒体。减少动态效果直接切换。未保存的草稿先确认再退场，选择继续编辑保留当前画面；确认 / 退场期间阻止重复关闭和编辑，重新打开或销毁会取消旧动画并使旧回调失效。退场结束才释放画布交互，焦点返回库中的原主体入口，找不到时回到资产库入口。
- 主体编辑器返回后，空间切换仍使用常驻标签按钮；没有悬停展开菜单，也不会因静止光标意外切换空间。
- 素材预览可把 Media 添加到当前视口中心，或在由生成节点发起时引用到该节点；素材卡也可以直接拖到画布或生成节点。
- 主体使用详情的延迟打开和回焦均绑定当前项目、画布与库空间；关闭、切换或销毁后，旧回调不会重新打开详情。
- 主体卡封面悬停约 130ms 或键盘聚焦时显示使用预览，固定在资产库面板右缘外 10px，并与卡片垂直中心对齐；宽度 240–340px，按视口和对话栏间的可用空间收窄，不足 240px 时不显示，不覆盖库内卡片。浮层只展示封面、名称、描述和素材数量，不接收鼠标操作；离开封面立即关闭，切换卡片先隐藏旧内容。“添加到画布”移到卡片封面底部，悬停或键盘聚焦卡片时显示，添加复用去重与一次撤销；管理多选、参考选择、重命名、菜单打开或只读画布时不显示。主体保留工具栏多选入口，普通浏览和悬停不显示左上角勾选框；进入多选后显示每张卡片的勾选框，点击勾选框或封面均可选择 / 取消，选中时压暗封面并描边，退出后隐藏勾选框并恢复点击编辑。卡片点击及菜单“编辑主体”进入现有编辑工作区；菜单、名称重命名和管理多选不触发悬停预览。参考选择状态不显示此浮层，点击主体进入当前面板的成员列表逐项勾选。返回 / 名称行下方仅在有真实描述时显示描述，保留换行、最长 72px 内部滚动，不增加说明标签；下方按引用顺序显示可用素材，用户可选择本次需要的参考。返回主体列表保留原搜索、标签与滚动。
- 双击卡片名称区域可快速重命名；个人 / 组织持久 Entity 的快速重命名同样经过版本检查，不直接篡改页面投影。更多菜单提供与当前空间一致的单项操作，平台卡片没有修改菜单。
...
- 单份 Media 继续支持拖入画布或参考区；主体卡不拖出。主体专区可通过批量使用所含素材添加独立节点，或进入主体后选择单份 / 多份 Media 再拖放或使用；跨区域切换清理原多选，不产生混合拖拽。


持久化边界：

- 实验分支的个人 / 组织 Media、普通目录、自定义标签及 scoped placement 元数据来自服务端目录，可在 PostgreSQL 重启后恢复；个人 / 组织 Entity、独立标签、排序时间及历史目录字段继续持久化。主体专区是对当前空间 Entity 的集中投影，没有新增持久文件夹或独立实体。内存体验版采用同样的创建、删除与引用保护规则，刷新重置。组织主体专区已开放；平台主体专区与个人主体复制 / 共享到组织尚未开放，未接入的整理 / 审核原型和平台 seed 不代表公共目录服务。
...
- 目录示例只保留一条五级路径：`默认目录 / 角色参考与视觉设计 / 第二幕角色与装备 / 精修与材质参考 / 最终选用的武器方案`。叶级放置“玄翎武器设定”，复用“玄翎_04_武器装备设定.png”，标签为“物品”；其余样本保持原目录，主体引用和源名称不变。免注册体验每次重置恢复该示例；账号种子默认不创建，只有显式使用 `seedDemoAssetLibrary(..., { withDirectoryExample: true })` 才建立 / 复用目录并放置未被用户修改过的样本。重跑不重复建树，也不覆盖已有的改名、标签或目录调整。
...
- Entity 已有个人 / 组织 create / get / list / update repository 和 placement 删除，主体专区继续复用原有稳定 ID、版本及引用。`0014_media_library_catalog.sql` 提供个人 / 组织 Media 目录，`0017_entity_library_tags.sql` 提供主体独立标签，`0018_entity_library_directories.sql` 保留历史目录、加入时间与原创建目标；底层已有目录接口继续兼容，但当前 UI 不提供主体移动或创建位置选择。`0019_organization_entities.sql` 增加组织主体归属、空间隔离的创建幂等和删除墓碑，以及组织 Media placement 引用约束。隔离本机预览已应用至 `0019`，共享开发与公网未应用；未批量改写案例数据。组织主体共享、平台目录版本与导入策略、Node 级引用、恢复和 GenerationResult 显式晋升仍未实现。
...
- 下载始终位于工具栏右侧，与其他常用操作一起显示图标和名称。
- 默认显示“图标＋文字名称”，更多工具仍以省略号图标表达；按钮置于 `12px` 圆角矩形底板内，按钮圆角 `8px`，主题表面以细边框和短距浅阴影区分画布。常驻图标使用 Lucide 标准矢量路径，统一方形画框与描边，保留 HD 字母、裁剪边角、下载托盘和更多圆点的完整轮廓。编辑工具与“更多工具”为一组，固定显示的“加入资产库”和下载为另一组，中间以细分隔线区分。
- 工具栏在 `20%–200%` 画布缩放下使用精确反向补偿，保持约 `44px` 的屏幕高度、`36px` 按钮、`18px / 1.85` 描边图标和 `12px / 600` 的标签文字；缩放补偿即时更新，不使用 transform 缓动或单独锁定合成层，避免连续缩放时临时变大变小。工具栏始终锚定媒体框上方；平移、滚轮滑动画布或移动节点时与节点同步移动，超出视口便自然裁切，不吸附视口顶部，也不重新推回可见区域。节点名称与规格仍随节点等比缩放，提示词框沿用独立尺寸规则。
- 工具栏只在单选并点击有内容的媒体框时出现；点击画布空白、其他面板或其他非媒体区域时收起。媒体、多选、分组及左下视口工具条共用表面和悬停、按下、展开、禁用与键盘焦点反馈；视口条保留 `38px / 32px` 紧凑密度。单选与多选复用同一按钮、标签、分隔线与菜单规则，统一 `3px` 内边距、`2px` 按钮间距，不保留单选的额外小屏缩小规则；工具名称默认常显，不随媒体宽度或画布缩放自动隐藏；用户仍可在自定义工具栏中主动切换为纯图标。分组工具条空白不触发组拖动。
...
- “自定义工具栏”支持固定或取消固定工具，并可选择是否显示工具名称；预览复用实际工具栏的分组与样式。设置保存在浏览器本地，已有的名称显示偏好仍然保留；没有设置时默认显示图标与名称。
...
- “添加到对话”把所选节点中的可用图片 / 视频 / 音频加入当前对话参考区并展开右侧栏；没有可用媒体时入口禁用，空生成节点不加入。沿用当前模式与草稿，不触发发送或扣费，不修改画布、连线或撤销；已有参考去重，提示词优化进行中仍可添加参考。
...
- 端口采用透明底的“圆形轮廓 + 加号”形态，使端口下方的连线仍然可见；其颜色接近选中边线但更弱，节点 hover 时不会抢过媒体内容。静止位置贴近媒体框左右边缘，节点被选中、指针进入媒体框或扩大的左右边缘交互区时才显现。普通端口沿用原有 34px 世界直径、距媒体框边缘 38px、2px 描边和 20 × 2.4px 加号，与媒体框同比例缩放；不得为优化吸附而修改可见端口尺寸或补偿为固定屏幕大小。起拖区围绕静止圆心建立椭圆，水平半径 34px、垂直半径 40px 随节点缩放，仅不可见命中容差保留 22px 屏幕半径下限，区域不进入媒体框内部。靠近核心区时端口直接跟随指针，外围吸引逐渐减弱到零；活动跟随不叠加位置缓动，可见圆环保持在命中区内。极低缩放时连线会弱化，生成节点从创建起同时提供左右端口。
- 有媒体内容的素材节点或生成节点可作为上游；生成节点左右边中心各有一个只位于媒体框外部的端口交互场，不覆盖媒体框主体。十字光标、悬停与起拖命中共用围绕端口圆心的起拖几何，重叠区域按最近的静止端口中心选择，可见邻居媒体主体优先响应自身手势。拖线目标继续使用独立的外侧半椭圆：外层 148 × 108px、吸附层 104 × 78px 随节点缩放，并保留远景容差；吸附层必须覆盖随缩放变化的可见端口。端口吸附后回到稳定中心，退出范围再增加 12px 屏幕容差形成滞后。连线进行中，合法目标节点的整个媒体框主体同时作为完整连接接收面：进入主体后预览线临时投影到语义正确的一侧边缘，画布上层会按媒体框的屏幕矩形放置独立的约一像素边缘光层，不参与画布世界缩放。目标进入捕获状态时，连接侧先出现一次短促局部亮边，随后由更窄的亮头带着柔和尾迹沿圆角边缘向另一侧扫过；光窗宽度和节奏按媒体框当前屏幕宽度自适应，并根据实际窗口宽度计算完全位于框外的起止位置。未经过区域保持透明，完整穿越后保留短暂暗场，因此不会只在左右端口闪烁，也不会暴露完整矩形轨迹。动画只在目标激活期间运行，进入新目标或重新进入时从连接侧重启。深色主题使用高亮中性白边线，浅色主题使用高对比冷灰边线；两者都不通过加粗边框、框外光晕、压暗媒体内容、径向光斑、旋转光环或倾转节点表达。退出或松手后光层快速消隐，系统减少动态效果时只保留细静态边线。按下开始拉线后，起点端口立即隐藏，预览线直接从媒体框左右边中心出发；端口只承担发现和目标吸附反馈，落定后的实际连线回归媒体框边缘中心。右侧拖出建立下游，左侧拖出建立上游；主体重叠时优先命中视觉层级更高的合法节点，外侧相邻候选重叠时按稳定中心距离切换。拖入已有节点的有效外侧吸附场或媒体框主体都会建立连接；拖到画布空白处松开后，预览线冻结为一次性的待创建反馈，端口保持隐藏。线末端按菜单实际渲染尺寸和画布容器滚动偏移直接锚定到竖排创建菜单的上边或下边，并轻微压入边缘以避免缩放、字体渲染或内部滚动产生视觉缝隙和错位。该菜单与双击创建菜单复用图片、视频两种卡片及相同图标；确认、取消或点击外部后才提交或清除该线。
- 多选共同框的整体输出端口复用同一连接状态机，采用实心中性圆面、清晰描边与加号。可见圆面在 `20%–200%` 缩放下限制为 `32–40px` 屏幕尺寸，点击区至少 `44px`，中心距选择框边 `28–44px`，命中区与框边至少相隔 `6px`；多选时隐藏的节点端口同时停用起拖区。深浅主题分别提供常态、悬停、按下、焦点和连接态反馈，连线锚点仍是共同边界右侧中心，不因显示尺寸增强而偏移。起拖后端口跟随指针，所有选中节点的右侧连接线同时汇聚到落点；吸附已有生成节点时批量建立全部合法出边，落到空白处时多条预览线保持显示并打开共同下游创建菜单。选择图片或视频后只创建一个生成节点，并以一次批量连接提交把所有来源连入该节点。
...
资产库的“主体”是可复用媒体引用集合，和本节画布上的空间“打组”不同；从选区新建主体不移动节点、不建立画布组或持续绑定。资产库的素材 / 主体分区、批量解析与权限规则见第 7.6 节。
...
默认收起为右上角独立的 44px 圆形 Agent 徽标，沿用原品牌图形和画布中性色；它是该角唯一常驻控件，不与分享、个人入口或积分组成同一操作组。徽标不支持拖动，点击后展开右侧完整对话栏。圆形入口与展开后的收起按钮共用 52px 标题栏的水平中心线；默认顶部 4px、右侧 12px，面板顶部内缩时随其位置同步，不分别硬编码两种状态的纵向位置。

交互：

- 待机时仅内部 Logo 活动，44px 圆形底座和点击范围保持稳定。空闲 1.5–2.5 秒后先双眨眼并轻晃身体，此后单眨眼每隔 6–8 秒、身体轻晃每隔 12–16 秒各自独立随机排程。双眨眼共 620ms、单眨眼 240ms；身体轻晃 1.25 秒，最大旋转 6°、位移 2px。独立眼睑遮合原图的透明眼睛，不压缩身体或重画品牌轮廓。悬停 / 键盘聚焦显示 `Reelay Agent`，Logo 轻抬 2px、倾斜 5°并放大至 1.06 倍；鼠标进入时取消旧待机动作，立即双眨眼回应一次，停留后保持安静，移出再排待机。按下时轻压，点击展开侧栏，圆形入口隐藏。面板展开、页面隐藏 / 离开、键盘可见焦点进入或销毁时，停止 timer 并取消未完成动作；恢复可用后重新排程，不补播。减少动态效果或浏览器不支持动画时保持静态。浅深主题的图形与眼睑一起反色，不改变生成任务状态。
- 展开后的收起入口位于标题栏内部最左侧，在会话名称前方，组合 24px 品牌 Logo 与右侧官方 Lucide `chevrons-right` 双箭头，组内间隔 4px。点击区为 56 × 32px，与右侧新建 / 多选共用 32px 高度、8px 圆角及 16px 通用图标尺度。常态无底色、无描边，箭头使用次要文字色；悬停显示浅底并提示“收起对话”，键盘聚焦有清晰轮廓。Logo 复用上面的眼睑与独立计时：仅在面板展开时活动，身体晃动幅度为圆形入口的 70%；悬停轻抬 1px、倾斜 3°并放大至 1.03 倍，立即双眨眼回应一次。双箭头与点击区保持稳定；两处 Logo 只让当前可见入口活动，暂停 / 恢复、焦点及减少动态效果规则一致。收起与会话标题组成左侧分组，右侧依次为新建对话、多选生成记录；长标题省略，窄窗保持同一结构。整个标题栏随面板一起收放，收起后原右上角 Logo 入口恢复并接收焦点；面板整体管理隐藏与键盘访问，收起保留会话、草稿和生成任务。减少动态效果时即时切换。
...
- 顶部左侧为收起按钮及当前对话名，对话名与下拉箭头组成历史会话按钮，菜单锚定标题下方；右侧依次为“新建对话”和“多选”，使用官方 Lucide“对话框 + 加号”与勾选框。空生成会话禁用多选，普通 Agent 模式不显示生成记录多选。收起按钮留在标题栏内，动效与命中规则见 10.1；关闭后回焦展开入口。标题按钮同步展开状态，支持键盘打开与 Escape 回焦。
...
- 底部使用一张完整圆角卡片承载输入和生成配置，不再拆成普通文本框与零散图标。输入主体左上放置添加入口与参考素材架；仅生成模式在右上显示高级设置入口，并按当前 Seedance 模型能力显示提示词优化，Agent 模式隐藏两项并释放参考区空间，底栏连续展示模式、当前模式的模型、参数摘要、积分提示与发送动作。输入卡片沿用所在 Agent 侧栏的表面色，并以边框而不是另一块异色底面区分边界；默认桌面宽度完整展示模式、模型和参数，模型品牌图标始终保留，窄面板先省略参数工作流前缀，再把模式和模型折叠为仍有完整可访问名称的图标，最窄时才省略参数次要尾段，不通过横向滚动或半截省略号破坏输入区。
...
- 综合参数摘要是可点击按钮，复用节点的参数模板、模型能力和归一规则，提供比例、分辨率 / 质量、时长滑杆与数值输入、音频、工作流 / 任务类型及输出格式等当前模型支持的选项；修改实时更新摘要。Seedance 2.5 编辑 / 延长时同样锁定 Auto 比例，时长分别显示只读智能与支持智能最左档的 4–30s 滑杆，沿用节点的归一与模拟费用规则；生成模式下，空白输入框与节点共用当前模型 / 模式的灰色占位引导，切换参数只更新占位文字，不修改用户正文；切回其他模型或 Agent 对话模式恢复通用占位。弹层从参数按钮上方打开，在面板边缘平移避让，内容过高时内部滚动。各模型参数只保存在当前页面的独立本地配置中，切回该模型可继续使用；不修改画布节点、撤销栈或 CanvasDocument，Agent 模式下禁用手动参数。参数按钮按内容宽度显示悬停 / 展开底色，剩余空间由独立弹性留白承接，发送按钮仍靠右。
- Agent 模式保留可编辑的独立多模型偏好池，至少保留一项，允许选择图片和视频模型；不宣称已经接入自动拆解、生成编排或后台托管服务。
- 生成与 Agent 两模式均可添加图片、视频和音频参考。左上加号与生成节点共用“本地上传 / 选择素材”两项菜单；选择素材打开资产库并进入持续参考选择，勾选即时加入打开时的对话参考区，再点移除，退出保留结果，不落到画布。画布选中素材仍可通过其工具栏独立的“添加到对话”动作加入。支持将本地文件或资产库中选中的素材批量拖入输入卡片，加入对话不会修改画布或扣积分；媒体拖放由输入卡片先于富文本编辑器接收，只加入参考区，不把拖放载荷中的素材 ID 或文件名插入 Prompt。普通文字拖放仍由编辑器处理。本地文件单个不超过 64 MiB，本页尚持有的本地文件合计不超过 128 MiB，超限或不支持的文件给出明确反馈。
...
- 提示词和素材草稿按会话身份隔离，切模式保留，切会话恢复各自草稿；有草稿的“新对话”不作为新建时可复用的空会话。旧文件选择回调与拖放不能跨会话加入；优化期间仍可增删 / 排序素材，已有建议保留启动快照，填入前校验引用仍有效。
- 提示词优化当前仅支持 Seedance 2.5、Seedance 2.0、Seedance 2.0 Fast。节点及对话框生成模式按当前模型显示入口，图片模型、其他视频模型不显示且不能发起优化；Agent 模式隐藏优化与高级设置。对话框手动填入只改变该会话的 Prompt 草稿，作为一次可撤销的编辑器事务，不写 CanvasDocument / 画布撤销栈、不扣积分，也不触发生成。只读画布仍允许本地会话草稿编辑与优化，生成发送继续核验写权限。切到不支持的模型关闭建议面板但保留已有结果，切回受支持模型仍可查看；原生成能力不变。
- 发起优化时冻结本次 Prompt、参考、模型参数与配置，默认约 `1800ms` 本地模拟处理；入口旋转但不锁定原输入。没有已有结果且输入为空时不可发起，已有结果的查看规则如下。
- 节点和对话框输入区的提示词优化 / 高级设置分别使用官方 Lucide `wand-sparkles` / `sliders-horizontal`，共用 16px 图标与 1.8px 描边、32px 点击区域；侧栏不单独放大工具图标，保持居中对齐。
- 优化建议使用最大 1160 × 920px、受视口宽高约束的大面板，桌面左侧约 40% 展示“优化前（原文）”、右侧约 60% 展示可编辑的“优化后”；窄屏切换前后页签。引用保留稳定素材 key 和分类编号，支持 hover 预览；不因优化把引用变成普通文字。面板不展示版本、改动摘要或模拟标签，提供复制、重新优化与填入输入框；操作图标通过共享入口使用官方 Lucide 包。
- 节点与对话框本轮草稿在同作用域有成功建议时，入口优先打开上次面板；原文、模型参数或参考改变后仍可查看，填入后也保留结果。对话框以实际会话对象加独立草稿轮次隔离：修改输入 / 素材 / 模型、切会话、关闭面板和填入均不换轮次；只有成功受理发送才进入新轮次，校验失败不清除原建议。新轮次不继承旧结果，旧完成回调和撤销不能影响新草稿。
- 填入后当前输入与上次写入快照相同、建议也未再编辑时，主操作显示禁用的“已填入”，不误报内容陈旧；编辑建议后恢复填入能力。若用户改变原输入、模型参数或参考，主操作改为“优化当前内容”，隐藏重复的“重新优化”，不覆盖新草稿。重新优化在原输入未变或仍是本次填入结果时使用原始来源和当前配置，避免递归扩写；其他变化明确采集当前输入。所有再优化入口统一保护尚未填入的手改建议，已填入过的同一份建议不重复确认；确认期间输入、建议或配置变化，旧确认失效。
- 优化完成 Toast 使用“提示词已优化，可再次点击提示词优化按钮查看。”，左侧柔和绿色成功图标、右侧“查看”直接打开结果；共享 Lucide 图标、轻入场动效与浅深主题适配，减少动态效果时关闭动画。平台默认 i 说明按内容自然宽度展示，常规宽度一行，窄屏空间不足时换行。
- 当前模型与上次成功结果的模型 ID 不同时，面板显示轻量“上次优化使用：模型名”，同模型参数变化不显示。对照栏标题为 16px / 600，正文 14px，以标题下细线和留白区分；原文区轻底色、优化后区保持编辑焦点，窄屏延续页签与滚动。对话任务受理后取消或失败不恢复旧优化轮次，不覆盖下一份草稿；通过记录“重新编辑”回填内容后，按当前草稿规则优化。
- 重试处理或失败时保留上次成功的“原文 + 建议”配对，直到新请求成功才一起替换，不能用新原文配旧建议。当前输入为空时仍能查看既有结果，但禁用处理；当前 Prompt 中仍有失效引用、目标失效或权限不允许时拒绝处理 / 填入。填入后的 toast 支持撤销，后续新编辑和已成功发送后的新草稿轮次不会被旧撤销覆盖。
- 优化面板头部使用轻量方案下拉，只列当前模型的“平台默认”和自定义配置，不再在对照面板中展开指令表单。“平台默认”、i 与下拉箭头组成统一视觉控件；i 保留独立 hover / focus / 点击说明“「平台默认」会随当前生成模型自动切换对应优化规则。”，不展示或允许编辑实际平台指令。自定义行在 hover / focus 时显示编辑入口，底部“+ 自定义优化配置”打开独立的小面板，适配模型仅列上述三个 Seedance 模型。
- 自定义面板包含名称、目标模型、优化指令（最多 2000 字符）；新建指令初始为空，编辑现有方案时模型锁定。保存为当前模型方案后选用，另一个模型的新方案仅保存到目标模型，不切换当前生成模型或目标模型已选方案。编辑原方案按同一 ID 原子更新，校验失败不部分写入；取消不改变保存内容。方案选择与修改仅影响下一次优化，不改已有建议或进行中请求冻结的指令。
- 配置按模型 ID 管理，不按全模态参考 / 视频编辑 / 视频延长等生成模式拆分；节点和对话草稿共用本机模型配置。平台规则来自 `data/model-catalog.js` 的 `optimizationInstructions`，仅用于执行，不进入配置展示。旧 v1 全局指令和 v2 默认项草稿迁移为可见的“自定义草稿”，平台默认不保留隐藏覆盖。当前为本机存储，无账号云端同步。
- 当前优化是确定性的本地模拟，不声称真实模型理解、识别素材或调用后端。建议按实际节点对象 / 会话草稿轮次及项目上下文隔离，不持久化为项目数据；开发说明见 `prompt-optimization-development.md`。
...
- 删除收起历史菜单并打开通用确认框，标题为“删除对话？”，正文展示具体对话名称并说明消息会被删除且无法撤销，确认按钮为“删除对话”。长名称完整换行。默认焦点在取消，Esc / 遮罩点击取消，关闭后焦点回到历史入口。确认期间背景不可操作。删除非当前项保持现有会话，删除当前项切至相邻记录，删完最后一项建立空白对话。切换当前会话关闭其结果面板，建议按实际会话身份隔离，不会填入新会话；删除会话后原建议不可再应用。
- 标题栏右侧的新建入口：复用未改名的空白会话，或建立具有唯一 ID 的新会话，不固定跳转到已可能删除的示例 ID。
- 有生成记录的会话不作为空白会话复用；仍有排队 / 生成中任务时阻止删除整个会话，并提示等待完成或先取消，不能通过删除会话绕过任务取消窗口。
- 菜单位于标题栏按钮下方 6px，左边缘对齐整条生成消息的底色块外边缘；右边缘与标题按钮对齐，标题按钮最大 288px、菜单最大 352px，随侧栏空间一起收窄；短标题也保留这一区域，避免菜单宽度随名称跳动。菜单只展示会话列表，不提供搜索或重复的新建操作。列表每行 38px、行间距 4px，最多同时展示 7 条（列表高 290px），更多内容在内部滚动；短面板按实际剩余高度进一步收敛，不带动画布。鼠标点击展开时焦点留在标题入口，当前会话保持勾选，只有真实悬停行后才显露操作；键盘 Enter / Space 打开时焦点进入当前会话。两种打开方式均只滚动历史列表以露出当前会话，不带动外层画布；Escape 关闭并回焦标题入口。演示夹具共 9 条，覆盖单字、长中文、中英文混排、英文长词串和标点名称。
...
- **滚动与悬浮阅读。** 消息列表收到普通滚轮或实际滚动时，取消待打开的悬浮预览并关闭未固定浮层；滚动边界引发的指针经过不视为阅读意图。最后一次滚动后静默至少 160ms，再发生真实鼠标位移才恢复悬停，随后仍遵循素材入口 80ms、Prompt / 详情 / 行内媒体 300ms 的等待。停止滚动本身不会自动弹出。点击及键盘打开不受此限制，固定阅读面板保留既有可视区与失效锚点规则。完整 Prompt 内部滚动保留父阅读面板与滚动位置，同时取消或关闭从属媒体预览，子层同样等待新的鼠标阅读意图；素材横条的滚轮翻页及固定行为保持。切会话、关闭侧栏取消旧悬停意图，销毁时释放监听器与计时器。
...
- **再次操作。** “重新编辑 / 再次生成 / 定位结果 / 三点”在媒体下方靠左排列；底部动作以记录底色混入正文色 6% 形成中性按钮底面，悬停 / 展开为 10%、按下为 14%，文字沿用正文色，不使用蓝色强调；成功记录始终显示定位入口，有画布结果时可用；尚未交付时置灰，预置案例提示“演示记录暂无画布节点”，其他未交付结果提示“生成结果尚未放入画布”，三点向其右侧横向展开“删除此记录”，原按钮位置保持。删除收起时不可聚焦；外部点击、焦点离开或 Escape 收起，Escape 回焦三点。“再次生成”使用原冻结输入创建新 TaskId 与新记录，重新按该次快照费用扣除，不覆盖旧结果。“重新编辑”恢复原提示词、参考 ID / 顺序、模型和参数；输入区已有正文或参考草稿时提示先保留或清空，不直接覆盖。进行中允许重新编辑，但只恢复原冻结输入，不取消或修改原任务；仍不提供再次生成或删除，终态记录删除不退成功费用、不删除已加入画布的节点。任务详情从参数右方直接打开，生成中也可查看；浮层锚定详情入口并受视口约束，展示带年份的发送 / 完成（取消）时间、耗时、费用与可复制 TaskId，任务完成时同步更新。复制使用标准线性图标，“反馈”与详情复制共用动作，复制成功后才显示“TaskId 已复制”。
...
- **批量删除。** 右上角“多选”进入当前会话的临时选择状态，关闭记录浮层并暂停媒体；整条记录可勾选，原记录操作暂不可交互。列表顶部固定显示全选、已选数量、删除与完成；全选支持部分选中，只包含成功 / 失败 / 已取消记录，排队与生成中的记录不可勾选。删除前通用确认框展示条数并说明不可恢复、画布结果保留；取消保留选择，确认后再校验任务身份、项目及会话归属，仅删除已选终态记录，不改草稿、画布节点、积分或任务结算。完成、再次点击多选或在选择区按 Escape 退出；收起侧栏、切会话 / 模式及成功发送也清空选择，不跨会话继承。空会话禁用多选，删除完所选记录后退出选择并显示轻提示。该功能不扩展普通 Agent 角色消息。
...
- 节点任务：`canvas-node-task-runner.js` 负责生成的运行记录、timer、取消和完成判定，不由根 `state` 保存任务 Map。任务记录项目 / 画布 / 节点 / task 归属、启动时实际节点身份及不可变输入；`app.js` 保留模拟扣费、结果字段写入、撤销与保存适配。提示词建议由 `prompt-optimization-service.js` 管理，独立 controller / view 承接设置、对照、编辑与明确应用；它不作为会修改画布的节点任务。右侧生成模式使用 10.7 的独立任务 service / executor，不把节点 runner 当作会话任务存储。
...
- 资产库 UI：`librarySpace`、`libraryZone`、`libraryFolderId`、`librarySearch`、类型 / 标签筛选、主体详情和多选 / 菜单 / 重命名 / 预览状态，以及各空间与主体专区独立的浏览上下文。普通目录与主体列表、主体列表与详情分别保留返回位置；导航由 `canvas-library-navigation.js` 管理。个人 / 组织 Media 与 Entity、个人与组织普通目录及 placement 由宿主 repository 投影进页面 store，平台仍为只读原型内容。这些 UI 状态和资产实体都不写入 CanvasDocument。
- 主题偏好归 `canvas-theme-controller.js`，根 state 不再另存 `themeMode`。
...
- React 壳已通过包管理器和构建流程使用 Lucide；legacy 画布经 `src/icons/index.js` 按需导入官方 `lucide` 包，开发与正式构建共用入口，移除手工路径表及通用 SVG sprite，不依赖境外 CDN 或完整图标包。
...
- 资产库已落地 WorkspaceMediaAsset、personal placement、ProjectAssetReference、ObjectStore 与个人根目录 Entity 的最小切片，但画布消费仍会创建 legacy 媒体投影。公网私有对象存储配置、对象与目录迁移已完成，正式主域 HTTP 读取与三主体浏览器展示已验收；实验分支已落地 Folder / organization Media placement、个人 / 组织 Entity placement 删除及集中主体专区；历史目录接口保留兼容，但 UI 不再提供主体移动；Entity 恢复、Node 级引用、回收站和 GenerationResult 晋升尚未落地。
