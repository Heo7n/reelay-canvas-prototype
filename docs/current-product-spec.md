...
- 端口采用轻量的“圆圈 + 十字”形态，默认隐藏，仅在节点悬停、选中或正在拖线时显示；生成节点从创建起同时提供左右端口，画布缩放低于 45% 时隐藏端口和连线，避免远景噪声。
- 有媒体内容的素材节点或生成节点可作为上游；拖线靠近可接入节点时会吸附并强化目标端口，拖到已有生成节点会建立连接，拖到画布空白处会先选择创建图片或视频下游节点。
- 下游提示词面板显示的是上游素材的实时画布引用，不复制媒体文件；上游媒体替换、重命名或切换后引用会同步更新，点击引用可定位上游，移除引用等同于断开连接。
- 连接禁止自连、重复连接和环路；选中节点时只强化其相关上下游，其他连接降为低对比显示。连接可单独选中、删除并通过撤销恢复。
