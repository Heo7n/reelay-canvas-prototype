...
- 没有真实 AIGC API。
- 没有生产账号生命周期、完整云端素材库或积分账本。本地已有个人 Media 与 Entity 持久化；实验分支新增个人 / 组织 Media 的保存目录、标签和放置，以及个人主体的标签与删除；主体在个人根级虚拟专区集中浏览，普通目录不再混排主体。新增持久化切片尚未迁移共享或公网环境。主体组织共享、平台审核及积分仍保留原型边界。公网原型已通过 Vercel 与 Supabase 部署用于评审；私有 Supabase ObjectStore 配置与三主体 12 图迁移已完成，正式主域已通过资产 / 主体 HTTP 读取、鉴权、大小限制与三主体浏览器展示验收，不构成生产可用承诺。
...
- 主体编辑器右侧先显示该素材的小图，原图加载并完成解码后原位切换；原图失败保留小图。切换素材立即换成当前素材的预览，旧异步完成不能覆盖它。同素材的控件更新保留主预览及视频元素；主体专区卡片使用小图，不因悬停预读原图或打开详情。
...
- 此入口不实现多人实时协作、云端上传、账号注册或测试作品留存；实验分支的个人主体专区、创建与删除使用同样的内存规则，刷新重置，组织 / 平台主体专区与组织主体共享仍未开放。
...
- 点击目录结果继续浏览，点击主体卡打开编辑器，关闭后回到原搜索结果；通过主体菜单“查看素材”进入成员浏览，查询和结果滚动独立保留，成员中的返回入口显示“搜索结果”。目录结果内继续进入子目录不丢失查询；明确切换区域退出该搜索会话。切空间时在新空间继续当前查询，同时分别保留原浏览位置。搜索生命周期跟随资产库，不因同一库内切换画布留下不可见查询；作用域变化清理搜索会话，删除标签也清理保存快照中的关联。
...
- 点击主体卡打开原主体编辑器；菜单“查看素材”进入其可访问的有序成员，保留主体名和返回“主体”的入口；返回恢复主体列表的搜索、标签及滚动。专区再返回恢复原素材目录，形成两层独立浏览上下文。失效主体明确显示不可用，不自动扩展到全库。主体内不直接上传，需进入编辑器添加素材。
...
- 从资产库添加会打开与左侧中部入口共用的资产库，进入当前空间的普通素材默认目录；从预览弹窗确认后加入当前生成节点的素材列。
...
左侧中部胶囊中的资产库是当前画布使用和整理可复用媒体的入口。Media 上传已接入 `WorkspaceMediaAsset + personal placement + ProjectAssetReference + ObjectStore`；个人 Entity 已接入独立 repository、乐观版本和 iframe bridge，产品界面称为“主体”。实验分支的个人 / 组织 Media 目录、标签和 placement 元数据已持久化；个人主体集中展示在根级虚拟专区，保留原有 Entity 数据和引用关系。组织主体共享、平台目录及旧共享 / 审核原型仍保留原边界，不等同于这些能力全部上线。
...
- 普通素材目录存放图片、视频和音频，统一采用网格视图；主体专区只展示个人 Entity。普通素材及主体详情的筛选浮层提供横向 `全部 / 图片 / 视频 / 音频` 类型单选与当前空间的多选标签；主体专区仅保留标签，不显示媒体类型，所有区域均移除形式筛选。类型与标签取交集，多个标签按任一命中；“未标记”与具体标签互斥，不写入数据。重置只改草稿，应用后才更新列表并清除多选；取消、外部点击或 Esc 丢弃草稿，入口用条件数量标记生效状态。个人普通素材、主体专区及其他空间各自保留浏览条件；目录始终作为导航，筛选内不提供新建标签。
...
- 主体专区是个人空间的根级虚拟浏览区，与默认目录同级；目录菜单从任意层级均可进入，默认目录第一卡指向同一区域。入口不能重命名、移动、删除或多选，不新建文件夹记录，也不占五级目录。专区平铺全部个人主体，忽略旧 placement 目录进行汇总；普通目录只显示文件夹和单 Media。创建主体不建立日期或同名目录，改名、增减引用不移动原文件；同一素材可被多个主体引用，仍留在原素材目录。主体内按有序引用跨目录展示成员，返回恢复专区状态；专区返回再恢复进入前的素材目录、查询和滚动。失效主体显示不可用，不静默切到全库。
...
- 普通素材目录的主操作是多文件上传，接受图片、视频和音频；主体专区通过新建 / 编辑主体添加所需参考。在受保护的 React 画布宿主中，个人上传创建带 checksum 的 upload intent，写入 ObjectStore，完成 WorkspaceMediaAsset 登记并幂等挂到当前项目；上传完成先建立 personal root placement，所选其他目录或组织空间经下述保存流程处理。宿主内非个人空间直接上传仍明确拒绝，直接打开静态画布时才使用页面内存媒体。
...
- 主体专区的“新建主体”和普通素材多选操作中的同名入口进入独立编辑工作区，主体卡直接点击、菜单编辑及详情编辑按钮也打开同一编辑器。新建标题固定“新建主体”，编辑标题跟随主体名称。编辑器采用左侧加宽面板，首选宽 760px，普通资产库的宽度偏好不变。上半区左右约 38:62：左侧纵向排列名称、标签、描述，描述填满剩余高度；右侧类型筛选与名称输入框顶部对齐，下方固定两列、两行可见高度的素材区独立滚动，底部等宽的“选择素材 / 本地上传”按钮与描述框底边对齐。素材名称在图块下单独居中显示并保留文件后缀；下半区为跨两栏的当前素材大预览，底部取消 / 保存固定。编辑器提供成员 `全部 / 图片 / 视频 / 音频` 筛选、从个人素材库添加、上传、单项预览和移除，仅图片可设封面。下方预览标题显示媒体类型与文件名，双击或 F2 可编辑主文件名并保留扩展名，只修改个人 placement 名称，不改源文件、画布或其他空间。预览块内右上角的“设为封面 / 当前封面”使用同尺寸文字按钮或状态，距上、右各 10px，左侧对应成员显示轻量封面标记。成员筛选不是全库筛选；主体至少引用一个 Media 才能保存。
...
- 主体编辑器的标签可留空，使用个人标签字典中的角色 / 场景 / 物品及现有自定义标签，支持搜索、多选和移除；最多 50 个。标签只修改草稿，取消沿用放弃确认；主体内容与自己的 placement 标签在同一保存事务内提交，成员素材的标签、目录和源文件不变。编辑请求同时校验主体版本与打开时的标签集合，标签被其他窗口修改或删除时明确拒绝，不静默覆盖；保存响应回显当前 placement 标签。标签不继承成员标签，不自动创建目录；自定义标签的新增 / 删除仍由现有资产库标签入口管理。
...
- 从任意普通目录、主体专区或画布创建，目标均为个人主体专区；创建请求明确使用 `folderId: null`，编辑器不显示保存位置选择。主体无需依附成员目录，已有主体编辑不改变旧目录元数据。草稿打开不写主体、画布或撤销；名称必填，可编辑描述、可选标签、成员和封面。未编辑取消直接返回，已修改沿用放弃确认，成功后进入个人主体专区并定位新主体；回到素材浏览时恢复创建前的目录上下文。创建不移动、打组或替换选中节点，不建立持续绑定。提交和异步完成均校验项目 / 画布实例与权限，旧回调不污染新草稿；矮桌面窗口保留固定创建 / 取消区域。
...
- 卡片三点菜单在浏览器顶层靠触发按钮展开并按视口避让；滚动跟随，触发器离开列表或关闭库时收起。单 Media 保留适用的重命名、移动、组织复制、删除和审核原型；个人主体卡提供查看内容、编辑主体、设置标签、重命名和删除，不提供移动、审核或共享。Entity 编辑与重命名沿用版本检查；Media 保存位置仍通过“保存到素材”弹窗修改，标签可独立整理，未接入持久化的旧动作不冒充成功。
...
- 单素材与个人主体的卡片菜单、多选操作均提供“设置标签”，弹窗分为添加标签 / 移除标签；批量只增删指定 ID，保留其他标签、名称、目录和组内容。提交前不修改数据，同一批次原子提交；失败保留草稿、成功后退出多选，避免筛选变化留下不可见选择。组织 Media 标签整理仅所有者 / 管理员可用，普通成员仍可在上传时选择或创建标签。新建标签在行内显式确认后独立保存，即使取消外层或切换库空间也同步原空间字典；跨账号 / 画布的旧结果不能污染新上下文。
...
- 主体专区卡片不通过悬停自动打开详情浮层；点击打开原主体编辑工作区。需要按成员选用时，从卡片菜单“查看素材”进入当前面板的主体内容。返回 / 名称行下方仅在有真实描述时显示描述，保留换行、最长 72px 内部滚动，不增加说明标签；下方按引用顺序显示可用素材，用户可选择本次需要的参考。返回主体列表保留原搜索、标签与滚动。
...
- 实验分支的个人 / 组织 Media、普通目录、自定义标签及 scoped placement 元数据来自服务端目录，可在 PostgreSQL 重启后恢复；个人 Entity、独立标签、排序时间及历史目录字段继续持久化。主体专区是对个人 Entity 的集中投影，没有新增持久文件夹或独立实体。内存体验版采用同样的创建、删除与引用保护规则，刷新重置。组织 / 平台主体专区及组织主体共享尚未开放，未接入的整理 / 审核原型和平台 seed 不代表公共目录服务。
...
- Entity 已有个人 create / get / list / update repository 和 placement 删除，主体专区继续复用原有稳定 ID、版本及引用。`0014_media_library_catalog.sql` 提供个人 / 组织 Media 目录，`0017_entity_library_tags.sql` 提供主体独立标签，`0018_entity_library_directories.sql` 保留历史目录、加入时间与原创建目标；底层已有目录接口继续兼容，但当前 UI 不提供主体移动或创建位置选择。`0018` 仅应用隔离本机预览，尚未应用共享开发或公网；本次专区改版不新增迁移或批量改写历史记录。组织主体共享、平台目录版本与导入策略、Node 级引用、恢复和 GenerationResult 显式晋升仍未实现。
...
默认收起为右上角独立的 54px 圆形 Agent 徽标，沿用原品牌图形和画布中性色；它是该角唯一常驻控件，不与分享、个人入口或积分组成同一操作组。徽标不支持拖动，点击后展开右侧完整对话栏。

交互：

- 待机时仅内部 Logo 活动，54px 圆形底座和点击范围保持稳定。空闲 1.5–2.5 秒后先双眨眼并轻晃身体，此后单眨眼每隔 6–8 秒、身体轻晃每隔 12–16 秒各自独立随机排程。双眨眼共 620ms、单眨眼 240ms；身体轻晃 1.25 秒，最大旋转 6°、位移 2px。独立眼睑遮合原图的透明眼睛，不压缩身体或重画品牌轮廓。悬停 / 键盘聚焦显示 `Reelay Agent`，Logo 轻抬 2px、倾斜 5°并放大至 1.06 倍；鼠标进入时取消旧待机动作，立即双眨眼回应一次，停留后保持安静，移出再排待机。按下时轻压，点击展开侧栏，圆形入口隐藏。面板展开、页面隐藏 / 离开、键盘可见焦点进入或销毁时，停止 timer 并取消未完成动作；恢复可用后重新排程，不补播。减少动态效果或浏览器不支持动画时保持静态。浅深主题的图形与眼睑一起反色，不改变生成任务状态。
- 展开后的收起入口位于标题栏内部最左侧，在会话标题前方，组合 24px 品牌 Logo 与右侧官方 Lucide `chevrons-right` 双箭头，组内间隔 4px。点击区为 56 × 32px，与右侧多选 / 新建共用 32px 高度、8px 圆角及 16px 通用图标尺度。常态无底色、无描边，箭头使用次要文字色；悬停显示浅底并提示“收起对话”，键盘聚焦有清晰轮廓。Logo 复用上面的眼睑与独立计时：仅在面板展开时活动，身体晃动幅度为圆形入口的 70%；悬停轻抬 1px、倾斜 3°并放大至 1.03 倍，立即双眨眼回应一次。双箭头与点击区保持稳定；两处 Logo 只让当前可见入口活动，暂停 / 恢复、焦点及减少动态效果规则一致。标题与收起按钮组成左侧分组，长标题省略，窄窗保持同一结构。整个标题栏随面板一起收放，收起后原右上角 Logo 入口恢复并接收焦点；面板整体管理隐藏与键盘访问，收起保留会话、草稿和生成任务。减少动态效果时即时切换。
...
- 顶部左侧为收起按钮及当前对话名，对话名与下拉箭头组成历史会话按钮，菜单锚定标题下方；右侧依次为“多选”和“新建对话”，使用官方 Lucide 勾选框与“对话框 + 加号”。空生成会话禁用多选，普通 Agent 模式不显示生成记录多选。收起按钮留在标题栏内，动效与命中规则见 10.1；关闭后回焦展开入口。标题按钮同步展开状态，支持键盘打开与 Escape 回焦。
...
- 资产库 UI：`librarySpace`、`libraryZone`、`libraryFolderId`、`librarySearch`、类型 / 标签筛选、主体详情和多选 / 菜单 / 重命名 / 预览状态，以及各空间与主体专区独立的浏览上下文。普通目录与主体列表、主体列表与详情分别保留返回位置；导航由 `canvas-library-navigation.js` 管理。个人 Media / Entity、个人与组织普通目录及 placement 由宿主 repository 投影进页面 store，平台仍为只读原型内容。这些 UI 状态和资产实体都不写入 CanvasDocument。
