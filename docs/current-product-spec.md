...
- 组织中心使用 `/app/w/:workspaceId/organization`、`/organization/credits` 与 `/organization/usage` 三个可深链页面。组织管理把精简组织信息和成员管理放在同一页；成员列表来自服务端 Workspace Membership 投影，显示真实演示成员、登录标识与组织角色，不读取项目权限。主账户可看到角色调整、凭证重置、会话撤销和主账户转移的安全流程入口，但这些入口当前明确为前端演示，不会修改演示账号、会话或角色；系统不展示任何成员的现有密码。积分页使用明确标注的 `100000 / 33000 / 67000` 原型口径和详情抽屉，不冒充真实 `CreditLedger`；组织用量页只展示待接入的指标结构和真实空状态，不绘制虚构趋势。
