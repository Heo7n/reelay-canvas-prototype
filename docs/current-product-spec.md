...
- 优化建议使用最大 1160 × 920px、受视口宽高约束的大面板，桌面左侧约 40% 展示“优化前（原文）”、右侧约 60% 展示可编辑的“优化后”；窄屏切换前后页签。引用保留稳定素材 key 和分类编号，支持 hover 预览；不因优化把引用变成普通文字。面板不展示版本、改动摘要或模拟标签，提供复制、重新优化与填入输入框；操作图标使用本地 Lucide 官方 SVG sprite。
