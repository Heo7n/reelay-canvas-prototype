...
- 组织中心使用 `/app/w/:workspaceId/organization`、`/organization/credits` 与 `/organization/usage` 三个可深链地址，并由同一父级路由加载和共享组织上下文；在左侧切换组织管理、积分管理和用量看板时不重复请求会话、工作区与成员数据，直接访问或刷新任一地址时仍会正常加载。为减少重复上下文，这组三页只保留品牌头，不重复显示工作台账户入口。组织中心在普通笔记本上保持紧凑桌面排版，并在 `1680px`、`2200px` 以上的宽视口逐级扩展外层面板和内容区，避免大屏继续受笔记本宽度上限约束。
  ...
  - 组织用量页仅对主账户与管理员开放，使用明确标注且可整体替换的确定性前端演示数据。首行并列窄幅“组织概览”与宽幅“365 天活动”，下一整行承载“期间分析”，让稳定经营状态、年度分布和所选期间洞察形成清晰层级。概览只突出组织可用积分与按近 30 天趋势计算的预计可用；近 30 天日均和历史日均作为较弱的辅助基线，不显示额外同比或环比噪声，且四项均保持稳定口径，不随期间筛选跳变。期间分析的指标、趋势、类型环形构成、模型 / 成员 / 项目来源排名与导出共用同一个已应用统计范围。“消耗来源”默认显示模型维度，随后可切换成员与项目；类型构成使用固定语义色环形图并可下钻，来源维度使用横向排名，长列表收敛为 Top 5 + 其他且总额与周期摘要一致。任务级扣费、退款与成员流水统一回到积分管理下钻，不在用量页复制完整流水表。内部分配不计入组织消耗；在 `GenerationTask`、计费快照与不可变 `CreditLedger` 接入前，页面不得把演示数据描述为真实组织账本。生产形态下的看板数据应是同一不可变流水按查询时刻动态聚合出的统计快照，而非长期冻结的静态副本；余额与流水近实时更新，图表聚合可允许约 5 分钟延迟并标示更新时间。
  ...
  - `365 天活动` 独立于期间分析，可按 365 天窗口向前翻阅，并在每日热力图、52 周柱状趋势与周累计趋势之间切换；三种视图共用底部月份轴，悬停反馈靠近目标，仅呈现日期范围与积分，累计视图额外显示当周增量。演示账本包含工作日 / 周末差异、活动高峰、项目间歇、退款和近期增长，不使用匀速增长或今日与近 7 天相同的假数据。
- 组织中心左侧导航已经承担当前分区名称，组织管理、积分管理和用量看板内容区不再重复显示同名大标题，但保留可访问标题。`365 天活动` 的每日、每周与累计视图共用同一套 `53 周 × 7 天` 年度绘图区、周列中心、固定图高和月份轴：每日显示日单元格，每周在同一列宽内显示周消耗柱，累计以周为离散节点连接累计消耗并显示当周增量。首尾悬停提示不得越出图表；稳定概览独立放在用量页首部，不与年度活动模式绑定。
