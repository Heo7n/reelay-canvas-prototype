...
- 右缘的收起按钮与左侧资产库入口共用胶囊按钮尺寸和间距来保持同一水平线，固定位置不跟随光标；使用 `24 × 40px` 独立点击区和 `14px` 左向箭头，常态低对比，悬停或键盘聚焦按钮、右缘调宽热区时加强，提示“收起资产库”；拖动调宽期间不额外强化收起按钮；按钮优先于调宽热区命中，使用普通点击光标，其余边缘继续支持调宽。收起后按钮随面板完全移出，焦点返回左侧资产库入口。`≤480px` 全屏降级时将同一按钮移至头部右侧，避免被视口裁切。
...
- 主体选择器绑定打开时的项目、画布与节点；切换所属画布 / 项目、目标节点失效、开始生成或提示词优化、失去写权限时关闭，确认前再次验证目标。中文输入法组合期间保留原搜索输入框，提交搜索后保留文本与光标；取消或 Escape 关闭后只回焦同一作用域内仍存在的入口。
- 点击空白区域时，小菜单会收起。
...
- 删除收起历史菜单并打开居中的独立确认框：约 380px 宽、16px 淡红底警示图标，与删除按钮共用底色和文字色、“删除对话”标题和具体对话名称；取消使用浅灰底，删除使用淡红底与低饱和红字，按钮高 32px。长名称完整换行。默认焦点在取消，Esc / 遮罩点击取消，关闭后焦点回到历史入口。确认期间背景不可操作。删除非当前项保持现有会话，删除当前项切至相邻记录，删完最后一项建立空白对话。切换当前会话会取消原会话的本地提示词优化。
