...
- 两个固定 `.test` 演示账号由服务端校验并使用 HttpOnly Cookie 维持独立会话；这只验证登录、路由保护、个人空间隔离和同组织项目共享，不是正式账号系统。
- Workspace、Membership 和 Project 当前保存在服务端进程内，服务重启后重置。
...
- 未登录访问工作空间或项目深链时会进入登录页；登录后只恢复当前账号有权访问的 `/app/w/...` 目标，外部或越权返回地址会回退到个人空间。
...
- `login.html` 继续保留静态视觉参考，不创建会话；正式账号、密码哈希 / 重置、风控和成员管理不得在两个演示账号上扩写。
...
- `/app/w/:workspaceId/projects` 是常规全部项目路由，个人 / 协作项目标签分别导航到个人和组织工作空间，支持本地搜索。两个分类都固定以“新建项目”作为首张卡，并混合本地演示封面和无缩略图的轻量语义卡。
- 鼠标进入现有项目卡或键盘焦点进入卡片时，封面右上角显示三点菜单，包含打开、重命名、修改封面、转为协作项目、删除项目；信息区显示快捷编辑笔。新建与重命名会写入 server-memory repository，并可被同组织另一账号读取；修改封面、转换归属和删除仍只显示未接入提示。
- 个人空间和组织空间由当前服务端 Membership 驱动；两个演示账号共享组织项目、个人项目隔离。浅色 / 深色仍是设备级偏好。
...
- 旧画布 Logo 按钮仍保留原型项目菜单：回到静态主页、静态全部项目、创建新项目、删除项目。将其改接 React 宿主导航前，不能把它描述为正式工作台导航。
...
login.html
  保留账户密码页的静态视觉参考；不创建服务端会话，也不再承接新账户逻辑。

home.html
  保留登录后主页与项目库的静态视觉参考；不消费共享 API。
...
src/server
  定义 Fastify 本地共享 API、HttpOnly 演示会话与 server-memory collaboration repository。
...
- 数据没有持久化。
- 已建立首批 Session、Workspace、Membership、Project ports、HTTP adapters 和本地共享 API；当前使用服务端进程内 repository，仅验证两个独立会话读写同一组织项目，不是持久化或正式鉴权。
...
Reelay 当前是一个本地可运行的 AIGC 创作平台原型：两个演示账号可通过受保护的 React 工作台使用隔离个人空间与共享组织项目，再进入旧无限画布验证生成节点、媒体组织和 Agent 辅助；账号、项目与画布内容都尚未达到生产持久化标准。
