...
- 用量看板的“预计可用”以近期趋势作为主估算，并同时展示历史平均参考、近期与历史日均差异及数据充分度；估算保留零消耗日，不把少量活跃日误当成连续日均。消耗构成的类型、成员、项目和模型行可打开当前周期、当前维度的轻量明细抽屉，最多展示最近 20 条任务记录，并提供进入积分管理完整流水的明确入口；该抽屉不替代组织账本。
