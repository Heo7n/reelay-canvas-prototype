...
- 组织中心使用 `/app/w/:workspaceId/organization`、`/organization/credits` 与 `/organization/usage` 三个可深链地址，并由同一父级路由加载和共享组织上下文；在左侧切换组织管理、积分管理和用量看板时不重复请求会话、工作区与成员数据，直接访问或刷新任一地址时仍会正常加载。为减少重复上下文，这组三页只保留品牌头，不重复显示工作台账户入口。组织中心在普通笔记本上保持紧凑桌面排版，并在 `1680px`、`2200px` 以上的宽视口逐级扩展外层面板和内容区，避免大屏继续受笔记本宽度上限约束。
  ...
  - 积分页以单张“组织积分余额”总览展示当前尚未消耗的 `100000` 积分，并按“可分配余额 `67000`、所有成员账户余额 `33000`”的顺序展示比例与静态构成。余额数字和构成区只负责展示，不承担导航；页面标题右侧的明确“积分流水”入口默认打开入账页签，成员余额标题右侧的“分配明细”入口打开分配页签，成员行“明细”打开该成员筛选后的消耗页签。积分流水使用宽度随桌面视口在约 `960px` 至 `1360px` 间扩展的右侧抽屉，固定标题与“入账 / 分配 / 消耗”页签，表格内容独立滚动且不依赖横向滚动。入账页只以“累计入账积分”概览外部积分进入组织，并列出时间、类型、来源说明和入账积分；分配页按“可分配余额、本月发放、本月回收、成员账户余额”的顺序概览，流水展示成员额度发放 / 回收、有效期、操作人与备注；消耗页不重复余额概览，以“时间、成员、项目、任务 / 模型、结算状态、消耗积分”的组织任务流水解释扣费，不显示扣减后余额。分配与消耗支持按成员筛选。发放和回收当前只给出前端反馈、不改写余额；抽屉不重复显示演示标签，但其中记录仍是可整体替换的确定性前端数据，不代表已经接入真实 `CreditLedger`。
  - 组织用量页仅对主账户与管理员开放，使用明确标注且可整体替换的确定性前端演示数据；今日 / 本周 / 本月 / 累计 / 自定义范围控制本期消耗、媒体产出、消耗构成与导出结果，组织可用积分以及近 30 日 / 历史累计两种预计可用天数保持组织级稳定口径。最近 365 天活动体块可按年度窗口向前翻阅，并在日历分布、52 周趋势与选定年度累计趋势之间切换，悬停只呈现日期和积分；消耗构成可按类型、成员、项目、模型切换分析维度。任务级扣费、退款与成员流水统一回到积分管理下钻，不在用量页复制完整流水表。内部分配不计入组织消耗；在 `GenerationTask`、计费快照与不可变 `CreditLedger` 接入前，页面不得把演示数据描述为真实组织账本。
