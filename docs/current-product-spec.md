...
- 端口采用轻量的“圆圈 + 十字”形态，静止时隐藏；节点被选中、指针进入媒体框或左右边缘交互区、正在从该端口拖线或进入有效吸附范围时才显现。端口外圈和十字在中远景中按屏幕尺寸自适应保持可读，进入磁吸区时使用固定屏幕尺寸的十字光标补充远景操作反馈；极低缩放时才隐藏端口并弱化连线，避免远景噪声。生成节点从创建起同时提供左右端口。
