...
- 组织中心使用 `/app/w/:workspaceId/organization`、`/organization/credits` 与 `/organization/usage` 三个可深链地址，并由同一父级路由加载和共享组织上下文；在左侧切换组织管理、积分管理和用量看板时不重复请求会话、工作区与成员数据，直接访问或刷新任一地址时仍会正常加载。为减少重复上下文，这组三页只保留品牌头，不重复显示工作台账户入口。组织中心在普通笔记本上保持紧凑桌面排版，并在 `1680px`、`2200px` 以上的宽视口逐级扩展外层面板和内容区，避免大屏继续受笔记本宽度上限约束。
  ...
  - 组织用量页仅对主账户与管理员开放，使用可整体替换的确定性前端演示数据；页面不重复显示演示标签或更新时间，演示边界由产品说明保留。首行并列窄幅“概览”与宽幅“长期活动”，下一整行承载“期间分析”，让稳定经营状态、长期分布和所选期间洞察形成清晰层级。概览以组织可用积分与预计可用为两个核心结论，按“标题与可用积分 / 双列日均参考 / 整行预计可用”的纵向节奏组织；层叠图形仅作低对比度语义提示，不承担数据表达。预计可用的说明继续明确引用近 30 天日均消耗依据，不显示额外同比信息。概览口径保持稳定，不随期间筛选跳变。期间分析不再单独重复图片产出、视频产出和总消耗摘要，直接以左窄右宽方式呈现消耗构成与消耗走势，再使用整行模型 / 成员 / 项目来源排名；图表、来源排名与导出共用同一个已应用统计范围，导出入口位于时间范围左侧并保持次级层级。“消耗来源”默认显示模型维度，随后可切换成员与项目；消耗构成使用固定语义色环形图并可下钻，来源维度使用横向排名，长列表收敛为 Top 5 + 其他且总额与周期摘要一致。进入积分管理的入口位于消耗来源列表末尾，不与期间筛选和导出争抢顶部操作区。任务级扣费、退款与成员流水统一回到积分管理下钻，不在用量页复制完整流水表。内部分配不计入组织消耗；在 `GenerationTask`、计费快照与不可变 `CreditLedger` 接入前，页面不得把演示数据描述为真实组织账本。生产形态下的看板数据应是同一不可变流水按查询时刻动态聚合出的统计快照，而非长期冻结的静态副本；余额与流水近实时更新，图表聚合可允许约 5 分钟延迟。
...
- 用量看板的“预计可用”以近 30 天趋势作为唯一主估算；近 30 天日均与历史日均只作为解释性参考，不生成第二个预计天数，也不随期间分析筛选变化。估算保留零消耗日，不把少量活跃日误当成连续日均。类型图例和成员、项目、模型来源行可打开当前周期、当前维度的轻量明细抽屉，最多展示最近 20 条任务记录，并提供进入积分管理完整流水的明确入口；该抽屉不替代组织账本。
