...
- 工作台顶部账户入口以紧凑的“组织标识 + 可截断组织名 + 个人头像”组合显示；hover 展开后显示当前登录账号与所属的 `星海视觉工作室`。组织归属行展示当前账号角色（主账户 / 管理员 / 成员）并用箭头与 hover 状态提示可进入组织管理页；当前页面尚未实现时给出轻量提示。画布 iframe 也从宿主上下文读取同一账号、组织和角色，不再把所有成员硬编码为主账户。
...
- 旧画布内部的“回到主页 / 全部项目 / 退出登录”入口通过 bridge 请求宿主导航；宿主在 dirty 与保存任务清空后进入 React 路由或提交退出 action，不再依赖静态页面。
