...
- 视频节点参数栏包含模型、综合参数、提示词优化和高级设置；生成方式并入综合参数菜单的第一项。图片节点没有生成方式，保留模型、综合参数、提示词优化与高级设置的紧凑工具栏结构。
- 融合显示的积分 + 生成按钮。
...
参数不再按媒体类型写死，而由每个模型的能力表动态提供。节点工作区底部保持一条内容驱动宽度的主工具栏：模型名和综合参数摘要按实际内容紧凑排列，模型后保留细分隔；入口默认透明，仅在悬停、键盘聚焦或激活时显示轻底色，剩余空间由右侧操作区前的弹性间隔吸收。模型和综合参数分别打开独立弹层；图片与视频节点的提示词优化都是一级动作，完成后由用户查看并手动填入；高级设置是同一卡片的内联展开入口，不再形成“更多”二级菜单。
...
- 提示词优化仅在生成模式显示，Agent 模式隐藏优化与高级设置。节点和对话框共用下述优化建议工作流；对话框手动填入只改变该会话的 Prompt 草稿，作为一次可撤销的编辑器事务，不写 CanvasDocument / 画布撤销栈、不扣积分，也不触发生成。只读画布仍允许本地会话草稿编辑与优化，生成发送继续核验写权限。切模式关闭建议面板，保留原草稿。
...
- 填入后当前输入与上次写入快照相同、建议也未再编辑时，主操作显示禁用的“已填入”，不误报内容陈旧；编辑建议后恢复填入能力。若用户改变原输入、模型参数或参考，主操作改为“优化当前内容”，不覆盖新草稿。重新优化在原输入未变或仍是本次填入结果时使用原始来源和当前配置，避免递归扩写；其他变化明确采集当前输入。仅手动修改过同源建议稿的重新优化需要确认。
...
- 优化面板头部使用轻量方案下拉，只列当前模型的“平台默认”和自定义配置，不再在对照面板中展开指令表单。平台默认仅显示信息提示“平台默认会随当前生成模型自动切换对应的优化规则。”，不展示或允许编辑实际平台指令。自定义行在 hover / focus 时显示编辑入口，底部“+ 自定义优化配置”打开独立的小面板。
