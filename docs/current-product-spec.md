...
- 工作台账户面板在身份与积分之后显示真实可访问的工作空间，再沿用画布个人菜单的账号设置、外观、帮助中心和退出结构；帮助中心包含快捷键、使用教程和反馈问题。头像支持 hover 预展开与键盘打开，面板离开后延迟收起。
...
- 个人与积分：可用积分显示在头像左侧，并与头像保持在同一绑定容器；积分徽标使用与工作台一致的 Sparkles 图标，不表达在线状态。hover 头像预展开账户菜单，点击积分徽标时才在账户菜单中显示积分详情。
...
- 演示会话 token 以摘要存库并具有过期 / 撤销状态，但两个固定账号、确定性 demo 密码散列和两级角色仍不是正式账号生命周期或完整权限系统。
...
Reelay 当前是一个本地可运行的 AIGC 创作平台原型：两个演示账号可通过受保护的 React 工作台使用持久化的隔离个人空间与共享组织项目，再进入旧无限画布验证生成节点、媒体组织和 Agent 辅助；正式账号生命周期、画布内容、资产、生成任务与积分仍未达到生产持久化标准。
