...
- 媒体预览框；空框中央使用清晰、适度放大的媒体类型图标，并保持视觉居中。
- 节点创建时确定的图片或视频类型。
- 选中媒体框并完成单击或拖动释放后显示的节点工作区；工作区包含参考素材、提示词输入区和底部参数栏。
- 参数栏的模型、生成方式、综合参数、更多四个独立入口。
- 融合显示的积分 + 生成按钮。
...
- 模型入口只展示与当前节点类型一致的模型。图片节点只能在图片模型之间切换，视频节点只能在视频模型之间切换；跨类型创作必须新建对应类型的节点。
- 模型选择是底部参数栏中的独立弹层，触发器显示当前模型；展开后的具体模型继续显示各自的模型标识，生成节点与 Agent 的模型入口仍统一使用线框立方体图标。
- 模型弹层相对自身触发器进行锚定，不通过固定的画布坐标摆放；具体定位遵循 7.4 的锚点重力和碰撞规则。
- 音频暂不提供独立生成模型，只作为素材类型或视频生成节点的综合参数存在。

### 7.3 提示词输入区

- 选中生成节点媒体框并在指针释放后，节点下方显示参考素材、提示词输入区和参数栏；按下与拖动过程中不提前展开。点击其他区域可以收回节点工作区。
...
- 输入区打开时会根据画布比例做有限度的反向缩放补偿：常用桌面缩放区间内以约 `800px` 的屏幕宽度保持稳定阅读和操作比例，画布缩小时相对放大，画布放大时相对收紧。
- 窄视口会按画布可用宽度留出边距；极远景和极近景仍受补偿上下限约束，不会无限放大或缩成脱离节点的悬浮面板。

### 7.4 参数

参数不再按媒体类型写死，而由每个模型的能力表动态提供。节点工作区底部保持一条收拢的参数栏，模型、生成方式、综合参数和更多分别打开独立弹层：

- 模型弹层只提供当前节点类型可用的模型；生成方式弹层承载文生、参考生成、首尾帧或图生等当前模型支持的输入方式。
- 综合参数弹层集中显示比例、分辨率或画质、时长和音频等能力项；收拢状态使用一行摘要显示当前综合参数。
- 更多弹层包含提示词优化和定时任务。提示词优化已经进入节点状态，可在节点内开启或关闭；定时任务仍未实现，当前不得表现为已经能够创建、保存或执行调度任务。
- 四类弹层都以各自触发器为锚点，使用统一的锚点重力定位：优先放在设计指定的一侧，空间不足时沿主轴翻转，并在交叉轴内平移以避开视口边缘；弹层保持屏幕空间可读尺寸，不随画布缩放变成固定在世界坐标中的大层或小层。
- 同一节点同一时间只展开一个参数栏弹层；切换入口时原弹层收起，点击外部或按 `Esc` 关闭当前弹层。
...
- 多节点一起移动。
- Alt 拖拽复制节点，并保留内部参数。
- 多选后除逐个节点的选中边线外，还显示比可见内容统一外扩的共同包围框。共同框使用轻量四角定位线、主题化半透明底色和极弱的内侧层次阴影，而不是另一层完整实线；深色主题下仍清楚可辨，同时避免与节点选中态混淆。右侧框外中心只显示一个整体输出端口，多选期间所选节点自己的端口隐藏。
...
- 创建组后生成可持久化的可视化组框，组框带标题、低饱和冷灰边界和组级工具条；组名字号只略大于节点左上角名称。选中组框后保留固定屏幕尺寸的透明角点命中区，但四个圆角折线提示不常驻；鼠标进入对应角点附近时才柔和显现，拖动期间只保持当前活动角可见。四条边中继续保留透明的宽命中区与对应缩放光标，不再显示蓝色圆点或胶囊把手；无 hover 的触摸设备上则以较低透明度显示四角提示。标题与组级工具条以组框上边缘为锚点，并对画布缩放做有上下限的屏幕尺寸补偿，远景保持可读、近景不过度放大。宽度足够时两者保持紧凑同排，窄组或长组名发生碰撞时工具条自动上移一行。
- 点击组内节点时仍选中节点；点击组框空白区域时选中组。
- 选中组框后显示“布局、整组执行、解组、下载”，并可拖动整组或手动拉伸组框边界；八向缩放手柄保留稳定的屏幕命中尺寸，不会在远景缩成难以操作的细点。组框编辑期间手柄优先于组内节点的端口热区，点回具体节点后端口交互恢复。
- 组框是独立的区域容器，不再自动按成员节点收缩。节点进入组框的主要覆盖区域会加入组，移到主要区域之外会移出组；加入与移出采用不同覆盖阈值，避免边界附近抖动。缩放组框结束后会重新结算当前成员和框内未分组节点，移动组框则只带动结算后的当前成员，不会重新吸入沿途节点。
