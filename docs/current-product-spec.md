...
- `个人空间` 的持久目录和自定义标签只对本人可见；`组织空间` 的持久 Media 目录和自定义标签由当前 Workspace 成员共享，成员可直接保存素材，不隐式发起审核。组织成员身份由服务端独立校验，不用项目编辑权限替代；`平台空间` 定位为创作参考发现，目前为本地只读短片目录：12 个 5–30 秒连续视频节选，覆盖人物动作、自然空间、动画叙事、城市科幻与幻想场景，新增开放短片的来源与署名可从详情来源链接查看，来源标签保留原发布素材名，不代表已接入真实电影搜索索引。两列 16:9 卡显示标题、来源、时长、真实镜头数与轻量点分隔特征；常显搜索覆盖内容、来源、逐镜头描述和常用技法别名，多个空白分隔词取交集。搜索保持常显，内容与场景词通过标题、描述、镜头文案和别名检索，不在筛选中穷举。点击筛选按钮展开单页浮层，默认六行摘要：景别、运镜、光影、构图、剪辑、片段时长；每行默认“不限”，已选项用顿号连接并单行省略。一次展开一行，显示可搜索的选项列表；选项搜索覆盖名称与别名，保持输入 DOM 和中文输入法，不改变主搜索或已选条件。只展示全库有素材的值，常用项按全库覆盖数排序，时长按区间升序；最多约六行选项可见，余项滚动。选项右侧显示保留其它维度及主搜索时的匹配数，零结果仍可选择，不静默放宽条件；行内“不限”清除该维度，顶部重置清除所有筛选但保留主搜索。底部结果数与“完成”固定可见；即时生效，完成 / 外部点击 / Escape 收起，Escape 与完成返回入口焦点。浮层最大宽度 360px、受资产栏约束，不占列表布局；关闭后筛选按钮数量回显，切空间保留条件，切画布重置。分类内多选取并集、跨维度及主搜索取交集；已知明确景别从镜头 framing 提取，未标注不猜测；片段时长采用真实秒数，区间为 ≤10 秒、>10 且 ≤20 秒、>20 秒，不卡整数四舍五入。标签只证明片段包含该特征，多个维度不保证落在同一镜头，尚未实现镜头级命中定位。卡片技法文字可点击，幂等追加该条件并保留其它条件及主搜索，不打开详情；多选时变为普通文字，避免误改条件。筛选变化清除批量选择，未实现自然语言语义搜索。不提供图片 / 音频类型、主体标签或未标记。悬浮 / 聚焦卡片 180ms 后只挂载一个静音循环视频，离开、滚动、多选、打开详情、收起侧栏或页面隐藏停止并释放资源；减少动态效果时只展示静态封面。平台不显示目录、上传、新建、主体入口或单条修改菜单；多选保留添加到画布，保存到素材在受控 import 落地前仍禁用。
...
- 参考使用保持手动：“复制提示词”仅复制当前整段 / 单镜头的已编辑文本，要求当前范围分析有效、处理完成且非空；成功按钮就地显示“已复制”，编辑、改范围或切镜头后恢复，复制失败可重试，旧异步完成不污染新会话。“完整片段加入画布”添加整个目录片段，沿用可撤销媒体节点创建及 librarySourceId；选段只限制播放与分析，尚不生成裁切文件。移除平台详情到 Agent 草稿的自动连接，不改变已有会话、发送、生成或扣费。详情以同一检索词汇展示参考特征及镜头数；关闭释放视频并回焦原卡，同一画布会话内记住最近 12 个片段的范围、结果、提示词与播放位置；刷新不保留，切画布 / 权限撤销清理，只读可查看分析和复制但不能添加节点。实际 AI 分析、语义检索、自动推荐、项目参考集合、主体替换、裁切导出和持久平台目录尚未接入。
