...
- 多节点一起移动。
- Alt 拖拽复制节点，并保留内部参数。
- 多选后除逐个节点的选中边线外，还显示由所选节点可见边界并集确定的共同包围框；共同框四边直接贴合最外侧节点的对应边，不再增加额外屏幕留白，节点之间的空白才作为共同空间。共同框使用完整但低对比的 `1px` 主题边线、极轻填充、内侧高光和两级柔和外阴影建立临时空间容器；它不复用节点选中线，也不再用四角短线与节点轮廓争抢。浅色主题使用冷灰暗投影并控制填充透明度，以免洗白媒体；深色主题把外阴影反转为低透明浅色扩散，使共同框能从近黑画布中分离，但不形成高亮光环。共同框内部未被节点、工具条、整体连接端口、连线或持久组框占据的空白区域显示抓手并可整体拖动选中节点；按下后沿用节点拖动的 `4px` 阈值、组归属结算和单条撤销记录，取消事件恢复全部起始坐标。右侧框外中心只显示一个整体输出端口，多选期间所选节点自己的端口隐藏。点击“打组”生成的持久组框继续使用独立的组内留白规则，因此会比临时共同空间稍大。
