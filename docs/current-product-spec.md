...
- 待机时仅内部 Logo 活动，54px 圆形底座和点击范围保持稳定。空闲 1.5–2.5 秒后先双眨眼并轻晃身体，此后每隔 6–10 秒单眨眼，每第三次附带轻晃。双眨眼共 620ms、单眨眼 240ms；身体轻晃 1.25 秒，最大旋转 6°、位移 2px。独立眼睑遮合原图的透明眼睛，不压缩身体或重画品牌轮廓。悬停 / 键盘聚焦显示 `Reelay Agent`，Logo 轻抬 2px、倾斜 5°并放大至 1.06 倍；鼠标进入时取消旧待机动作，立即双眨眼回应一次，停留后保持安静，移出再排待机。按下时轻压，点击展开侧栏，圆形入口隐藏。面板展开、页面隐藏 / 离开、键盘可见焦点进入或销毁时，停止 timer 并取消未完成动作；恢复可用后重新排程，不补播。减少动态效果或浏览器不支持动画时保持静态。浅深主题的图形与眼睑一起反色，不改变生成任务状态。
- 展开后的收起入口位于标题栏内部最左侧，在会话标题前方，使用官方 Lucide `panel-right-close`；与右侧多选 / 新建共用 32px 点击区、8px 圆角与 16px 图标。常态无底色、无描边，悬停显示浅底并提示“收起对话”，键盘聚焦有清晰轮廓；图标和点击区保持稳定，不使用外凸耳片或 Logo 探头。标题与收起按钮组成左侧分组，长标题省略，窄窗保持同一结构。整个标题栏随面板一起收放，收起后原右上角 Logo 入口恢复并接收焦点；面板整体管理隐藏与键盘访问，收起保留会话、草稿和生成任务。减少动态效果时即时切换。
