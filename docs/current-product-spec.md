...
快捷键说明放在右上角个人菜单的“帮助”子层中；“帮助”使用 hover intent 展开：鼠标进入帮助行时自动展开缩进子层，向下滑动到子层或“退出登录”路径时保持展开，不让退出登录位置因子层收起而跳动；只有鼠标向上越过帮助行或关闭个人菜单时才收起。帮助父级也支持键盘 Enter / Space 展开或收起。帮助父级使用问号语义图标；父级右侧箭头在收起时向右，展开时旋转为向下，表达当前层级状态。子层包含“快捷键”、“使用教程”和“反馈问题”，视觉上从属于帮助而不与主菜单同级。子层不使用左侧竖线，也不显示右侧展开/外链提示图标，避免轻量菜单内信息噪声过高。hover 到“快捷键”时再展开窄高型快捷键清单；清单不重复显示标题，仅保留语义标签，并与个人菜单保持可见间距，避免遮挡主菜单。点击“使用教程”会打开线上手册，点击“反馈问题”会打开飞书反馈表。外观菜单项右侧提供浅色、深色、跟随系统三个行内图标按钮，点击后滑块移动到对应模式，左侧外观图标同步切换。
...
- 撤销：当前活动画布映射到各 `CanvasDocument` 自己的 `undoStack`；项目重命名不写入该栈，生成中的节点不会被参数撤销恢复为陈旧任务状态。
...
- 当前生成任务只存在页面内存中，虽已记录启动参数快照，但还没有持久化、取消 UI 或失败退款。
