...
- 组织中心使用 `/app/w/:workspaceId/organization`、`/organization/credits` 与 `/organization/usage` 三个可深链地址，并由同一父级路由加载和共享组织上下文；在左侧切换组织管理、积分管理和用量看板时不重复请求会话、工作区与成员数据，直接访问或刷新任一地址时仍会正常加载。为减少重复上下文，这组三页只保留品牌头，不重复显示工作台账户入口。组织中心在普通笔记本上保持紧凑桌面排版，并在 `1680px`、`2200px` 以上的宽视口逐级扩展外层面板和内容区，避免大屏继续受笔记本宽度上限约束。
  ...
  - 组织用量页仅对主账户与管理员开放，使用明确标注且可整体替换的确定性前端演示数据。页面按“稳定概览 → 期间分析 → 主要消耗来源 → 365 天活动”的顺序组织：组织可用积分、按近 30 天趋势计算的预计可用、近 30 天日均消耗和历史日均保持稳定口径，不随筛选跳变；期间分析的指标、趋势、类型环形构成、成员 / 项目 / 模型来源排名与导出共用同一个已应用统计范围。类型构成使用固定语义色环形图并可下钻，成员、项目与模型使用横向排名，长列表收敛为 Top 5 + 其他且总额与周期摘要一致。任务级扣费、退款与成员流水统一回到积分管理下钻，不在用量页复制完整流水表。内部分配不计入组织消耗；在 `GenerationTask`、计费快照与不可变 `CreditLedger` 接入前，页面不得把演示数据描述为真实组织账本。生产形态下的看板数据应是同一不可变流水按查询时刻动态聚合出的统计快照，而非长期冻结的静态副本；余额与流水近实时更新，图表聚合可允许约 5 分钟延迟并标示更新时间。
  - 期间分析显式保留“今天 / 近 7 天 / 近 30 天 / 日期范围”四个入口，默认使用近 30 天。三个固定范围立即应用且控件宽度稳定；日期范围使用带“本月 / 上月 / 全部历史”快捷项的确认浮层，日期编辑只改变草稿，点击“应用”后才更新数据，取消、点击外部或 Escape 均不修改已应用统计。手动范围首尾日期都计入统计，并在再次打开时保留；日期入口按最终选择显示本月、上月、全部历史或 `MM/DD–MM/DD`，数据区同时显示实际统计日期。比较口径分别为昨日同期、前 7 天、前 30 天、上月同期、上上月和前一等长周期；全部历史不显示无意义环比。所有自然日边界按组织时区 `Asia/Shanghai` 表达。
