...
- 左上角是项目级导航，只承担返回主页、当前项目名和项目菜单。
- 左侧中部是稳定尺寸的竖向胶囊栏，依次常显内部画布、资产库、分享和个人入口。分享紧邻个人入口上方，不占据首项；它不再依赖 hover 展开，避免胶囊轮廓跳变并保持入口可发现。分享调用系统分享能力，缺少支持时复制当前项目链接。带自定义提示的入口只保留 `aria-label`，不再叠加浏览器原生 `title` 提示。
- 左下角是视口工具胶囊，依次提供小地图、适应视图、暂不可用的整理画布和 20%–200% 缩放滑条。滚轮位于整个工具胶囊及其空白内边距时都不会带动画布。
- 右上角只常驻 Agent launcher；个人、积分和分享不再与 Agent 组成同一横向操作组。

个人菜单承载账号身份、“我的积分”、组织中心、账号设置、主题、帮助与退出，并以 `我的积分 → 组织中心` 作为前两项；退出账号使用与普通菜单项一致的中性色，不在菜单内预先使用危险红色。个人入口 hover 时只显示轻量文字提示，点击后才展开交互菜单；再次点击、点击菜单外部或按 Escape 关闭。菜单底部与整个左侧胶囊外框的底边对齐，而不是与内部个人头像圆圈对齐；由于帮助内容不再参与主菜单排版，该锚点在交互过程中保持稳定。个人入口的文字提示只在菜单关闭时出现，菜单展开后立即隐藏，避免与面板重叠。积分行显示当前可用余额并随本页模拟扣费更新；整行可点击，通过严格的 iframe bridge 直接打开 React 账号设置的“我的积分”分栏。个人按钮使用当前姓名生成可访问名称，并同步展开状态。帮助中心 hover 或键盘展开后，在主菜单右侧显示独立子菜单，不挤占主菜单内部空间；子菜单包含“快捷键”、“使用教程”和“反馈问题”，右侧箭头保持指向侧面层级。离开帮助中心及其子菜单后收起。hover、键盘聚焦或点击“快捷键”时，再向更右侧展开 `246px` 的紧凑单列快捷键卡片，入口同步维护真实 `aria-expanded` 状态；卡片不重复显示标题，按“画布操作 / 节点操作”分组，并用统一的轻量组合键标签表达输入。清单内容必须对应当前运行代码，包括中键或空格配合拖动来移动视图、滚轮平移、`Ctrl/Cmd + 滚轮` 缩放、双击空白打开添加节点菜单、框选与追加多选、复制拖动、删除、撤销和分层关闭，不能把计划能力或过时绑定写入帮助。点击“使用教程”会打开线上手册，点击“反馈问题”会打开飞书反馈表。外观菜单项直接显示当前主题名称（浅色模式 / 深色模式），点击整行即可在两种模式间切换；右侧切换胶囊平时隐藏，hover 到整行时轻量显示，点击切换时短暂增强并播放滑动反馈动画。

Agent 对话栏展开或调整宽度时，不拖动左上、左侧和左下工具区。生成节点不设置常驻添加按钮；用户通过双击画布打开“添加节点”菜单，明确选择图片生成、视频生成或上传，空状态只提供视觉提示。资产库展开后，左侧胶囊隐藏，左下工具平移到面板右侧；面板使用独立的“返回画布 / 资产库 / 空间切换”头部，不复制项目导航。数量统计继续位于面板底部；当前按桌面视口验收，历史窄屏规则仅作为防御性降级保留。
...
- 资产库展开时，画布自身左上角导航隐藏；资产库头部只提供返回画布、资产库标题与空间切换，不在管理素材时重复放置项目和内部画布菜单。关闭资产库后恢复原画布导航。
...
- 面板首选宽度为 `550px`，右边缘可用指针拖拽或键盘方向键在 `380–780px` 间调整。和 Agent 同开时可以临时压缩，但会保留用户最近选择的首选宽度。
...
左侧中部胶囊中的资产库是当前画布使用和整理可复用媒体的入口。它已经采用一个无 DOM 的页面内存 store 管理素材、主体、文件夹和空间 placement，但尚未接入 Workspace Asset repository、对象存储或正式权限服务。
...
- 素材分栏的主操作是多文件上传，接受图片、视频和音频，并把可用媒体登记到当前可编辑空间与目录。本地文件继续使用 `blob:` 对象地址，只在本页生命周期内有效。
- 主体库的“创建主体”目前同样打开文件选择器：所选媒体先去重登记到当前空间素材库，再原子创建只引用这些 Media 的主体；创建失败不会留下半个主体。它还不是带字段表单、既有素材选择器和版本管理的完整主体编辑器。
...
- 个人空间的单项或批量操作包括移动、共享到组织空间和删除；组织空间不显示“共享到组织空间”。素材分栏中的图片与视频还可提交 Seedance 合规审核，但音频素材单项以及任何包含音频素材的批量选择不提供该操作；审核仍只在内存中登记幂等请求并显示反馈，没有接入审核服务或结果页面。
- 多选只作用于当前筛选后可见的条目；进入多选后，命令组按 `全选 → 取消 → 筛选 → 视图` 排列，“取消”会明确退出多选并清空当前选择。切换空间、分栏、目录或搜索 / 筛选上下文也会清理不再可见的选择，避免跨上下文误操作。
...
- 画布节点目前仍接收 legacy 媒体副本，并用 `librarySourceId` 维持迁移期来源关系；这不是正式 `AssetReference`。资产 store、文件夹、主体、审核和选择状态都不写入 CanvasDocument。

持久化边界：

- 初始素材、主体、文件夹和 placement 来自确定性 seed；上传、创建、整理、共享和审核产生的变化也只保存在页面运行内存中，刷新会整体重置。平台内容只是只读 seed，不是已上线的公共目录服务。
- 正式数据层仍采用 Workspace Media Asset + Project / Node AssetReference；GenerationResult 只有显式保存后才晋升为 Asset。平台目录的版本与导入策略、组织资产权限、Blob/ObjectStore、跨项目持久引用检查和跨刷新持久化尚未实现。
...
- 资产库 UI：`librarySection`、`librarySpace`、`libraryFolderId`、`librarySearch`、`libraryFilter`、`libraryDisplay`、多选 / 菜单 / 重命名 / 移动 / 预览状态，以及按空间与分栏记忆的搜索和筛选；领域数据由独立的页面内存 asset-library store 持有，二者都不写入 CanvasDocument。
- 主题：`themeMode`。
...
- 资产库内部已经用 Media / Entity / Folder / Placement 的无 DOM 内存 store 取代多组松散数组，主体只保存 Media 引用；但画布消费仍会创建 legacy 媒体副本，Blob URL 生命周期、Workspace Asset repository、对象存储与正式 AssetReference 尚未落地。
