...
- 工作台顶部账户入口以紧凑的“组织标识 + 可截断组织名 + 个人头像”组合显示；hover 展开后显示当前登录账号与 `星海视觉工作室`。组织归属与积分摘要合并为同一账户概览块，组织行在上、积分摘要在下，共用底色与圆角，内部仅使用低对比度分隔线；组织行不再重复显示“所属组织”分组标题，直接展示账号角色（主账户 / 管理员 / 成员）。组织图标、名称、角色和末端层级箭头均保持稳定；点击组织行进入当前 Workspace 的独立组织中心路由。画布 iframe 也从宿主上下文读取同一账号、组织和角色，不再把所有成员硬编码为主账户。
...
- 组织中心使用 `/app/w/:workspaceId/organization`、`/organization/credits` 与 `/organization/usage` 三个可深链页面；为减少重复上下文，这组三页只保留品牌头，不重复显示工作台账户入口。组织管理把精简组织信息和成员管理放在同一页；组织卡显示成员数与组织 ID，主账户可在卡片内预览修改组织头像和名称。成员列表来自服务端 Workspace Membership 投影，显示真实演示成员、登录标识与组织角色，不读取项目权限；主账户和管理员可从角色列打开独立选择浮层，主账户角色不可变，管理员也不能修改自身角色。组织资料与角色修改、凭证重置和会话撤销当前均明确为前端演示，不会修改共享组织、演示账号、会话或 Membership；系统不展示任何成员的现有密码。积分页按“累计入账积分 − 已分配积分 = 未分配积分”展示明确标注的 `100000 / 33000 / 67000` 原型口径和详情抽屉，不冒充真实 `CreditLedger`；组织用量页只展示待接入的指标结构和真实空状态，不绘制虚构趋势。
