...
- 节点工作区使用 `248–320px` 的屏幕内容自适应高度和 `20px` 圆角；左上主体与参考素材入口为 `48 × 48px`、`14px` 圆角，以 `16px` 内边距排布，主体入口、参考入口与后续素材架的左侧起点依次相距 `68px / 64px`。空内容与短提示词使用默认高度，输入内容增加时编辑区逐行增高，主工具栏始终贴住编辑区底边。高级设置在同一卡片底部内联展开：图片节点增加 `118px`，视频节点增加 `154px`，不会压缩提示词或移动主工具栏在编辑区内的相对位置。
...
- 下载始终位于工具栏右侧并使用图标表达。
- 工具栏使用独立缩放补偿，在远景和近景下保持接近一致的屏幕可读尺寸，并随画布同帧更新；靠近视口顶部时自动下压到可见区域。
...
- 左上角显示类型图标 + 文件名。
- 类型图标与名称以媒体框上沿为基准保持稳定间距，并使用有上下限的屏幕缩放补偿：名称以 `13px` 为基准，乘以限制在 `0.85–1.8` 范围内的画布倍率，远景保留可读下限、近景随媒体适度放大，但不会压入媒体框边线。名称按缩放后的实际可用宽度省略；媒体框小于 `250px` 屏幕宽度时隐藏右侧规格，把顶部空间留给名称。
- 默认去掉文件扩展名作为显示名。
- 可双击改名。
- 改名只影响画布显示名，不改变原始文件名。
...
展开后的面板由顶部对话栏、中间消息区域和底部一体式生成输入区组成。默认桌面宽度约 `560px`，仍可在联合画布走廊约束内调整；默认高度贴合完整视口，顶部与底部可独立拖动或通过键盘调整，并始终保留输入区可用的最小面板高度。空对话不再重复放置大号品牌欢迎区，创作引导由底部输入框占位文案承担。
