...
右上角头像下方保留轻量积分徽标，初始可用积分为 `3000`；个人菜单中显示可用积分和累计消耗。当前测试原型只在单次页面生命周期内扣减可用积分并增加累计消耗，刷新页面后重置为 `3000 / 0`。
