...
- 左下角画布工具组按 `小地图 → 适应视图 → 整理画布 → 画布比例` 排列；“整理画布”当前只显示不可操作的“即将支持”占位，不暗示已经具备节点与连线自动编排能力。资产库和个人入口已移入左侧中部的竖向浮动栏，快捷键入口位于个人菜单的“帮助中心”子层。
...
- 左侧中部是竖向胶囊栏，依次提供内部画布、资产库和个人入口。鼠标进入胶囊或键盘焦点进入其中时，分享入口从胶囊上方滑出；隐藏时使用 `visibility`、`pointer-events` 和焦点顺序共同退出交互，不留下不可见按钮。分享调用系统分享能力，缺少支持时复制当前项目链接。
...
- 项目名默认是 `Untitled`；可写画布支持双击或键盘 `Enter / F2` 进入改名，`Enter` 提交、`Escape` 取消。当前 iframe 内的项目改名仍是页面级原型状态，不描述为服务端项目持久化。
...
styles/canvas-chrome.css
  在不侵入组框与节点视觉规则的前提下定义画布壳层四区布局、悬浮材质、工具 reveal、资产面板头部和明暗主题覆盖。
