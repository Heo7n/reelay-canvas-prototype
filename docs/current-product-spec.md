...
- 多节点一起移动。
- Alt 拖拽复制节点，并保留内部参数。
- 多选后显示由所选节点可见边界并集确定的共同包围面；四边直接贴合最外侧节点的对应边，不增加额外屏幕留白，节点之间的空白才作为共同空间。共同包围面拆成职责独立的两层：节点下方的选择面只绘制接近透明的中性面积差与向内渐隐的柔和膜边，不使用连续边线、外部卡片阴影或多级扩散光；节点上方的透明交互层只承载整体端口与命中矩形。两层复用同一个屏幕矩形，因此缩放时不会产生几何分离；节点会自然遮住与自身重合的选择膜，不再由完整亮线或硬边压过节点。多选成员恢复普通节点结构边线，集合范围由下层选择膜表达。共同包围面的圆角复用节点媒体框的主题规格并投影到当前屏幕倍率，不使用固定屏幕圆角，因此远景下四个角仍与外侧节点轮廓贴合。深色主题用克制的冷灰内收膜边保证暗背景辨识；浅色主题进一步降低中心面积差，以柔和内缘替代容易被误读为持久容器的外轮廓。两套 token 分别定义而非简单反相，hover 与按下只逐级增强内缘和极弱填充。连接态只增强整体端口，不点亮整个选择面。共同空间中未被节点、工具条、整体连接端口、连线或持久组框占据的空白区域显示抓手并可整体拖动选中节点；按下后沿用节点拖动的 `4px` 阈值、组归属结算和单条撤销记录，取消事件恢复全部起始坐标。右侧框外中心只显示一个整体输出端口，多选期间所选节点自己的端口隐藏。点击“打组”生成的持久组框继续使用独立的组内留白规则，因此会比临时共同空间稍大。当选择集合精确等于单个持久组的全部成员时，持久组框是唯一空间边界，内层临时选择面及其空白拖动命中停用；节点批量工具栏和共同输出端口仍保持可用，不把节点选择强制转换为组选择。少选任一组员、跨组选取或加入组外节点后，临时选择面立即恢复。
...
- 创建组后生成可持久化的可视化空间容器，组框带贴住上边界的标题签、位于框外的组级工具条，以及独立于临时选择膜的容器材质；默认、hover、选中分别使用克制的整体填充、顶部轻洗、稳定边界和标题层级表达状态，深浅主题各自定义适配值。所有状态保持稳定的 `1 CSS px` 容器边界，只改变对比、顶部轻洗与短距深度，不通过加粗边线、远距离发光或缩放框体表达选中。标题签使用独立主题表面和轻边界承载组身份，组名字号只略大于节点左上角名称；工具条不进入容器内部，避免远景下覆盖组内节点。选中组框后在节点层上方提供四个固定 `26px` 屏幕命中区，只有鼠标进入对应角点附近或正在拖动该角时才显现圆角折线提示；四条普通边线不再承担不明确的缩放热区，避免靠边节点、节点端口和整组拖动互相抢事件。无 hover 的触摸设备上以较低透明度显示四角提示。标题与组级工具条都以组框上边缘为锚点，并对画布缩放做有上下限的屏幕尺寸补偿，远景保持可读、近景不过度放大；标题贴边、工具条框外悬浮，窄组或长组名发生碰撞时工具条自动上移一行。
