...
- 添加节点：在视口中心创建生成节点。
- 素材库：打开项目素材面板，可上传、筛选并把图片、视频或音频加入画布。
- 个人：打开账号菜单，包含可用积分、累计消耗、进入组织、账号设置、外观、帮助中心、退出登录。
...
| 类型 | 模型 |
| --- | --- |
| 图片 | GPT Image 2、Nano Banana Pro、Nano Banana 2、Midjourney V8.1/V7、Niji 7、Seedream 5.0 Lite、Kling Image 3.0/Omni、Imagen 4、FLUX.2、Stable Diffusion 3.5 |
| 视频 | Seedance 2.0/Fast/Mini、Kling Video 3.0/Omni、Veo 3.1/Lite、Runway Gen-4.5、Luma Ray3.2 |
| 音频 | Suno v5.5、Eleven Music v2、Eleven Sound Effects v2、Eleven Multilingual v2、Lyria 3、Stable Audio |
...
- 从本地直接拖入画布或生成节点的媒体，也会同步登记到当前项目素材库。
- 素材库当前保存在页面运行内存中；本地文件使用浏览器对象地址，刷新页面后不会保留，后续需要由真实上传与项目存储服务替换。
...
- 图片默认提供 HD 放大、裁剪、去背景、加入素材库；扩展菜单包含橡皮擦、调整和旋转。
- 视频默认提供画质增强、裁剪片段、提升帧率、加入素材库。
- 音频默认提供音质增强、裁剪片段、降噪、加入素材库。
- 下载始终位于工具栏右侧并使用图标表达。
- HD/画质/音质增强当前更新原型媒体状态；加入素材库和下载已具备实际交互，其余编辑项保留明确的服务接入入口。
