...
- 工作台顶部账户入口以紧凑的“组织标识 + 可截断组织名 + 个人头像”组合显示；hover 展开后显示当前登录账号与 `星海视觉工作室`。组织归属与积分摘要合并为同一账户概览块，组织行在上、积分摘要在下，共用底色与圆角，内部仅使用低对比度分隔线；组织行不再重复显示“所属组织”分组标题，直接展示账号角色（主账户 / 管理员 / 成员）。积分图标、标题与累计消耗说明沿用下方菜单项的左侧文字基线，余额独立右对齐。组织图标、名称、角色和末端层级箭头均保持稳定；点击组织行进入当前 Workspace 的独立组织中心路由。画布 iframe 也从宿主上下文读取同一账号、组织和角色，不再把所有成员硬编码为主账户。
- “账号设置”在 React 宿主中以桌面弹出面板打开，只包含“个人主页 / 积分记录”两栏；组织级用量统一进入组织中心，不在个人面板中重复展示。不包含订阅和设备管理。个人主页使用统一信息面：上方左右展示个人身份与组织归属，下方并排展示邮箱和手机；不显示演示态标签或重复登录标识。可选邮箱与手机在格式有效后自动保存，不提供显式保存按钮；格式无效或请求失败时保留当前输入并给出提示。联系资料不改变登录标识，也不会自动订阅报表；后续报表发送必须另设显式开关。积分记录在真实账本接入前只展示标明边界的空状态，不生成虚构流水。旧画布中的账号设置入口通过 bridge 打开同一面板。
- 组织中心使用 `/app/w/:workspaceId/organization`、`/organization/credits` 与 `/organization/usage` 三个可深链地址，并由同一父级路由加载和共享组织上下文；在左侧切换组织管理、积分管理和用量看板时不重复请求会话、工作区与成员数据，直接访问或刷新任一地址时仍会正常加载。为减少重复上下文，这组三页只保留品牌头，不重复显示工作台账户入口。组织中心在普通笔记本上保持紧凑桌面排版，并在 `1680px`、`2200px` 以上的宽视口逐级扩展外层面板和内容区，避免大屏继续受笔记本宽度上限约束。
  - 组织管理把精简组织信息和成员管理放在同一页；组织卡显示成员数与组织 ID，主账户可在卡片内预览修改组织头像和名称。成员列表来自服务端 Workspace Membership 投影，显示真实演示成员、登录标识与组织角色，不读取项目权限；主账户和管理员可从角色列打开独立选择浮层，主账户角色不可变，管理员也不能修改自身角色。主账户和管理员可打开非主账户成员的账号三点入口；紧凑浮层用于演示重置登录密码和停用 / 恢复账号，但当前登录账号只提供密码重置，不允许停用自身，管理员也不能操作主账户。停用状态只在当前页面生命周期内降低该成员行的视觉权重，不额外显示状态文字，刷新后恢复，普通成员的不可用账号操作显示为 `—`。组织资料、角色和成员账号操作均不会修改共享组织、演示账号、会话或 Membership；系统不展示任何成员的现有密码。
  - 积分页以单张“组织积分余额”总览展示当前尚未消耗的 `100000` 积分，并按“可分配余额 `67000`、所有成员账户余额 `33000`”的顺序展示比例与静态构成。余额数字和构成区只负责展示，不承担导航；余额卡片上方的轻量操作栏提供明确“积分流水”入口并默认打开入账页签，成员余额标题右侧的“分配明细”入口打开分配页签，成员行“明细”打开该成员筛选后的消耗页签。积分流水使用宽度随桌面视口在约 `960px` 至 `1360px` 间扩展的右侧抽屉，固定标题与“入账 / 分配 / 消耗”页签，表格内容独立滚动且不依赖横向滚动。入账页只以“累计入账积分”概览外部积分进入组织，并列出时间、类型、来源说明和入账积分；分配页按“可分配余额、本月发放、本月回收、成员账户余额”的顺序概览，流水展示成员额度发放 / 回收、有效期、操作人与备注；消耗页不重复余额概览，以“时间、成员、项目、任务 / 模型、结算状态、消耗积分”的组织任务流水解释扣费，不显示扣减后余额。分配与消耗支持按成员筛选。发放和回收当前只给出前端反馈、不改写余额；抽屉不重复显示演示标签，但其中记录仍是可整体替换的确定性前端数据，不代表已经接入真实 `CreditLedger`。
  - 组织用量页仅对主账户与管理员开放，使用可整体替换的确定性前端演示数据；页面不重复显示演示标签或更新时间，演示边界由产品说明保留。首行并列窄幅“概览”与宽幅“长期活动”，下一整行承载“期间分析”，让稳定经营状态、长期分布和所选期间洞察形成清晰层级。概览以组织可用积分与预计可用为两个核心结论，按“标题与可用积分 / 双列日均参考 / 整行预计可用”的纵向节奏组织；层叠图形仅作低对比度语义提示，不承担数据表达。预计可用的说明继续明确引用近 30 天日均消耗依据，不显示额外同比信息。概览口径保持稳定，不随期间筛选跳变。期间分析标题与带年份的实际日期范围同行呈现，不再单独重复图片产出、视频产出和总消耗摘要；消耗走势以整行、随容器宽度自适应的图表建立时间变化主线，并在整个时间区块内提供日期、积分和任务数的悬停预览；下方以左窄右宽方式并列消耗构成与模型 / 成员 / 项目消耗来源。消耗构成使用上方居中环形图与下方填满可用纵向空间的双列图例，避免在窄栏内横向挤压；图例优先突出实际产出量，视频精确到秒并显示 `s`，图片显示张数，操作显示次数，积分与占比作为消费指标。环段与图例支持悬停、键盘聚焦联动，环心临时显示对应类别积分和占比并弱化其余类别，移开后恢复总消耗。完整分类数据始终保留在图例中，悬停不作为唯一信息载体。图表、来源排名与导出共用同一个已应用统计范围；看板只保留低层级“导出用量报表”入口，将当前范围导出为 Excel 聚合报表，包含组织概览、每日用量、消耗构成以及模型 / 成员 / 项目排行，不从看板导出 CSV 或逐任务积分流水。“消耗来源”默认显示模型维度，只统计图片与视频生成模型；媒体处理工具和 Agent 能力仍在对应的消耗构成中查看，不混入模型排名，随后可切换成员与项目。消耗构成使用固定语义色并可下钻，来源维度完整保留所有排名项，不合并成“其他”。来源主列表只展示名称、比例条、占比、积分和下钻箭头；产出量留在消耗构成与详情抽屉，避免重复信息。来源列表过长时仅列表区独立滚动，表头在列表内吸顶并与数值共用列宽；模型、成员与项目三个维度均固定提供低干扰搜索图标，用户主动点击后搜索框向左展开。进入积分管理的入口位于消耗来源列表末尾，不与期间筛选和导出争抢顶部操作区。任务级扣费、退款与成员流水统一回到积分管理下钻，不在用量页复制完整流水表。内部分配不计入组织消耗；在 `GenerationTask`、计费快照与不可变 `CreditLedger` 接入前，页面不得把演示数据描述为真实组织账本。生产形态下的看板数据应是同一不可变流水按查询时刻动态聚合出的统计快照，而非长期冻结的静态副本；余额与流水近实时更新，图表聚合可允许约 5 分钟延迟。
  - 期间分析显式保留“今天 / 近 7 天 / 近 30 天 / 日期范围”四个入口，默认使用近 30 天。三个固定范围立即应用且控件宽度稳定；日期范围确认浮层先呈现可编辑的开始与结束日期，再把“本月 / 上月 / 全部历史”作为低层级快捷填充置于其后，避免将快捷项误读为顶部页签。快捷项只预填并标记范围，开始与结束日期始终可继续编辑，修改任一日期即转为自定义范围。日期编辑只改变草稿，点击“应用”后才更新数据，取消、点击外部或 Escape 均不修改已应用统计。手动范围首尾日期都计入统计，并在再次打开时保留；日期入口按最终选择显示本月、上月、全部历史或 `MM/DD–MM/DD`，数据区同时显示实际统计日期。比较口径分别为昨日同期、前 7 天、前 30 天、上月同期、上上月和前一等长周期；全部历史不显示无意义环比。所有自然日边界按组织时区 `Asia/Shanghai` 表达。
  - `长期活动` 独立于期间分析，可按 365 天窗口向前翻阅，并在每日热力图、52 周柱状趋势与周累计趋势之间切换；三种视图共用底部月份轴。每日单元格在宽屏保持接近正方形，并在较窄绘图区适度增高以减少无效留白；每周悬停仅呈现周日期范围与积分消耗；累计视图的整周水平区块都可触发预览，并使用贯穿绘图区的竖向游标、累计值和当周增量。演示账本包含工作日 / 周末差异、活动高峰、项目间歇、退款和近期增长，不使用匀速增长或今日与近 7 天相同的假数据。
- 组织中心左侧导航已经承担当前分区名称，组织管理、积分管理和用量看板内容区不再重复显示同名大标题，但保留可访问标题。`长期活动` 的每日、每周与累计视图共用同一套 `53 周 × 7 天` 响应式绘图区、周列中心、稳定图高和月份轴：每日显示日单元格，每周在同一列宽内显示周消耗柱，累计以周为离散节点连接累计消耗并显示当周增量。首尾悬停提示不得越出图表；稳定概览独立放在用量页首部，不与长期活动模式绑定。
- 用量看板的“预计可用”以近 30 天趋势作为唯一主估算；近 30 天日均与历史日均只作为解释性参考，不生成第二个预计天数，也不随期间分析筛选变化。估算保留零消耗日，不把少量活跃日误当成连续日均。消耗构成使用“图片生成 / 视频生成 / 媒体处理 / Agent 处理”的稳定业务分类，其中“媒体处理”承载高清化、去字幕、提升帧率等可扩展工具，不被限定为编辑增强。类型图例与消耗来源使用不同下钻语义：类型图例展开该业务类型下的“模型或处理工具 → 聚合规格”；模型来源以计费规格为主分析，再在同一个“关联使用”区切换主要使用成员与关联项目；成员来源以消耗构成为主分析，再切换使用模型与关联项目；项目来源以消耗构成为主分析，再切换使用模型与参与成员。项目来源排行和项目抽屉默认严格继承期间分析的已应用时间范围，抽屉标题区始终明示当前统计范围；“项目全周期”作为克制的独立入口切换到从项目首次有效用量至今的汇总，并可返回当前期间，不把全周期数字混入本期排行。关联维度不并排堆叠多张排名表，也不继续进入二级抽屉；抽屉保持单一纵向滚动，并只呈现当前选中维度的名称、占比和积分。模型明细只按影响扣费的维度聚合：视频保留分辨率并在产出列累计总秒数，普通图片保留分辨率并在产出列累计张数，GPT Image 2 保留质量与输出尺寸；画面比例、批量张数和单次视频时长不占据聚合规格列。任务数只作为类型抽屉顶部的整体规模摘要，模型来源顶部改用计费规格数量，成员和项目来源分别使用活跃项目与参与成员数量；完整单次配置继续留在任务流水。抽屉不直接复制任务流水，也不替代组织账本。
- 组织中心锁定外层页面滚动，“返回｜组织中心”和左侧导航保持在视口内，右侧管理内容在面板内部独立滚动；成员或流水增多时不会把页面入口推到顶部遮挡区域。
