...
- 返回主页使用独立箭头，经宿主在保存收尾后执行 React 路由。项目 chevron 只标记为“项目菜单”，当前菜单只提供“查看全部项目”；iframe 尚未获得授权后的项目列表投影，因此不伪装成真实项目切换器，也不再提供画布内的项目创建或删除假持久化操作。
...
- 资产库展开时，画布自身左上角导航隐藏，只保留资产库面板顶部的同一套返回主页、项目名与项目菜单，并在右侧提供内部画布菜单和收起按钮，避免出现两套项目入口。
...
- 本地上传会把素材加入生成节点上方素材列。
- 从资产库添加会打开与左侧中部资产库入口共用的项目素材面板；选择后加入当前生成节点的素材列。
...
左侧中部胶囊中的资产库是画布媒体输入与画布内容导航的统一入口。展开后默认是当前项目侧栏，而不是直接进入全局资产库。面板默认宽度加宽到更适合列表阅读的尺寸，并可通过右边缘拖拽调整。
