...
- `login.html` 继续保留静态视觉参考，不创建会话；正式账号、密码重置、风控和成员管理不得直接在固定演示账号上扩写。
...
- 鼠标进入现有项目卡或键盘焦点进入卡片时，封面右上角显示三点菜单；`admin/edit` 可见重命名与修改封面，只有 `admin` 可见转为协作项目与删除，`view` 只可打开。信息区对可编辑成员显示快捷编辑笔。新建与重命名写入 PostgreSQL；修改封面、转换成员关系和删除仍只显示未接入提示。
- 五个账号都属于 `Reelay 创作组`。个人项目只对创建者可见；协作项目只对显式加入的组织成员可见，项目角色为 `admin/edit/view`。组织 Membership 只证明账号属于组织，不能替代项目权限。浅色 / 深色仍是设备级偏好。
