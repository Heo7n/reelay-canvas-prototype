...
- 页面明确标记为“受邀演示环境”，使用明显虚构的 `.test` 演示账号；主账户、管理员和成员三种快捷身份可直接填入对应账号与固定演示密码，并持续提醒不得输入真实凭据。服务端校验固定演示密码并创建 HttpOnly 会话 Cookie。
- 未登录访问工作空间或项目深链时会进入登录页；登录后只恢复当前账号有权访问的 `/app/w/...` 目标，外部或越权返回地址会回退到所属组织主页。
- 注册、忘记密码、短信验证和第三方登录均未接入；登录标题不再把登录误写成已经支持注册，忘记密码、Google 登录、用户协议与隐私政策以明确的“暂未开放”状态展示。
- 注册入口切换到同一路由的 `?mode=register` 说明态，并保留可信的 `returnTo` 参数；该状态明确说明当前仅提供受邀演示账号，不渲染假注册表单，也不会创建账号、发送验证码或收集联系方式，可直接返回演示登录。
- 左侧使用用户确认的白色环形空间与 Reelay 机器人视觉，右缘通过页面层的浅色渐变和细颗粒带过渡到登录栏，Logo 仍由真实页面元素呈现；浅色 / 深色沿用 `reelay-theme-mode`。
- 右侧以账户密码登录作为唯一可提交主流程；密码显隐、提交中锁定、服务错误提示和字段可访问状态均由页面行为测试覆盖，未接入能力不伪装成可提交操作。
...
- Reelay 品牌字标、头像账户入口与工作台一级导航；“创作首页 / 项目空间”在主页和项目库之间提供稳定的当前页标识。工作台顶部不再单独暴露积分按钮，`3000 / 0` 模拟积分显示在头像展开的账户面板中。
- 主页首屏先展示明确的创作标题、创作意图输入框和按“策划 / 生成 / 制作”分组的快捷能力，再展示最近项目；三张创作主题卡下移到“创作灵感”分区。轮播仍每 4 秒自动切换，使用中间主卡聚焦、两侧卡片展开后退的空间层级；手动切换后重新计时，指针悬停、键盘焦点进入、页面不可见或系统启用减少动效时暂停。
- 创作意图与空白新建使用两个独立 action intent，并共享项目创建忙碌状态：任一入口正在创建时，另一入口同步禁用并拒绝提交，避免并发创建和导航竞态。只有非空意图可以进入创作启动 action；服务端成功创建项目后，页面才把本次提交写成带 `version / workspaceId / projectId / prompt` 的单次 `sessionStorage` 交接并导航到对应画布。旧画布只在宿主下发的工作空间与项目身份都匹配时消费；非法、旧格式或 scope 不匹配的交接会立即清除且不创建节点。空白 Enter、重复提交或创建失败都不会创建意外项目，也不会把陈旧提示词留给之后打开的其他项目。
- 最近项目区固定以“新建项目”作为首张卡，并明确它创建的是仅自己可见的个人项目；后接四张从当前账号可访问工作空间按更新时间混排的项目，不在该区域再次分栏。协作项目在最近项目和项目库中都以右下角多人图标标识。项目卡文字固定为项目名称和最近编辑时间两行。
- `/app/w/:workspaceId/projects` 是“项目空间”常规路由，默认展示当前账号有权访问的全部个人与协作项目，并提供带数量的“全部 / 个人 / 协作”筛选。搜索词和筛选类型保存在 URL 查询参数中；协作筛选不会展示会创建个人项目的新建卡，无结果时区分搜索无匹配、尚无个人项目和尚未被加入协作项目三种说明。
- 鼠标进入现有项目卡或键盘焦点进入卡片时，封面右上角显示三点菜单；同一时间只展开一个项目菜单，点击外部、焦点移出或按 `Esc` 都会收起。`admin/edit` 可见重命名与修改封面，删除始终只对项目级 `admin` 开放，`edit` 会看到禁用态及权限提示，`view` 只可打开。只有个人项目的 `admin` 可见转为协作项目。重命名会先 trim 草稿，未变化不提交、空值保留编辑并显示错误、请求中锁定，只有服务端成功后才退出编辑；失败会保留草稿和可见反馈。新建、重命名和删除写入 PostgreSQL；删除前必须二次确认，删除后项目立即从相关成员的列表和画布入口隐藏，但保留项目、成员关系与 CanvasDocument，等待后续回收站恢复。修改封面和转为协作项目仍是明确的未接入提示。
...
- 空白新建卡创建项目后直接进入 `/app/w/:workspaceId/projects/:projectId/canvases/main`；创作意图入口则在项目创建成功后写入绑定目标 `workspaceId + projectId` 的版本化一次性交接，再进入同一路由。宿主会先校验会话、工作空间和项目，再以 iframe 打开旧画布并通过一次 `canvas:ready` 握手发送版本化上下文与 CanvasDocument；旧画布取得该权威上下文后才校验并消费主页交接，其他项目不会穿透消费遗留提示词。旧画布按 `projectId + canvasId` 自动保存迁移快照。新项目尚无服务端文档时，纯浏览只把当前空画布登记为内存同步基线，不创建空的数据库记录；主页提示词形成的首个节点或后续真实用户修改才会写入 revision 1。
