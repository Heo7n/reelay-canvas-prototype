...
- 默认目录计为第 1 层，总计最多 5 层；只展示直属子目录和条目，父级与同级通过目录树切换。添加目录沿用既有内存目录的唯一名称、行内改名、层级与权限校验；本轮没有新增统一目录持久化或迁移旧 placement。
...
- `个人空间` 与 `组织空间` 在当前前端原型中可编辑；`平台空间` 是扁平的只读灵感搜索结果，只展示 Media，不显示目录、上传、新建、素材组入口或单条修改菜单。平台结果仍可搜索、筛选、切换视图、预览和进入多选；多选只提供“添加到画布”和“保存到素材”。前者会从结构化 Media 投影创建真实画布节点；后者在后端受控 import、来源授权与个人 placement 写入命令落地前保持禁用，不能用页面内存 placement 冒充已保存。该差异只用于验证交互，不代表服务端组织权限已经完成。
...
- 混排列表的主操作是多文件上传，接受图片、视频和音频。在受保护的 React 画布宿主中，个人空间上传会创建带 checksum 的 upload intent，写入 ObjectStore，完成 WorkspaceMediaAsset 登记并幂等挂到当前项目。持久 placement 当前只是 personal root，UI 子目录尚不持久；宿主内非个人空间上传会明确拒绝，直接打开静态画布时才使用页面内存媒体。
...
- 多选操作中的“新建素材组”进入独立编辑工作区；组卡菜单及组内编辑按钮也进入同一编辑器；新建时标题固定为“新建素材组”，编辑既有记录时标题跟随素材组名称。左侧编辑栏跟随资产库的当前首选宽度；名称、描述、“添加素材”标题及其下由素材分类和“从素材库添加 / 上传”组成的工具行固定，只让工具行以下的素材卡片区独立滚动。编辑器包含该素材组已添加素材的 `全部 / 图片 / 视频 / 音频` 四类筛选、从个人素材库添加、上传、单项预览与移除；只有图片可以设为封面。预览头部左侧以单行的媒体类型图标和文件名称标识当前素材；双击文件名称或按 `F2` 可原位重命名，输入只修改扩展名前的主文件名，扩展名固定保留，修改直接写回同一 WorkspaceMediaAsset，不在 Entity 内复制名称。右上角使用约 `80 × 30px` 的纯文字圆角按钮显示可执行的“设为封面”，已为封面时原位替换成同尺寸、非交互的“当前封面”状态；不使用旧书签图标。左侧对应素材卡片常显轻量“封面”标记。四类筛选只改变当前素材组素材的显示，不是全局资产筛选；素材组至少引用一个 Media 才能保存。
...
- 默认目录计为第 1 层，总计最多 5 层。现有目录仍保留内部 kind 和 placement 边界；统一混排只改变读取与导航，不提供跨旧目录类型的统一写入。文件夹重命名、移动继续校验同级唯一、同空间、无循环及深度上限。目录持久化仍未实现。
- 文件夹移动仍使用目录树选择目标，保留当前目录高亮与内部滚动。包含素材组的目录共享不可绕过素材组的能力限制；旧内存 placement 行为不代表组织共享已持久化。
- 卡片三点菜单在浏览器顶层靠触发按钮展开，按视口避让；滚动跟随，按钮离开列表或关闭库时收起。单 Media 保留适用的重命名、移动、组织复制、删除和审核原型；组卡只显示查看内容、编辑素材组和重命名，不显示审核、移动、共享、删除。持久 Entity 编辑与重命名沿用版本检查；没有开放新的持久命令。
- 多选只作用于当前可见条目，以 `${kind}:${id}` 保存选择，避免 Media 与 Entity 同 ID 相互污染。卡片悬停或键盘聚焦露出复选框，点击进入多选；多选时常显，触摸设备保留入口。全选、取消和上下文清理作用于网格。批量“添加到画布”及“新建素材组”复用当前空间的有序 Media 引用解析并去重，再校验目标和权限；包含组的选择不提供审核、移动、共享、删除，避免把组操作偷换为其内部媒体操作。
...
- 可共享的单 Media 只新增 organization placement，不复制媒体；文件夹共享沿用既有类型及子树权限限制。本实验不通过文件夹路径开放素材组的共享或统一目录持久化。
- 单 Media 与可操作文件夹的删除仍需确认，并按现有规则移除当前空间 placement；仍被当前空间素材组引用的 Media 不能单独移除。素材组删除本轮不开放，不能通过混排批量操作解除其引用或删除底层媒体。当前没有回收站、恢复和永久删除链路。
