...
- 左下角是收起态约 `248 × 40px` 的视口工具胶囊，依次提供小地图、适应视图、暂不可用的整理画布和 20%–200% 缩放滑条。滚轮位于整个工具胶囊及其空白内边距时都不会带动画布；项目条、资产面板和工具条共享 `8px` 外边距与 `4px` 面板沟槽，构成更紧凑的左侧纵向基线。项目条与工具条仍保持等宽和稳定命中区，压缩量主要来自外部留白与容器内边距，使中央资产 / 主体工作面获得更多有效高度。
