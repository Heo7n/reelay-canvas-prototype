...
`login.html` 用于验证国内首期账户密码登录的页面结构与进入工作台的路径，不是已完成的账号系统：

- 使用明显虚构的 `.test` 演示账号并默认填充，提交非空账号和密码后进入 `home.html`。
- 不校验固定密码，不创建 Token、Cookie 或本地登录状态，也不会上传或保存输入内容。
...
- 正式账号、组织成员和权限开发前，必须先确定生产运行时、路由、后端会话与数据库方案，不能在这套静态表单上继续堆叠鉴权逻辑。

### 2.2 登录后主页原型

`home.html` 已实现为与画布隔离的登录后创作主页原型，不是公开营销落地页。它当前包含：

- Reelay 品牌字标、`3000` 模拟积分与头像账户入口；头像旁不使用额外下拉箭头。
- 三张可切换的创作主题卡：AI 分镜与脚本协作、视频创作工作流、项目素材统一组织。
- 创作意图输入框与能力快捷项；提交非空意图后进入画布，并通过单次 `sessionStorage` 交接在当前画布中心创建已填写提示词的生成节点。
- 最近项目区固定以“新建项目”作为首张卡，后接四张按最近时间混排的个人与协作项目示例，不在该区域再次分栏；协作项目在最近项目和项目库中都以右下角多人图标标识。项目卡文字固定为两行：项目名称和最近编辑时间，不混入生成状态、画布数等其他信息。通过 `#all-projects` 即时切换的常规“全部项目”页面状态支持本地搜索与“个人 / 协作项目”文字标签筛选，当前标签仅用下划线标记。两个项目分类也都固定以“新建项目”作为首张卡，并混合少量本地封面和轻量语义卡。鼠标进入现有项目卡或键盘焦点进入卡片时，封面右上角显示三点菜单，包含打开、重命名、修改封面、转为协作项目、删除项目，不提供“移动至文件夹”；鼠标进入封面下方信息区时，项目名右侧显示快捷编辑笔。重命名支持当前页面生命周期内的内联演示，刷新后重置；修改封面、转为协作项目和删除仍只显示未接入提示。项目卡仍统一进入当前内存画布原型，不代表项目已经持久化或可按项目 id 加载。
- 个人空间与组织空间的切换演示、浅色 / 深色切换；组织切换只验证账户菜单信息结构，不代表真实成员、权限或协作已接入。
- 桌面三卡布局、平板单主卡布局与移动端横向项目卡片，支持键盘焦点和 `prefers-reduced-motion`；高度超过 `1050px` 的桌面视口会适度增加顶部留白、输入区高度和区块间距，普通笔记本高度保持紧凑节奏。

主页与画布的边界：

- 主页逻辑位于 `src/home/index.js`，配置位于 `src/config/home-prototype-config.js`，样式位于 `styles/home.css`，不进入 `app.js` 或 `styles/app.css`。
- 画布项目菜单的“回到主页”进入 `home.html`，“全部项目”进入 `home.html#all-projects`；后者在共享顶部栏下即时切换为常规页面状态，但仍不是正式应用路由。
- 模拟积分仍没有共享账本；两个页面刷新后都显示当前阶段的 `3000 / 0` 测试基线。
...
资产库展开后的收起按钮位于面板左下角，数量统计位于右下角；在 320px 宽度下两者仍保持在视口内。
在 `480px` 及以下视口中，顶部操作区保留在第一行，项目 / 画布导航进入第二行，避免分享、积分、头像与导航互相覆盖；快捷键说明改在个人菜单内展开并限制在视口范围内。
...
- 当前阶段没有真实项目路由和持久化；回到主页进入独立主页原型，全部项目进入共享顶部栏下的项目库页面状态，创建/删除项目会通过确认弹窗重置当前原型状态。
...
- 在 `480px` 及以下视口中，资产库作为全屏面板打开：画布、顶部操作区、左侧入口、Agent 和撤销提示退出键盘与读屏访问，焦点进入资产库搜索框或关闭按钮。资产库自己的项目 / 画布菜单仍保持可用；关闭或回到更宽视口后恢复背景访问。桌面宽度下资产库和其他侧栏仍可并行操作。
...
- 未锁定节点的图片、视频模型位于同一个连续滚动列表中，并用分组标题区分；锁定后保留两个类型标签作为状态提示，另一类型不可用，列表只展示当前类型模型，并显示“已生成图片/视频，此节点仅可继续使用图片/视频模型”。生成节点与 Agent 中所有模型选择入口统一使用线框立方体图标，不再混用供应商字符或闪光图标。
...
```text
index.html
  定义页面骨架、工具栏、画布层、Agent 对话栏、隐藏文件输入和脚本加载顺序。

login.html
  定义账户密码流程样机、默认演示账号和进入创作主页的静态入口。

home.html
  定义登录后主页骨架、轮播、创作输入、能力入口、账户菜单、最近项目区和项目库页面状态。
...
styles/login.css
  独立定义登录流程样机的品牌视觉、表单、浅深主题与响应式行为。

styles/home.css
  独立定义主页的浅深主题、三卡主视觉、创作入口、项目卡、项目库页面状态和响应式行为。
...
src/config/home-prototype-config.js
  保存主页轮播、能力项、全部项目与最近项目派生 mock 数据。

src/login/index.js
  处理演示账号表单、密码显隐、主题读取和进入主页；不得扩展为前端伪鉴权。

src/home/index.js
  处理主页轮播、创作意图交接、项目库页面状态、账户菜单、工作空间演示和主题切换。
...
Reelay 当前是一个由登录后创作主页与无限画布组成的 AIGC 创作平台前端原型：用户可以从主页继续示例项目或带着创作意图进入画布，在画布中创建生成节点、组织图片/视频/音频素材、模拟生成结果，并通过右侧 Agent 对话栏获得创作辅助。
