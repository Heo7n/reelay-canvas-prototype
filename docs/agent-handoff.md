...
- 后续公网更新从验收后的集成主线发布，发布前单独核对部署范围和环境。2026-09-05 核验公网生产部署为 `dpl_3e9rT3hKtg6p6XuKHKnxZdfF9hMv`，记录的代码 SHA 为 `ed5b4012b658e33652d7b4a2aa9ac80141a8230c`；项目尚未连接 Git 自动部署。当前用户只授权远端代码同步，暂不部署，不能根据本地或远端 `main` 推断线上版本。
...
2026-09-07 阅读位置修复：高级设置切换原先因节点签名变化而重建整块 DOM，导致长提示词回到开头。reconciler 现在允许同一 live node 就地更新；`canvas-node-prompt-view.js` 保留仍展开的编辑器外壳和 textarea，未变化的媒体也不重建，文字仅在真实内容变化时赋值。高度测量使用非交互副本，避免临时压缩原输入框。节点根与正文监听只绑定一次，新控件独立绑定，脱离文档的布局回调停止；同 ID 的新节点对象不继承原编辑器。定向 58 项通过；真实浏览器已验证深浅主题 3000 字提示词滚到 1578px 后连续四次开关高级设置和切换参数，正文高度、滚动位置、内容及画布位置不变，输入框与媒体 DOM 身份保留。完整 `npm run check` 的 604 项测试、`npm run build` 与 `git diff --check` 通过，浏览器控制台无 error / warn。

提交与同步核验：本 worktree 与 `D:\Software\codePro\0707` 的 `codex/canvas-development` 起点均为 `c6dd68a`。本轮节点视觉与交互改动独立整理；主目录另有未提交的模型、Agent 与高级设置改动，双方涉及 12 个相同文件。主目录已将高级设置统一为单一 `assetValidationEnabled` 并把视频高级区从 154 改为 118，集成时按最终内容保留该高度，并同时保留本轮正文状态与缩放规则。测试列表、缓存版本和产品文档需合并，不能整文件覆盖；主目录先形成独立提交后再由一个任务集成验证。本轮不自动同步其他任务的工作区，也不修改共享 API、数据库、ObjectStore 或公网部署。

2026-09-07 节点滚轮补充：修复节点展开后提示词面板落到鼠标下方、导致连续滚轮平移被整块拦截的问题。生成节点编辑器空白、未聚焦正文和无溢出的聚焦短文本现在允许普通滚轮平移画布；已聚焦且溢出的正文保留内部滚动，到顶部或底部也不链出。正文与空白中的 `Ctrl/Cmd + 滚轮` 继续缩放画布，菜单、侧栏、素材架、按钮、滑杆和具体输入控件保持本地策略，仍禁止泄漏为浏览器缩放。真正执行画布滚轮操作时退出当前提示词焦点，节点与内容保持；Escape 排除输入法组合后仅局部退出编辑，不收节点或新增保存、撤销。未聚焦正文使用普通指针，聚焦恢复文本光标，优化期间保留忙碌反馈；原有键盘编辑与 pointer 手势边界保持。此次定向真实 app 与只读行为测试 `54/54`、完整 `npm run check` 的 `603` 项测试、`npm run build` 与 `git diff --check` 均通过。隔离浏览器用真实点击、带正文坐标的滚轮及 Escape 验证 `1920` 字提示词：未聚焦时画布移动，聚焦后正文滚动且画布不动，Escape 保持展开并恢复画布导航，Ctrl 滚轮缩放且文本不变；预览控制台无 error / warn。内存试用标签已保留长提示词示例，共享项目数据未改动。

2026-09-07 当前切片：按用户补充的 Tapnow 原图重新校准节点视觉；视频编辑器在最大、最小截图中都约为 `1200 × 372px`，图片约为 `1275 × 372px`，因此撤回上一版随画布倍率收窄编辑器的假设。`canvas-node-editor-layout.js` 现在按媒体类型使用固定屏幕宽度：视频 `800px`、图片 `850px`，仅受扣除侧栏后的可用走廊减 `24px` 限制。内容高度为 `248–320px`，高级设置仍增加图片 `118px` / 视频 `154px`；编辑器圆角 `20px`，参考入口为 `48px`、`14px` 圆角、`16px` 内边距，主体入口、参考入口与素材架起点按 `68px / 64px` 递进。正文 `17px`、底栏 `16px`，只有实际走廊变窄时按容器降级，画布缩放本身不压窄输入区。

深色媒体和编辑器同为 `#1f1f1f`；媒体两主题采用 `22px` 世界圆角和 `1.5px` 世界边线，选中边线为深色 `#707070` / 浅色 `#999b9f`。名称屏幕字号为 `13px × clamp(scale, 0.85, 1.8)`，按实际宽度省略，媒体小于 `250px` 屏幕宽度时隐藏规格。媒体与编辑器的屏幕间距为 `12px × max(scale, 1)`，对应 `20%–100%` 为 `12px`、`150%` 为 `18px`、`200%` 为 `24px`。生成入口改为复合胶囊与圆形发送块，继续使用 Reelay 图标和原有功能。媒体横图 `620px`、方图 `460px`、竖图高 `520px` 的世界尺寸规则及 `20%–200%` 倍率范围保留，旧文档 `705px` 容器锚点只用于坐标兼容，不把本轮称为参考最大、最小媒体几何的完整复刻。

实际可见边界继续用于小地图和已有局部排列；适应视图仍以媒体内容为边界，不能据此声称展开的编辑器总能完整进入视口。编辑器与媒体工具栏同帧补偿，已移除 `80ms` 追赶过渡和 `100ms` 补偿 timer。仅新展开编辑器播放 `120ms` 透明度淡入，参数重绘不重播，减少动态效果时关闭；侧栏开关、宽度变化同步编辑器并重新测高，菜单按可用边界限宽、限高。释放后展开、切比例底边中心、手势和生成计费语义保持；未新增整画布自动整理或编辑器靠边自动平移。本次修订的 `npm run check` 601 项测试、`npm run build` 和 `git diff --check` 通过。视频六档倍率实测宽 `800px`、默认高 `248px` 不变且参数无溢出；图片 `850px` 宽、横竖比例切换底边锚点、浅深主题、长文本达到 `320px` 后内部滚动、视频高级设置总高 `474px`，以及 `1024px` 双侧栏走廊中的菜单和控件均复核通过，控制台无 error / warn。预览继续使用本 worktree 的 `5174`，保留完整项目路由与内存节点试用标签；共享 API / 数据边界与原任务保持一致。
