...
- `canvas-arrange-layout.js` 纯布局、`canvas-arrange-scope.js` 范围 / 分组映射、`canvas-arrange-controller.js` 菜单状态、`styles/canvas-arrange.css` 独立样式接入左下入口。默认当前选择 / 当前组内，无选择默认全画布，显式可切换；自动按连线从左到右，另有宫格 / 横排 / 竖排。混合范围按完整组刚性移动，部分组员扩选在执行前说明；仅主动选组框时整理成员并按需扩大组框，不缩小手动留白。使用含标题和已展开输入区的真实几何，范围外障碍保持原位。空范围、单单元、异常成员、权限和当前手势均有明确边界。
...
- 基于当前 `codex/canvas-agent-refinement` 的 `955c825`，在既有多选工具栏首位加入“新建主体”，与打组 / 保存到素材 / 下载共存；完成框选才显示，连接期间隐藏。修正框选结束的状态顺序：先清理 marquee action 再渲染，避免工具栏等待下一次交互才出现。
