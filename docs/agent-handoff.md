...
- 开发预览首次合适的空生成会话默认导入四条历史示例：全模态视频成功、竖图成功、多素材失败、视频取消。独立 src/dev/generation-history-presets.js 经controller/service导入冻结终态快照；不执行任务、不调用当前账户扣退、不放置画布节点，刷新余额仍3000/0。失败/取消显示历史退款，重新编辑/再次生成走正常入口；再次生成才扣费和交付。只初始化一次，已有记录或草稿、新建会话均不重复填充，生产不注入。
...
- 开发示例由 `src/dev/generation-demo-presets.js` 独立提供，5 个代表案例：纯文字方形图片、全模态横版视频、12 图多素材、单图竖版图片、单图竖版视频，覆盖横 / 竖 / 方构图；多素材 Prompt 从首句引用图片1至图片12，所有参考通过结构化文档绑定，取消原先四遍重复且仅引用图片1的文案；填充不发送、不扣费，有草稿先内联确认替换。5174 实测 12 个独立引用、0 个失效引用，新案例留在输入区。成功 / 失败统一在发送后 11 秒，前 7 秒可取消；示例输出媒体并非真实模型按提示词 / 比例生成。
