...
- migration checksum 会先把 CRLF / CR 归一为 LF，SQL 也由 `.gitattributes` 固定为 LF；迁移器可安全升级旧 CRLF ledger，但仍会拒绝语义变更。当前 schema 通过 0010 加入 WorkspaceMediaAsset、MediaAssetPlacement、upload intent 与 ProjectAssetReference，并由 0011 修正长期资产审计记录与可撤销 membership 之间的生命周期约束。
...
- 画布内资产库已按 `个人 / 组织 / 平台` 三空间重做。个人 / 组织空间提供 `素材 / 主体库`、目录、网格 / 列表、搜索、媒体筛选、预览和拖入画布；平台空间则是固定网格的单一“灵感搜索”，不显示主体、目录、上传、视图切换或条目修改菜单，只保留跨目录搜索、类型筛选、多选以及“添加到画布 / 保存到素材”。保存平台条目会在页面内存 store 中按平台源幂等导入独立个人 Media 副本，个人改名不会回写平台记录。个人 / 组织空间顶部控件高度按“分栏 > 搜索 > 目录命令行”收敛；搜索与文件夹行内编辑不显示额外黑框；“上传 / 操作”主按钮右缘与“素材”选中块右缘对齐，多选态使用 `list-checks` 批处理图标且不再叠加下拉箭头，未选条目时保持深色主按钮身份并降低整体强度来表达禁用，而不是切换成浅灰次级按钮。网格卡片固定约 `164px`，面板在 `380–780px` 间调整时按外宽约 `550px / 728px` 阈值从 `2 → 3 → 4` 列重排，不拉伸条目；右侧宽命中热区只显示居中的短浅色握柄，不再绘制贯穿面板的黑线。指针、键盘、ARIA 和 Agent 联动共用实际宽度边界，Agent 关闭后恢复资产库首选宽度；`≤480px` 全屏降级隐藏无效调整器。素材卡、主体卡和主体编辑器选择卡都把唯一选择标记放在左上角：待选态为半透明浅色块，选中态为白底深色勾；右上三点菜单随当前列数在最后一列向左翻转。主体卡只显示封面、名称和操作入口，不再重复素材数量；hover / focus 详情按当前卡片右缘近邻定位，离卡会取消待打开详情，已固定详情也不会被普通 hover 覆盖。详情保留大封面、名称 / 素材数同排、描述和整宽“添加到画布”，会把引用 Media 去重展开为独立节点、围绕可用视口排列并临时全选，不自动打组。只有 Seedance 系列视频模型显示专用“添加主体”入口；标题为“主体库”的近工作区尺度个人 / 组织多选器用“添加”确认，将 Media 去重展开进既有参考区，Kling 和图片模型不显示入口，也不留空槽。两条链路都不保留 Entity 绑定，且一次操作对应一次撤销边界。模型品牌 SVG 以本地 `<img>` 加载并对单色资产应用浅深主题滤镜，避免 `file://` 下外部 CSS mask 透明。个人空间 Media 上传现在通过 host bridge 执行 checksum 约束的 upload intent，写入 ObjectStore，登记 WorkspaceMediaAsset + personal root placement，并幂等创建当前项目的 ProjectAssetReference；项目资产可列表和读取内容，本地 PostgreSQL + filesystem ObjectStore 可跨服务重启恢复。直接创建主体时所选 Media 会先持久，但 Entity 记录、Folder、organization placement、平台目录导入、共享和审核仍由 `canvas-asset-library-model.js` 的无 DOM 内存 store 持有，刷新会重置。公网 Vercel 尚未接入私有 Supabase Storage，不能把本地 filesystem adapter 描述为云端资产服务。
...
3. 资产下一步以已落地的 WorkspaceMediaAsset + personal placement + ProjectAssetReference + ObjectStore 为基线，把目前页面内存中的 Entity 有序 Media 引用和 Folder / placement 不变量迁入独立 repository，再接组织发布、审核与正式 Node 级来源引用。不要把这些领域逻辑塞进 `CollaborationStore`、`app.js` 或 CanvasDocument；GenerationResult 只有显式保存后才幂等晋升为 WorkspaceMediaAsset。
