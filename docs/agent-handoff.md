...
- 少量参考展开区：1–2 个素材使用 112 × 84px 完整比例预览、8px 间距、靠左分类编号；取消浮层固定最小宽度，单张 / 双张随内容收紧。3 个以上保留 52px 紧凑横条与现有分页；尺寸变量和分页容量由同一布局计算提供，返回详细预览后仍保留对应布局。

- 最新视觉：输入摘要已去掉底色、边框、阴影和卡片内边距；参考摘要、下方结果与底部操作共用左边线，以 12px 内部分区及 36px 记录间距建立层级。常规参考入口为上图下统计：112px 宽、68px 高横向封面 + 4px 间距 + 18px 高统计行，图片独立 9px 圆角，入口及统计行无底色 / 无外框；Prompt 14px / 20px、最多三行，参数 13px / 22px。Prompt 与参数包在同一个自然高度容器中，间隔 6px、共用左边线，整组相对参考入口垂直居中；短提示词不再把参数推到底部。引用气泡为 17px 缩略图、12px 分类编号、20px 行高、6px 圆角。统计使用底部单行中性色图标与数字，非零类型成组靠左、按需展示；整个参考入口 hover / 键盘聚焦 / 展开时 160ms 放大至 1.035 倍，不挤动正文，减少动态效果时禁用缩放。素材横条仅等待 80ms 即展开，与放大反馈重叠；快速经过取消打开，正文及行内预览仍为 300ms。消息及完整 Prompt 的引用与正文自动补一个普通空格，尊重已有空白 / 换行，保留行首对齐且不改任务快照。节点 / Agent 输入参考区及消息 Prompt 的图片 hover 预览为无名称 / 无 X 的纯图，最大 240 × 200px、原比例完整显示、6px 细边且外框随图片收缩；从素材横条点击进入的详细预览保留返回导航及 280px 宽度，相邻记录留白 36px。媒体宽 78%、上限 560 × 340px，保留真实比例；38px 底部按钮使用克制浅蓝与中性色。素材横条标题无图标，与首个缩略图左对齐，翻页按钮并排置于右侧；无关闭按钮，翻页固定浮层并取消关闭计时，保留导航 DOM、末页宽度及 180ms 横移。独立视频播放器复用 video，中央播放、半透明控件、实际分辨率，初始静音且不自动播放，隐藏暂停、删除释放。
...
- Agent 默认 560px、范围 480–880px。参数始终与 Prompt 同列并保持一行，“任务详情”及其右侧 14px 信息图标始终完整显示，取消按宽度隐藏图标的规则。记录宽 ≤480px 使用 96px 宽、58px 高封面 + 4px 间距 + 16px 高统计行、两行 Prompt 和 12px / 20px 参数；≤390px 仍不换行，参数可横向滚动。两侧同开至少 1156px 视口并保留 280px 画布走廊，小屏按实际可用宽度降级。
...
- 定位结果按钮只在成功交付画布后显示，沿用放置模块持有的原 canvas / node 对象身份；可切回原内部画布、选中结果并适应视图。只读可浏览，删除 / 撤销、同 ID 替换、跨项目或访问失效时拒绝，不补建节点或重复扣费。
- 本轮验证：5174 最窄 480px 侧栏（扣除滚动条后记录宽 432px）四条记录的参数均为单行 20px 高、右列宽 328px，无横向溢出；模型名及“任务详情”完整显示，正文与参数间隔 6px，整组与参考入口的中心线对齐。开发预览实际显示成功 / 成功 / 失败 / 取消四条记录；host 文档就绪会触发 controller 渲染，加载中、错误项目及只读状态不提前导入。已验证参考分页不关闭与末页宽度、视频播放 / seek / 静音 / 全屏、跨画布归属与退款幂等；浅色 / 深色与详情入口复核通过。最终 npm run check 共 1141 项通过（827 legacy / 187 shell / 123 server / 4 setup），npm run build 与 git diff --check 通过。5174 已验证单张 / 双张展开宽度 138px / 258px、横竖图完整比例、浅色 / 深色及多素材翻页保持展开；静态画布产物已更新。本批次作为本地提交交付，尚未推送或发布。
