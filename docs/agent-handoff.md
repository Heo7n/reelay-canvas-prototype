...
- legacy 画布壳层已重排为四个独立浮动区域：左上返回主页 / 项目名 / 项目菜单，左侧中部常显内部画布 / 资产 / 分享 / 个人，左下小地图 / 适应视图 / 禁用的整理占位 / 缩放滑条，右上只保留 Agent。积分已从右上常驻徽标迁入个人菜单，并通过严格 `canvas:open-account` 的 `profile | credits` bridge 打开 React 对应分栏；旧无 `section` 消息兼容为 `profile`。个人入口只在 hover 时显示文字提示，点击才展开菜单；菜单以触发头像和身份头像的中心线为固定锚点，帮助子层只向下扩展，展开后隐藏入口文字提示。项目名编辑态只使用 hover 同款浅色底框，不额外显示描边、下划线或阴影。当前项目菜单只跳转全部项目，不提供虚假的 iframe 内项目列表、创建或删除；真实项目切换仍应由 React host 提供授权投影。资产面板头部复用同一项目导航并保留内部画布入口。该切片的壳层覆盖集中在 `styles/canvas-chrome.css`，不要把它与 `styles/app.css` 中的组框 / 临时选择面规则重新耦合。
