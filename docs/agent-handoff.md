...
- 已通过 `npm run check`、`npm run build` 和 `git diff --check`；后续短窄屏修整补验 CSS 与 6 项 view 测试。5174 实测浅 / 深主题、9 项混合引用的完整保留与媒体预览、手动填入和撤销，控制台无 error / warn；独立浏览器 390 × 600 验证长文、展开高级配置及确认状态，正文可滚动、底部操作可访问。当前仅本地实现，沿用 5174；没有真实优化模型调用，不新增积分消费、后台持久化或公网发布。
