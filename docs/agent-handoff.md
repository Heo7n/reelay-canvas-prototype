...
- 规范与遗留清理已提交为本地 `5c13646`。后续接入独立 Playwright 验收：登录、画布 HTTP 保存 / 刷新读回、生成定位、取消退款、优化填入 / 撤销共四条用例通过，包含引用保留与控制台 / 网络检查；完整 `npm run check` 的 1687 项测试及 E2E 类型检查通过。测试使用自动分配端口的内存服务，模拟外部媒体只读仓库样本，不连接共享数据。
- 视觉微调用 `check:visual` 加页面验收；相关核心行为改动运行 `test:e2e`，已构建同一代码时用 `test:e2e:run`。浏览器测试独立于日常完整检查，在 CI 单独执行。当前新增检查配置尚未推送或发布，公网仍为上述 PR #29 的最近核验版本。
