...
- 账号设置是 React 弹出面板，包含个人主页、积分记录、用量看板。可选联系邮箱与手机号通过 PostgreSQL 持久化，但它们不是登录标识、未做验证，也不会自动订阅报表。积分与用量目前保持明确空状态，不可把 `3000 / 0` 扩写成真实账本。
- 头像菜单中的组织入口已改为独立 Workspace 路由。`/app/w/:workspaceId/organization` 把精简组织信息与真实只读成员列表放在同一页，组织资料和角色列支持明确标注的本页预览交互；`/organization/credits` 展示“累计入账 − 已分配 = 未分配”的积分演示口径与详情抽屉，`/organization/usage` 保持任务和账本未接入时的真实空状态。组织资料、角色、凭证和会话操作目前都不会写入共享组织、账号或 Membership；成员现有密码永远不进入页面或 API。
