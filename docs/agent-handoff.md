...
- 消息 Prompt 的行内媒体预览以当前引用块为锚点居中，限制在消息横向边界内，上方不足时向下避让；素材总览仍沿消息左边线展开。完整 Prompt 阅读面板取消固定 430px 宽度，外框与消息内容区等宽并随侧栏缩放，保留滚动位置；5174 已核验 480 / 680px 侧栏对应 432 / 632px 阅读面板。
