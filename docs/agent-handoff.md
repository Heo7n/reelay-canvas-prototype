...
- `app.js` 与 `CanvasHost.tsx` 仍有集中职责，按功能边界渐进提取；节点结构及高频移动撤销尚未全部迁入内容命令，不宣称已完成整套画布迁移。颜色、字重和模态组件也尚未全站统一；下一轮视觉方向、密度、图标与删除 / 恢复讨论集中在[设计规范第 7 节](design-system.md#7-下一轮讨论与落定)，提案不视作已确认或已实现。
