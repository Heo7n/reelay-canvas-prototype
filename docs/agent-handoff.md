...
下一切片让旧画布消费 `host:init`，通过明确的 `CanvasDocument` API 加载 / 保存当前项目画布；CanvasDocument 使用独立 migration 和 repository，不把 `app.js` 全量内存状态直接当成数据库领域对象。保存权限复用 ProjectMembership：`admin/edit` 可写、`view` 只读。完成真实路由画布持久化后，再删除重复的静态登录与主页入口。
