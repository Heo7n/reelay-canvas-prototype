...
- legacy 画布壳层已重排为四个独立浮动区域：左上返回主页 / 项目名 / 项目选择，左侧中部常显内部画布 / 资产 / 分享 / 个人，左下小地图 / 适应视图 / 禁用的整理占位 / 缩放滑条，右上只保留 Agent。积分已从右上常驻徽标迁入个人菜单，并通过严格 `canvas:open-account` 的 `profile | credits` bridge 打开 React 对应分栏；旧无 `section` 消息兼容为 `profile`。个人入口只在 hover 时显示文字提示，点击才展开菜单；菜单与左侧胶囊外框底边对齐，前两项为“我的积分 / 组织中心”，退出账号使用中性色，帮助中心与快捷键分别向右侧分层展开而不改变主菜单高度。快捷键三级卡片使用 `246px` 紧凑单列布局，不重复标题，按“画布操作 / 节点操作”分组，并以实际画布绑定为唯一文案来源；其中移动视图明确展示中键或空格配合拖动。项目名编辑态只使用 hover 同款浅色底框，不额外显示描边、下划线或阴影。项目选择器由 React host 传入当前账号授权项目投影，支持搜索、缩略图、当前项勾选和固定的新建项目入口；打开其他项目与新建项目意图都会先等待当前画布保存，再由宿主路由或现有 workspace action 执行，iframe 不持有 repository 或伪造项目。资产面板改为独立的“返回画布 / 资产库 / 空间切换”头部，不再复制项目导航。画布壳层覆盖集中在 `styles/canvas-chrome.css`，资产库独立样式位于 `styles/canvas-asset-library.css`；两者都不要与 `styles/app.css` 中的组框 / 临时选择面规则重新耦合。
- 画布内资产库已经按 `个人 / 组织 / 平台` 三空间和 `素材 / 主体库` 两分栏重做。Media、Entity、Folder 与 Placement 由 `canvas-asset-library-model.js` 的无 DOM 内存 store 统一持有；Entity 只保存去重、有序的 Media 引用，直接创建主体会先登记新 Media，跨空间共享只增加 placement。目录把默认目录计为第 1 层，其下最多 4 层自定义文件夹；目录树弹层支持展开 / 收起、当前目录高亮和长列表内部滚动。个人与组织空间可在原型内上传、创建子目录、重命名、移动、多选、提交内存审核、递归删除文件夹；个人还可把文件夹子树递归共享到组织。模型会拒绝目录循环、跨空间 / 跨分栏、同级重名和深度溢出；音频素材单项及任何包含音频素材的批量选择不提供 Seedance 合规审核。平台空间只读。网格 / 列表、当前目录搜索、图片 / 视频 / 音频筛选、媒体 / 主体预览和媒体拖入画布已经接通。该切片仍会在刷新时重置，组织权限、审核服务、ObjectStore、Workspace Asset repository、正式 AssetReference 和整个主体加入画布都没有实现；不能把 seed 与前端可变性描述为真实云端资产权限。
...
3. 资产下一步不要继续扩写画布内存交互，而应从 Workspace Asset、ProjectAssetReference 与 ObjectStore 的最小纵向故事开始：上传 → 加入项目 → 刷新 / 服务重启后仍存在。GenerationResult 只有显式保存后才幂等晋升为 Asset；现有 Media / Entity / Folder / Placement store 是可替换的原型契约，不是持久 repository。
