...
- 头像菜单中的组织入口已改为独立 Workspace 路由。`/app/w/:workspaceId/organization` 把精简组织信息与真实只读成员列表放在同一页，组织资料和角色列支持明确标注的本页预览交互；`/organization/credits` 展示组织余额、成员额度以及入账 / 分配 / 消耗明细；`/organization/usage` 仅对主账户与管理员展示确定性前端演示看板。用量页首行并列窄幅概览与宽幅长期活动，下一整行承载期间分析；概览按可用积分、双列日均参考、整行预计可用的纵向结构组织，低对比度层叠图形仅作语义提示。期间分析不再重复独立的产出摘要，先以整行且随容器宽度自适应的消耗走势建立时间变化主线，再在下方并列消耗构成与模型 / 成员 / 项目消耗来源；构成区采用上方环形图、下方双列图例，并以“媒体处理”统摄高清化、去字幕、提升帧率等可扩展工具。构成和来源共用同一套分析抽屉，直接展开模型或处理工具及其聚合规格子行，横向呈现产出、积分和消耗占比，不把聚合查看拆成多层点击页面；明细只保留影响扣费的聚合维度，视频按分辨率汇总总秒数，普通图片按分辨率汇总张数，GPT Image 2 按质量与输出尺寸汇总，任务数仅作为顶部规模摘要。进入积分管理的轻量入口置于来源列表末尾。默认近 30 天，固定预设与需要确认的日期范围共用同一账本聚合结果，走势、构成、成员 / 项目 / 模型排名和导出不会各自维护范围。日期面板先显示起止日期，再显示本月、上月、全部历史快捷填充，并保留已应用选择；全部历史不做无意义环比。独立的 `长期活动` 可向前翻阅，在每日 / 每周 / 累计三种视图之间切换；每日格子在宽屏接近正方形、在较窄绘图区适度增高，每周提示只显示范围和消耗，累计模式按整周水平区块响应并显示贯穿图高的竖向游标。演示数据包含高峰、间歇、周末差异与退款；界面不再重复显示“演示数据”或更新时间提示，但文档继续明确其演示边界。任务级流水不在用量页重复实现，而由积分管理承接。组织用量数据位于独立数据层，后续应以 `GenerationTask`、计费快照和不可变 `CreditLedger` 替换，不能把当前演示记录当成真实账本。组织资料、角色、凭证和会话操作目前都不会写入共享组织、账号或 Membership；成员现有密码永远不进入页面或 API。
- 当前组织中心作为 `w/:workspaceId` 的子路由复用工作台已经加载的 `WorkspaceContext`，首次进入只额外读取组织成员。主账户与管理员可访问组织信息、积分管理和用量看板，普通成员只显示只读“组织信息”，可查看组织成员、账号邮箱与角色；直接访问组织积分或用量地址会返回组织信息。三个分区切换不重复请求工作区和项目数据，也不在内容区重复显示左侧导航已经表达的大标题，但保留隐藏的语义标题。用量页导出入口位于时间切换左侧；消耗构成分析抽屉同时呈现任务数量、产出、积分和占比。用量页稳定概览位于首部；每日 / 每周 / 累计统一使用 `53 周 × 7 天` 绘图区、固定高度、周列中心和月份轴，切换模式不得引发布局跳动。
