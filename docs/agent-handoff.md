...
- legacy 画布壳层已重排为四个独立浮动区域：左上返回主页 / 项目名 / 项目菜单，左侧中部内部画布 / 资产 / 个人与向上 reveal 的分享，左下小地图 / 适应视图 / 禁用的整理占位 / 缩放滑条，右上只保留 Agent。积分已从右上常驻徽标迁入个人菜单，并通过严格 `canvas:open-account` 的 `profile | credits` bridge 打开 React 对应分栏；旧无 `section` 消息兼容为 `profile`。当前项目菜单只跳转全部项目，不提供虚假的 iframe 内项目列表、创建或删除；真实项目切换仍应由 React host 提供授权投影。资产面板头部复用同一项目导航并保留内部画布入口。该切片的壳层覆盖集中在 `styles/canvas-chrome.css`，不要把它与 `styles/app.css` 中的组框 / 临时选择面规则重新耦合。
