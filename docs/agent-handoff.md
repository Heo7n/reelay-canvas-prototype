...
- 头像菜单中的组织入口已改为独立 Workspace 路由。`/app/w/:workspaceId/organization` 把精简组织信息与真实只读成员列表放在同一页，组织资料和角色列支持明确标注的本页预览交互；`/organization/credits` 展示组织余额、成员额度以及入账 / 分配 / 消耗明细；`/organization/usage` 仅对主账户与管理员展示确定性前端演示看板。稳定概览只保留组织可用积分与近 30 日 / 历史累计预计可用天数；今日 / 近 7 天 / 本月 / 全部 / 自定义只作用于期间消耗、媒体产出、消耗构成与导出。独立的 `365 天活动` 可向前翻阅，在每日 / 每周 / 累计三种视图之间切换，并共用底部月份轴和目标附近的轻量悬停反馈。类型 / 成员 / 项目 / 模型构成来自同一演示账本，长维度使用 Top 5 + 其他并保持总额一致；演示数据应包含高峰、间歇、周末差异与退款，页面必须保留“演示数据”标识。任务级流水不在用量页重复实现，而由积分管理承接。组织用量数据位于独立数据层，后续应以 `GenerationTask`、计费快照和不可变 `CreditLedger` 替换，不能把当前演示记录当成真实账本。组织资料、角色、凭证和会话操作目前都不会写入共享组织、账号或 Membership；成员现有密码永远不进入页面或 API。
- 当前组织中心作为 `w/:workspaceId` 的子路由复用工作台已经加载的 `WorkspaceContext`，首次进入只额外读取组织成员，管理 / 积分 / 用量三个分区切换不重复请求工作区和项目数据。三个分区不再在内容区重复显示左侧导航已经表达的大标题，但保留隐藏的语义标题。用量页的稳定概览已并入 `365 天活动` 下方；每日 / 每周 / 累计统一使用 `53 周 × 7 天` 绘图区、固定高度、周列中心和月份轴，切换模式不得引发布局跳动。
