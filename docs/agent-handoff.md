...
- 后续公网更新从验收后的集成主线发布，发布前单独核对部署范围和环境。2026-09-05 核验公网生产部署为 `dpl_3e9rT3hKtg6p6XuKHKnxZdfF9hMv`，记录的代码 SHA 为 `ed5b4012b658e33652d7b4a2aa9ac80141a8230c`；2026-09-07 再次核验项目尚未连接 Git 自动部署。当前用户已要求推送、合并和同步公网，并要求公网可见三主体 12 张原图；此前“只同步代码、暂不部署”已不适用。代码、migration、私有桶、素材写入与部署须分别验收，不能根据本地或远端 `main` 推断线上版本。
