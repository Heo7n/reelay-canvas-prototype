...
外部试用采用 `npm run build:experience`，输出 `dist/experience`，与内部 `npm run build` 的 `dist/shell` 分开。构建期选择内存服务；只复制当前公开三主体 12 图并生成 512px WebP。输出自带静态 Vercel 路由配置，禁止包含 API、数据库配置、环境文件或内部登录代码。部署必须从该输出目录到单独的体验项目，不复用内部主域别名或内部 `.vercel/project.json`。不配置数据库、Storage 或账号环境变量，不执行 migration / seed。

`npm run preview:experience` 在 `127.0.0.1:5177` 启动只读静态服务；`/api/*` 为 404，`/app/*` 返回 React 壳。验证匿名直接进入、三主体与缩略图、修改在 SPA 切换中保留、刷新重置、新建项目深链回退、标签页隔离。临时上传以浏览器内存及 Blob URL 为边界；实际账号 / 密码、项目数据仍只属于内部环境。
...
Vercel Function 的请求与响应受 `4.5 MB` 负载限制，同源代理 PUT 仍限制为 `4 MiB`。超过代理上限且不超过 `50 MiB` 的文件，已鉴权 intent 返回私有 Storage 签名 PUT；finalize 读取实际内容核验 SHA-256、长度和元数据，完成前不进入素材库。音视频与超过 `4 MiB` 的原图，经同源 API 鉴权后返回 `private, no-store` 的 300 秒签名重定向，文件与 Range 由 Storage 交付。filesystem 上限仍为 `64 MiB`。公网桶单文件上限需与签名直传的 `50 MiB` 配套，桶保持私有并保留媒体 MIME 白名单。
