...
文档的单一职责与选读路径由开发工作流维护；本文不复制另一份文档路由。
...
每次替换一个交互或面板时，同步删除被替代的 DOM、CSS class、旧事件监听器、旧文档描述和不再使用的 querySelector / id。只清理本次替换范围，不留两条活动路径，也不扩展成无关重写。
...
阶段形成可运行、可回退的提交，未验证状态不打基线标签。自动测试、浏览器验收和公网验证各自说明覆盖；代码、数据、远端与公网状态分开报告。具体分支与提交规则以开发工作流为准，不把无关清理与新增大功能混成一个意图。
