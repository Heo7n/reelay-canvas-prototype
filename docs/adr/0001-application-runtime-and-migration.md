...
工程壳第一阶段不需要数据库、域名或服务器凭据。如果团队没有既定后端约束，推荐后续使用 TypeScript Fastify + PostgreSQL 的模块化单体，通过同源会话和 API 服务前端；在部署形态确认前不生成空后端项目。

## 待用户确认

以下答案会影响 Phase 0B 的 mock 深度和 Phase 0C 的服务端范围：

1. 多账号组织演示是同一浏览器依次切换账号，还是两个浏览器同时看到共享数据。
2. 首期账号标识优先使用用户名、邮箱还是手机号。
3. 演示权限是否先采用“组织所有者 / 编辑者”两级。
4. 未来主要部署在公网云服务还是企业内网；具体云厂商和域名可以以后再定。

在答案确定前，默认按“同一浏览器切换、用户名 + 密码、所有者 / 编辑者、公网云但供应商未定”推进可逆 mock。
